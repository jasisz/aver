...
3. checks every record and sum a bridge encoder reads (section 9.2); then the law statements: it declines a package that declares any constant `Q.M`, where `Q` is a namespace prefix of any law's `theorem` namespace and `M` a bridge `model` any law statement names, since that is where the name would resolve if the statement were read inside that namespace; and it declines when the value of `law_statement_<i>` of a law with bridges does not use every listed bridge's `model` constant (`Expr.getUsedConstants`), which a `_root_.<model>` spelled where elaboration drops it (a type ascription's type) would otherwise pass;
