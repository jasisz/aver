...
pub fn emit_verify_law_forall_auto_proof(
    ...
    if let Some(op) = op.as_ref() {
        if let Some(lines) = emit_sub_wrapper_law(vb, law, &intro_names, op, &fn_lean) {
            return Some(lines);
        }
    }
    ...
    match expr {
        Expr::FnCall(callee, args) => {
            if let Some(name) = expr_dotted_name(callee) {
                if let Some(fd) = find_fn_def_by_call_name(ctx, &name) {
                    if fd.effects.is_empty() && fd.name != "main" && fd.name != skip_fn {
