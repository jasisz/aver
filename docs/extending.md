...
Aver has no `for`/`while`. Iteration in the future will use goal-based constructs. This is how a `maintain` keyword would be added:
...
All functions live in namespaces (e.g., `Int.abs`, `List.len`, `Console.print`). Each namespace has one implementation, typed on `NanValue`: `call_nv()` plus the `<op>_nv` functions. To add a new function to an existing namespace:
...
1. Add a variant to `Expr` in `src/ast.rs` and an arm in `src/codegen/expr_walk.rs`, the one exhaustive child-walk over `Expr` (a new variant deliberately fails the build there)
