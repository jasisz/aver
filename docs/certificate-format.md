...
| `Artifact.lean` | `AverCert.Artifact.data : ArtifactData` and the byte facts of acceptance: `plans_ok`, `roles_ok`, `strings_ok`, `axes_ok`, `framing_ok`, `exports_ok`, `imports_ok`, `start_ok`, `closure_ok`, `whole_ok`, `envelope_ok`. `plans_ok` joins the per-plan checks with the other conjuncts of `plansAccepted`, each its own declaration (`rest_indices`, `rest_types`, `rest_data`, `rest_roles`, `rest_eqref`, `rest_newtypes`, `rest_inhabited`, `rest_cons`). A package with more than 32 plans moves the data and the heaviest facts to `ArtifactData.lean`, `ArtifactStrings.lean`, `ArtifactClosure.lean`, `ArtifactInterface.lean`, `ArtifactRestTypes.lean`, `ArtifactRestData.lean`, `ArtifactRestRoles.lean` and `ArtifactRest.lean`, which Lake can build in parallel. |
...
- `match_` with the arm shapes the emitter lowers: an Int literal cascade with a catch-all last, a two-arm Bool match, a two-arm Option or Result match, a user-variant `ref.test` cascade of two or more arms covering every constructor, a String literal cascade with `_` last, a single-arm flat tuple destructure, and a two-arm List match (`[]` and `[head, ..tail]` in either order, or either one first and `_` second);
- `construct` for user constructors and `Some`, `None`, `Ok`, `Err`, carrying the node's type;
- `interp` whose parts are all Strings;
- `list t items`, a list literal: `[]`, or a non-empty literal whose items all have the element type `t`, when the type table declares the `List<t>` cons helper (`listCons`) and the helper's planned signature is `(t, List<t>) -> List<t>`.
...
`AcceptedArtifact.consPinned` requires every declared cons helper to be a planned function whose plan is the wall's cons plan (`Grammar.isConsPlan`: its body is `List.prepend(local 0, local 1)`). The typing of a literal of `List<t>` requires that helper's planned signature to be `(t, List<t>) -> List<t>`, and its code entry is pinned like every other plan's, so a literal's calls reach a function whose meaning is `List.prepend`, proved rather than assumed.
...
- the report pins, each a theorem `_root_.AverCertChecker.report_pin_<k>` (21 of them), every field read through its projection function as above: that `Artifact.data.modBytes`, `modLen`, `manifest` and `wasip2ComponentEnvelope` are the checker's bytes, `AverCert.manifest` and the declared envelope; that `subject.artifactHash` is the recomputed hash; that `artifactRoot`, the obligation export names, `subject.exports`, the policies, the termination witnesses, `contracts`, `declaredUncertified`, `capabilities`, `start`, `hostRoleTable`, `stringHostRoles`, `target`, `profile` and `abi` are their JSON values; and that `ClaimAxes.reportEntries` and `ClaimAxes.reportFacets` are the JSON's names, class and facets. Each is proved by `rfl`, except four computed from every plan. The report entries are proved by `first | decide +kernel | rfl`. The facets, the policies and the termination witnesses are computed per call group, and deciding them as stated would ask every exported plan for its group's members, a walk over all plans each time. Their proofs instead rewrite the pin's left side by a wall equation that holds for every package (`ClaimAxes.reportFacets_eq_fast`, and `map_policy_of_derived` and `map_termination_of_derived` from the accepted certificate's `obligationsDerived` conjunct) to the same data computed once per group of the plans' runs (`ClaimAxes.runsOf`: the producer lists a group's plans together, callees first; when the runs' groups are strictly increasing each run is its whole group, and otherwise the computation falls back to the definition), then decide that. The statement stays the checker's; the proof cites only wall lemmas and the package's `certificate`, and a package can make it slower but not different;
...
4. collects the axioms of `AverCertChecker.checked` and of every report pin, and declines on any axiom outside `[propext, Classical.choice, Quot.sound]`. A report pin closed by `decide +kernel` through a `sorry`-backed package decision procedure carries `sorryAx` into this audit;
...
The witness and the audit run on every `verify` and `check`, after any cache restore, and are never cached. The audit collects axioms over the declarations themselves, along the edges `Lean.collectAxioms` walks (a constant's type, the value of a definition, theorem or opaque, the constructors of an inductive), and does not read axiom data recorded in an `.olean`. It keeps one memo for all roots, since the roots of a large certificate share most of their closure (btc-listener: about 700 roots). The memo records a constant's axioms only once they are final: constants that reach each other in a cycle (an inductive and its constructors) get one set for the whole cycle, recorded when the cycle is complete (Tarjan's strongly connected components), so each root reports exactly the axioms reachable from it, the same set a fresh `collectAxioms` returns, whichever roots were asked before it. The final replay re-checks terms but enforces no axiom policy, so the audit is load-bearing. The token gate of stage 7 is a hardening layer in front of it; the audit and kernel acceptance are what the verdict rests on.
