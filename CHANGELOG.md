...
- **`Run.lastTurn()` says how long the generated loop waited and worked.** Since the loop does the waiting, a process that timed its own calls saw no wait at all and counted the whole wait as work. `Run.lastTurn()` answers a `Run.Turn` with `waitedMs`, how long the loop's wait before the current turn took, and `workedMs`, how long the turn before it worked, from the return of its wait to the start of this one, in whole milliseconds of a monotonic clock. A process or an answer module calls it from anywhere and declares the effect `Run.lastTurn`. Both numbers are 0 while the loop seats the processes, and `workedMs` is 0 in the first turn. It works on the VM, in generated Rust, on wasm-gc, on wasip2 and under the JavaScript adapter.
- **Only a program that reads it pays for it.** The loop reads the clock twice per turn, right before and right after its wait, only when some function of the program declares `Run.lastTurn`. Every other program generates the same loop, the same wasm-gc and wasip2 modules and the same Rust code as before, and its recordings have no new entries. What does move, once more in this release as it did for `Run.fail`, is the `Run` capability's contract hash, because the capability has more operations: a generated crate's provider table, a recording's header and a certificate's source model name it.
