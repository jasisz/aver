...
The bridge KPI is now **23 accepted-artifact-v1 / 0 legacy-witness-v1**. For
composition, plans carry only `selfSum` versus an ordered chain of callee export
...
The detailed migration narrative in the next paragraph preserves the S1–S4
stage descriptions; its embedded 21/2 counter is historical. The normative S5
counter and family table above are 23/0.

The header's *23 exports across 14 manifest classes* and this section's *nine certified classes* count the same certified functions at two different granularities, not two conflicting totals. The nine-class taxonomy below is the coarse byte-recognizer grouping — the source shapes the classifier recognizes. The emitted `cert-manifest.json` enumerates that same surface more finely as 23 exports across 14 manifest classes, and tags each export with the acceptance path it rides. Twenty-one of the 23 exports ride the plan-first `accepted-artifact-v1` path — the scalar `expr-fragment-v1` fragments plus the `adt-constructor`, `verbatim-string-eq`, `verbatim-string-concat`, fuel-recursion (`recursion-plan-v1`), mutual-recursion (`mutual-plan-v1`), verbatim ADT-match (`verbatim-plan-v1`) and Int-face ADT-match (`int-dispatch-v1`) bridge families; the remaining two ride the legacy `legacy-witness-v1` byte-recognizer path. The fuel-recursion bridge is a byte-origin veneer: the fuel-induction proof face and the emitted body literal are unchanged, but the manifest carries a checked `recursion-plan-v1` plan that the audited Lean checker validates context-sensitively — the plan must be one of the two recognised fuel-recursion grammars, every self-call must target the export's own byte-derived function index, every host call must cite the byte-derived box/combinator/sub role table, and the export's declared function type must be the canonical carrier signature — before the audited Lean lowerers reproduce it byte-for-byte and the Lean byte slicer finds those exact bytes inside the module. The recursive body's byte-origin is thereby established in-kernel instead of only by the Rust disassembler; a certified recursion body the canonical plan cannot reproduce byte-exactly (for example one carrying normalized local-alias hops) simply keeps the legacy witness route with no claim, fail-closed. The verbatim ADT-match bridge (`verbatim-plan-v1`) is the same kind of veneer for the `Cod := WVal`/`verbatimRepr` widened-match and variant-dispatch classes: because those bodies spill the multi-use scrutinee to a scratch local (which the pure-ANF expr-fragment grammar cannot express) the manifest carries a dedicated `VerbatimRawPlan` — a right-nested `ref.test` cascade of per-tag leaves (field projection, `array.new_data` string literal, `ref.null`, or `f64.const`). Its result signature is a disjoint plan variant: either `.refNull heapTy` or `.f64Scalar`. The audited type-section parser derives the actual result kind from module bytes and requires an exact match, so adding `[eqref] -> [f64]` does not relax the existing `[eqref] -> [(ref null heapTy)]` path or its `0x63` nullability requirement. Because there is no host or self call to bind, the byte-equality gate (the audited Lean lowerer reproduces the exact code entry and the slicer finds it by export name) carries most of the binding: for f64 plans it pins the scalar projection local, the `if` block's `0x7c` result type, and all eight `f64.const` immediate bytes. The other in-kernel bind requires every `array.new_data` leaf's claimed payload to equal the byte-pinned contents of its passive data segment (the code entry encodes only the segment index and copied length, so an equal-length payload substitution otherwise keeps identical bytes). The semantic widened-match model remains independent of this byte-origin veneer. In particular, the f64 case certifies verbatim structure and exact bytes, not floating-point arithmetic; no float-value reasoning enters the claim schema or TCB. With those binds, no two distinct plans lower to the same accepted artifact; a body byte-noisier than the canonical dispatch keeps the legacy route with no claim, fail-closed. The Int-face ADT-match bridge (`int-dispatch-v1`) is the analogous veneer for the `Cod := Int` variant-dispatch and widened-Int-match classes: the `cases`-spine proof face and the Int-valued model are unchanged (the model stays an independent read anchor, never derived from the plan), while the manifest carries a dedicated `IntDispatchRawPlan` — a right-nested `ref.test` cascade whose arms project the tested variant's payload and either return it or combine it with a boxed constant through a contracted host helper, with a boxed-constant terminal default. Unlike the verbatim family the arms consume host contracts, so the plan names helpers by ROLE only: the audited lowerers are parameterized by the claim's byte-derived host-role table (a plan cannot invent a callee), the acceptance predicate requires that table to map roles to pairwise distinct indices (`hostTableIndicesDistinct` — under a duplicated table two plans differing only in an arm's role would lower to identical bytes, blinding the byte gate), and the scratch-local layout is a fixed function of the arm count rather than plan data. The export's declared function type must be exactly the unary `[eqref] -> [(ref null carrier)]` signature (the same in-kernel shape check the verbatim family uses, with the Int carrier as the result heap type); the obligation's whole host builder must additionally EQUAL the canonical builder for the claimed table (`obligation.host = intDispatchCanonicalHost carrier hostTable`, one extensional definitional equality mirroring in-kernel the checker's whole-host pin — otherwise a role permutation in the plan plus a consistently permuted table cancels out byte-identically, and a merely sampled probe would leave unsampled slot behaviour free); and the obligation's code table must carry the plan-lowered body with the exact canonical locals count (`armCount + 2`, what the byte lowering declares — an existentially-free count would admit a zero-locals table whose body traps immediately, making the partial-correctness obligation vacuously true). Everything else — tags, arm order, roles, constants, operand order, the default — reaches the lowered bytes and is pinned by the byte-equality gate, and the per-role indices a claim cites must be confirmed by the strict byte-derived role table (carrier-binop signature, strict first-arithmetic, uniqueness — an ambiguous helper leaves the role unbound and the export on the legacy route). A body byte-noisier than the canonical dispatch keeps the legacy route with no claim, fail-closed. The byte-recognizer families and the manifest classes therefore describe the same work, and `artifact_bridge_counts` in the manifest reports the 21/2 split directly.
...
As of schema 40, the current `cert_goals` matrix is **23 accepted-artifact-v1 / 0 legacy-witness-v1**; the historical 21/2 wording above describes the earlier migration stage. The JSON fixture remains 12 certified exports and moves from **9/3 to 11/1**, with only `jsonFloat` left on `legacy-witness-v1`.
