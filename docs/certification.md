...
The header's *23 exports across 14 manifest classes* and this section's *nine certified classes* count the same certified functions at two different granularities, not two conflicting totals. The nine-class taxonomy below is the coarse byte-recognizer grouping — the source shapes the classifier recognizes. The emitted `cert-manifest.json` enumerates that same surface more finely as 23 exports across 14 manifest classes, and tags each export with the acceptance path it rides. Fourteen of the 23 exports ride the plan-first `accepted-artifact-v1` path — the scalar `expr-fragment-v1` fragments plus the `adt-constructor`, `verbatim-string-eq`, `verbatim-string-concat` and fuel-recursion (`recursion-plan-v1`) bridge families; the remaining nine ride the legacy `legacy-witness-v1` byte-recognizer path. The fuel-recursion bridge is a byte-origin veneer: the fuel-induction proof face and the emitted body literal are unchanged, but the manifest carries a checked `recursion-plan-v1` plan (self-call, descent through the contracted `sub` role, base/step arms) that the audited Lean lowerers reproduce byte-for-byte and the Lean byte slicer finds inside the module's own bytes, so the recursive body's byte-origin is established in-kernel instead of only by the Rust disassembler. The nine byte-recognizer families and the fourteen manifest classes therefore describe the same work, and `artifact_bridge_counts` in the manifest reports the 14/9 split directly.
