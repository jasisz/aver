...
Named for ℤ — the *Zahlen*. `Int` becomes the mathematical integer on every backend, closing the last place a verified program could disagree with its proof at runtime; and the prover learns to close whole families of laws from the helper laws you (or the Method loop) write, retiring the per-figure built-in proofs it used to carry.

### Added

- **Membership, sortedness, and conditional inductive laws prove universally through one generic driver fed by your helper laws — no per-figure built-in proofs.** The auto-prover no longer carries hand-written Lean templates for specific shapes (insertion-sort sortedness, list membership, match-dispatch folds). It inducts generically and consumes the helper `verify ... law` blocks you — or the Method agent loop — supply as lemmas, so one mechanism closes a whole family. A figure that needs a new auxiliary fact is closed by *stating that fact as an Aver law*, not by extending the compiler; the bespoke proof emitters those shapes used to need were deleted because the generic driver subsumes them. Lean backend.
- **Bool-valued predicate laws can be stated with `holds`.** A law whose claim is a predicate that must hold for all inputs — `sorted(sort(xs)) holds` — is written directly instead of being forced into an `lhs => rhs` equation; the boolean predicate is bridged to a proposition and proven for every input, not just the sampled domain.
- **Tail-recursive accumulator functions prove equal to their spec on both backends.** A function that threads an accumulator — a tail-recursive factorial, sum, product, or list-reverse — proves equal to its direct recurrence (`qrev(x, []) = rev(x)`, `factTR(n, 1) = fact(n)`) by generalizing the induction over the accumulator, universally on Lean and additively on Dafny, across the `{List, Nat} × {+, ×}` grid.
- **Single-list conditional laws are tried universally and fall back to a sound bounded check automatically.** A `when`-law over one list that the generic driver can close is proven universal with no annotation; one it cannot close keeps its honest bounded sample check. The choice is made empirically from a single build rather than a static guess — try universal, fall back to sampled. Lean backend.
...
- **`aver proof --minimize` rewrites each exported proof to the tactic that actually closed it — and can only ever tighten a proof, never break one.** The auto-prover cannot know statically which tactic a given law needs, so at every law it pins a portfolio of alternatives — `first | (simp …) | (… <;> omega) | … | sorry` — ending in a `sorry` floor so a shape no strategy recognizes degrades to a caught gap rather than a hard build error that would poison the whole module. That hedge is robust but noisy: the shipped proof carries alternatives that never fired, plus a `sorry` floor even where the proof closed for real. `--minimize` (Lean-only; implies `--check`) resolves the hedge against a real build. It prefixes every branch with a marker and builds the project ONCE; Lean tries a portfolio's branches left to right and commits to the first that closes, tracing each it reaches, so the winning branch is the highest-numbered marker that surfaces (the branches tried and discarded before the winner trace too — the markers are not rolled back). Each portfolio is then collapsed to its winner and the project is RE-VERIFIED; if a theorem no longer closes — a mis-parsed winner — it keeps its original portfolio. So minimization is **fail-safe** (it never ships a proof that does not build) and **status-preserving** (it keeps exactly the branch Lean committed to): a law proven for real drops its alternatives and its `sorry` floor and reads like a hand-written proof, while a law that only closed via its floor collapses to a bare `sorry` — the honest gap is kept, never silently dropped, so the `sorries` / `universal` numbers from `--check` are unchanged. It is an opt-in polishing pass for a proof you are about to ship or review (it costs two extra `lake build`s), not part of the normal verify loop. Lean backend.
...
- **`Int` is now arbitrary-precision (ℤ) on every backend — VM, Rust, and wasm-gc.** Integer arithmetic no longer overflows or wraps: a result that exceeds 64 bits keeps going instead of silently truncating, so `Int` is the mathematical integer everywhere, matching what proofs and `verify` already assumed. This closes the last place a verified Aver program could disagree with its proof at runtime. Small values keep a native-integer fast path; only genuinely large magnitudes pay for big-integer storage. Integer *literals* are arbitrary-precision in source too: a constant whose digits exceed 64 bits — `let big = 1267650600228229401496703205376` — is written directly (no `Int.n("<digits>")` wrapper) and behaves like any other `Int` everywhere a normal literal does — in arithmetic, through parameters, and as a `verify` expected value — digit-exact on every backend. The one place a `>i64` literal isn't accepted is a match *pattern*; match on a comparison there (`match n == <value>`), which the checker points you to. See `examples/core/big_integers.av`.
...
- **The brute-force lemma-discovery enumerator is gone — `aver proof --discover` and `--emit-laws` are removed.** The enumerator guessed auxiliary helper laws by enumerating candidate equations over a law's proof cone and VM-filtering them; in practice it closed none of the hard unproven tasks directly, contributed nothing to the measured proof-corpus coverage (which never ran it), and carried the subsystem's heaviest soundness surface for an autonomy no task needed. The capability it fed is kept: a proof still builds on auxiliary helper laws via the committed-lemma feedback loop (`ProofStrategy::SimpOverLemmas`), which re-pins an in-scope `Induction` law to simp over lemmas committed in a `DiscoveredLemmas.lean` and re-proves them in the same `lake build`. Those lemmas are now produced deliberately — by hand or by the Method agent loop (the `the-method` skill, which proposes auxiliary laws and lets the kernel certify them) — rather than by enumeration. The enumerator's full source is archived at the git tag `archive/lemma-discovery-enumerator`.

### Fixed

- **`aver verify` now fails loudly on a file that does not parse.** A file with a syntax error used to be silently dropped — `verify` reported "No verify blocks found" and exited 0, hiding the parse error and any real verify blocks behind it (a CI step using bare `aver verify` would treat a broken file as green). It now surfaces the parse error and exits non-zero, matching `aver check` and `aver audit`. A genuinely empty or comment-only file is still reported as "no verify blocks" with success, as before.
- **A `verify` case whose expected value wraps onto an indented continuation line no longer breaks the cases that follow it.** Wrapping a long right-hand side (e.g. a nested event-literal record) is supported, but the wrap's closing dedent was mistaken for the end of the case block, so every case after a wrapped one was parsed at the top level and reported a cryptic "expected expression, found '=>'" on the wrong line. The wrap's indentation is now balanced correctly, so a block may freely mix wrapped and single-line cases, and `aver fmt` handles such blocks. (Previously a wrapped case happened to work only when it was the last in its block.)
