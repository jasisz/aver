...
- ✅ Console, Args.get, Env.get, Time, Random, Disk, outgoing Http, and the connected half of Tcp — `connect`, `close`, `writeLine`, `writeBytes`, `writeNow`, `readLine`, `readBytes`, `readSome`, `readNow`, `poll`, `send`, `sendBytes`, `ping` — including binary reads and writes, the two non-blocking primitives, and `Sending` write-readiness polling.
