...
- **A map's key type must be one that has an ordering.** A map iterates its entries sorted by key — when you run a program, in a compiled binary, and in the exported proof — so the key needs an order both sides can state. `Int`, `String` and `Bool` have one. `Float` does not: a NaN has no place in the finite range. Neither do records, variants, tuples and lists, which were being ordered by their printed form, which is not a property of the value. Any of those in key position is now a type error at `aver check`, wherever the key type is decided: an annotation, a map literal, the key argument of `Map.set`, the pair list handed to `Map.fromList`, a `verify` given, a capability operation's signature, or a signature reached through a dependency. Float and every other type stays legal as a map *value*, and `Map<T, Unit>` sets are unaffected for the three key types that remain.

  This closes the last way a passing `aver verify` and a certified `aver proof` could disagree about a map. `aver proof` refuses to export a claim whose key type it cannot order, but it read that key type from every signature the claim reached rather than from the map actually being read — so one modelled key anywhere in reach opened the gate for all of them. A function with an unused `Map<String, Int>` parameter that built a `Map<Float, Int>` in a local binding exported `keysOf [] = [2.0, 1.0]` as a kernel-certified theorem while `aver verify` reported the same case failing with `[1.0, 2.0]`. Programs of that shape no longer typecheck, and the export refusal stays as a backstop for a key type the exporter cannot see at all.

- **An explicitly re-exported type keeps its identity through the whole tree.** A module with `depends [Low]` and `exposes [Item]` handed importers a fresh `High.Item` rather than an alias for `Low.Item`, so a value built through the facade did not match the type the implementation module declared, and a record constructed on one side of the boundary was rejected on the other. The re-export now carries the original type's identity along with its public shape — record fields, sum constructors, variants, opacity — through as many facades as you chain, and `Facade.Item` works as a qualified alias. Default exposure still covers local declarations only: a dependency's type crosses the next boundary only through an explicit `exposes [Type]` or `exposes opaque [Type]`, a facade does not grant the right to name the implementation module's qualifier, and two different types with the same name stay ambiguous instead of the first one winning.
...
- **Deciding whether two maps are the same map is now refused when the proof model cannot order the key.** The model keeps a map's entries sorted by key, which is what makes two maps holding the same entries the same value — but the sort needs an ordering. Where the model could not state one it kept the entries in the order they were written, so it could tell two maps apart that the runtime considers equal, and `a != b` over two maps holding the same pairs was `false` when you ran it and provable in Lean. The key types that could do this are no longer writable (see the ordered-key entry above); the refusal stays for the case where the compared map's key type is not decided anywhere the exporter can see.
...
- **The map iteration-order refusal now follows a call through an interpolated string, through a tail call, and into a function passed by name.** The refusal was meant to follow calls rather than spelling, so an observation hidden a few functions below the law is still caught. Three shapes escaped it. A call inside `"{firstValue(m)}"` was invisible, because proof export deliberately keeps interpolation unlowered and the walk skipped the segment. A call in tail position inside a recursion group was invisible, because the tail-call transform runs before typechecking, so every such call has already changed shape by the time the gate looks — a law over a function whose whole body is `readValues(m, n)` saw a cone containing nothing but itself. And a function handed to a callback parameter — a reader applied as `step(m)` and called as `run(readKeys, m)` — was invisible, because a name in argument position is not a call: the walk stopped at the caller and never looked inside the function whose body does the reading.
...
- **A map iterates in one order on the VM, in compiled Rust and in the exported proof.** `Map.keys`, `Map.values` and `Map.entries` return entries sorted by key — that part was already true when you ran a program — but `aver proof` exported a model that kept insertion order instead. A map built by writing `z`, `a`, `m` has values `[2, 3, 1]`, which is what `aver verify` reports; the exported Lean said the values were `[1, 2, 3]` and the kernel accepted it. Either tool could confirm a claim the other refuted, on the same source, on the same commit.
  ...
  One consequence to know about if you have integer keys: they sort numerically — a map holding `2` and `10` iterates `[2, 10]`; running a program used to give `[10, 2]`, because that path sorted keys by their printed form rather than by the documented comparator. Order-blind claims — `Map.len`, `Map.get`, `Map.has` — export exactly as before.
...
- **The map key comparator is a total order.** It fell back to comparing raw bit patterns whenever a comparison was undecided, and that is not a total order: a NaN sits above `1.0` and below `-1.0` by bit pattern while `-1.0` is below `1.0`, a three-way cycle. On a map wide enough for the sort to notice — around sixty keys — reading it ended the process with `user-provided comparison function does not correctly implement a total order` instead of returning. The key types a program can write are now `Int`, `String` and `Bool`, none of which could reach that path; the comparator is fixed underneath them anyway, because the runtime keeps handling values the language no longer lets you key on.
