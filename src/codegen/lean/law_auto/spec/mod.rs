...
use super::super::expr::{emit_expr, resolve_rewrite_output};
...
pub(super) fn emit_spec_function_equivalence_law(
    ...
    let bodies_match_exactly = emit_expr(&resolve_rewrite_output(impl_body, ctx, None), ctx)
        == emit_expr(&resolve_rewrite_output(spec_body, ctx, None), ctx);
