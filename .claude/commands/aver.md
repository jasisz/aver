...
verify tick
    tick(fresh()) => (State.update(fresh(), armed = true), Clock.TickReply.Later(Wait.Wake.After(50)))
    tick(State.update(fresh(), armed = true)) => (fresh(), Clock.TickReply.Now(Clock.Tick.Tock))
...
fn armed(state: State) -> Tuple<State, Clock.TickReply>
    ...
    match state.armed
        ...
        true -> (State.update(state, armed = false), Clock.TickReply.Now(Clock.Tick.Tock))

verify armed
    armed(fresh()) => (State.update(fresh(), armed = true), Clock.TickReply.Later(Wait.Wake.After(50)))
    armed(State.update(fresh(), armed = true)) => (fresh(), Clock.TickReply.Now(Clock.Tick.Tock))
```
...
**The generated coordinator** is asked for by the `[run]` table. Its four keys name one module, the entry module, which is where the loop is generated. The entry module lists `Work`, every answer module and every job kind in `depends`, because the generated loop names them even though the entry's own source never does; `error[run-binding]` names the missing one:
...
The program declares a `Pending` sum with one constructor per process — named after the process with a capital letter, `Ticker` for `ticker` — carrying the instance number of the request it waits on and the wake it is parked on, the `View` record the loop fills — exactly these six fields — and three pure policies over it. The entry module needs no `exposes`, and `ticker` above sits in this same file:

```aver
module Node
    intent = "One process, one answer module, one job kind and three policies; the loop between them is generated."
