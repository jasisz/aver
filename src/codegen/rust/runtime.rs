...
pub fn generate_runtime() -> String {
    ...
    enum AverListInner<T> {
        ...
        Prepend { head: T, tail: AverList<T>, len: usize },
        Concat { left: AverList<T>, right: AverList<T>, len: usize },
    }
    ...
    pub fn string_join(parts: &AverList<String>, sep: &str) -> String {
...
struct HttpResponse {
    pub status: i64,
    pub body: String,
    pub headers: aver_rt::AverList<Header>,
}
...
pub fn generate_http_runtime() -> String {
    ...
    pub fn post(url: &str, body: &str, content_type: &str, headers: &aver_rt::AverList<Header>) -> Result<HttpResponse, String> {
        body_request("POST", url, body, content_type, headers)
    }

    pub fn put(url: &str, body: &str, content_type: &str, headers: &aver_rt::AverList<Header>) -> Result<HttpResponse, String> {
        body_request("PUT", url, body, content_type, headers)
    }

    pub fn patch(url: &str, body: &str, content_type: &str, headers: &aver_rt::AverList<Header>) -> Result<HttpResponse, String> {
    ...
    fn body_request(method: &str, url: &str, body: &str, content_type: &str, headers: &aver_rt::AverList<Header>) -> Result<HttpResponse, String> {
    ...
    fn build_response(resp: ureq::Response) -> Result<HttpResponse, String> {
        ...
        Ok(HttpResponse { status, body, headers: aver_rt::AverList::from_vec(headers) })
    }
}"#
    .to_string()
}
...
pub fn generate_http_server_runtime() -> String {
            ...
            let request = match parse_http_request(&mut stream) {
                ...
                Err(msg) => {
                    let _ = write_http_response(&mut stream, &HttpResponse {
                        status: 400, body: format!("Bad Request: {}", msg), headers: aver_rt::AverList::empty(),
                    });
                    continue;
                }
            };
    ...
    fn parse_http_request(stream: &mut TcpStream) -> Result<HttpRequest, String> {
        ...
        Ok(HttpRequest { method, path, body, headers: aver_rt::AverList::from_vec(headers) })
    }
