...
stdlib/
  ...
  run.av              — Run capability: Run.Wake (when the generated loop asks an answer module again), Run.fail (end the run with a reason) and Run.failure (the loop reads it back), Run.lastTurn (how long the loop waited and worked around the current turn; the loop marks its waits with Run.waitStarts/Run.waitEnds only in a program that declares it); VM/Rust provider in aver-rt/src/provider/run.rs, module globals on wasm in src/codegen/wasm_gc/run_fail.rs and run_turn.rs
```
