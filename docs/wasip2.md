...
- `--world <world>` — which WIT world the component targets. Two values: `wasi:cli/command` (default — long-running process exporting `wasi:cli/run.run`) and `wasi:http/proxy` (HTTP server, exporting `wasi:http/incoming-handler.handle`; shipped in 0.19). The proxy world requires `main` to end in exactly `HttpServer.listen(port, handler)` — the compiler reads the handler fn reference out of that call and wraps it in the canonical-ABI incoming-handler export. Programs whose effects do not fit the chosen world fail at compile time with `target-effect-unsupported` pointing at the offending call.
...
| `HttpServer.listen` | `wasi:http/incoming-handler.handle` export (Phase 3 / 0.19 shipped). Requires `--world wasi:http/proxy`. The handler wrapper decodes the host-supplied incoming-request into an Aver `HttpRequest` (method via the 10-case variant, path-with-query split into path/query, headers iteration as `Map<String, List<String>>`, body via `incoming-body.stream` + drained `input-stream.blocking-read`), runs the user's `fn(HttpRequest) -> HttpResponse`, marshals the result into an outgoing-response (`outgoing-response` constructor + `set-status-code` + body via `outgoing-body.write` + chunked `blocking-write-and-flush` + `outgoing-body.finish`), and calls `response-outparam.set`. `Content-Length` is synthesised from the response body byte count. The `port` argument is ignored at codegen time — the host's listener flag (`wasmtime serve --addr=:N` etc.) binds the socket. |
