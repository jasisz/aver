...
- wasm-gc and wasip2: the current Aver backend runs a job inline at `begin`, so a `take` in the next expression answers `Some`; parallel jobs through host workers are not implemented yet; `[work] max-jobs` is ignored with `warning[work-max-jobs-ignored]`; on wasip2 a generated loop keeps `View.stopping = false` and ends through its policy or normal exhaustion, because WASI 0.2 has no signal subscription; an explicit `Process.stopRequested` call is still rejected
