...
- **A bare name in an effect list must be `yield` or a capability namespace.** A function's `! [...]` and a module's `effects [...]` used to accept any bare identifier in silence, so a misspelled `! [yeild]` bought neither the effect nor a diagnostic; it is now an error naming the unknown effect and saying how an effect is written.
