...
This expands to two cases. Multiple `given` lists expand as a cartesian product, capped at `10_000` cases (`[verify] max-cases` in `aver.toml` moves the cap). Stub names may be local (`lowDie`) or qualified imports (`Helpers.lowDie`).
...
- `aver verify` runs it as a **finite sample check**: the cartesian product of the `given` domains is enumerated (capped at 10,000 cases, or `[verify] max-cases`) and evaluated against the law's RHS using whatever stubs you supplied.
...
capped at `10_000` cases per block — the same `[verify] max-cases`
ceiling as parser-side declared expansion. Over-budget blocks fail with
