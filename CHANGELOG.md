...
- **wasm-gc and wasip2 now walk fused `String.chars` loops with a UTF-8 cursor instead of allocating a list of one-character strings.** The two backends implement the same internal cursor and Unicode codepoint-folding contract as the VM and generated Rust while continuing to refuse unsupported mutable buffer, byte-sink, and list-builder intrinsics. On the real 2,000-block btc-listener workload under Node/V8, five-run means fell from 0.627 s to 0.501 s for the indexed-output walk and from 0.739 s to 0.520 s for the UTXO walk; ancestry-attributed `Bytes.fromHex` profile samples fell from 10.06% to 3.40% and from 10.77% to 5.26%, respectively. Embedded Wasmtime improved by 18.3% and 15.1% on the same walks. This is the cursor-only first stage of [#1208](https://github.com/jasisz/aver/issues/1208); byte sinks and string builders remain a separate follow-up.
