...
- **The generated loop no longer carries the job limit.** `__maxJobs`, `__roomLeft`, `__startable<K>` and their laws are gone, and so is the `room` field of the view: a program with custom policies drops `room: Int` from its `View` record. Every task a job kind's `task` offers in a turn is begun in that turn, so `started` must remove the task it is handed or the turn does not end. Before, a program built without `max-jobs` wrote the building machine's core count into its generated source, so the same program meant something else, and certified differently, on another machine.
...
- **A negative `After` or `Either` deadline in a generated loop is due at once.** It used to be read as "no deadline yet", so the turn's timeout depended on which slot held it.
...
- **An empty `[run]` never seats a yielding helper of a dependency.** `aver run`, `check` and `verify` used to lower a dependency with the default loop bound to it and refused a keyed helper there for having nothing to seat it with.
- **`[run]` policies named in a module other than the one the entry is loaded as are refused** with `error[run-binding]`. Such a program generated no loop, and an entry without `main` ran nothing and exited 0.
...
- **`aver check` no longer refuses an answer module, or a helper that asks an answered capability, for not seeing the answer module in its own dependency cone.** The entry judges that binding.
...
- **When a generated run ends, the jobs that parked requests wait on are cancelled** as well as the jobs in the loop's own table, so a job an answer module began itself no longer outlives the run. `docs/services.md` describes that pattern, the job as a request, which needs no `task`, `started` or `landed`.
- **The documentation states exactly what `aStartedTaskIsNotAskedAgain` guarantees**: the task just started is not the next one offered. It used to claim that no task is ever started twice.
