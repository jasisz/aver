...
| `Module.lean` | `CertModule.wasmSha256`, the artifact hash string `Schema.Holds` compares against. |
...
The producer never emits, and MUST NOT emit, `ArtifactBytes.lean`, `ArtifactComponentBytes.lean`, `CheckerWitness.lean`, `lakefile.lean`, `lean-toolchain`, any wall source, any build cache, or plan sidecars (`*.plan`, JSON plan trees).
...
| `wasm_sha256` | 64 lowercase hex | kernel-pinned | SHA-256 of the delivered artifact (the whole component for wasip2). The verifier MUST recompute it and reject a mismatch; the witness pins `subject.artifactHash` to the recomputed value, and `Schema.Holds` requires `artifactHash = CertModule.wasmSha256`. |
...
The declared function type of every present role except `divmod` is pinned separately (`roleTypesPinned`, section 7.2).
...
- `roleTypesPinned`: every present role's declared function type is exactly the type its role fixes: `box` `i64 -> carrier`; `add`, `sub`, `mul` `carrier carrier -> carrier`; `cmp` and `eq` `carrier carrier -> i32`; `toIndex` `carrier -> i32`; String equality `$string $string -> i32`; concatenation `Vector<String> -> $string`. Body equality does not constrain the declared type, and a helper declared at a supertype still validates by subtyping, so this conjunct is required. *Known gap:* `divmod` has no declared-type pin; its body is pinned by template only.
...
**Hash.** The verifier MUST hash the delivered artifact and reject unless it equals `wasm_sha256`. The witness pins `subject.artifactHash` to that hash, and `Schema.Holds` pins it to `CertModule.wasmSha256`.
...
The rendered text passes the statement gate (nonempty, at most 16000 bytes, no control character, no `:=`, `--` or `/-`, balanced `()[]{}⟨⟩`), and every dotted name in it MUST be `_root_.`-qualified, because the pin elaborates at the root namespace.

### 9.3 `laws`

Each entry is an exact object `{label, theorem, statement, corollary, bridges}`. `label` is the source `module.fn.law` identity and `theorem` the fully qualified model theorem; both are plain dotted identifiers of at most 200 bytes, and only `theorem` may carry `'` after a segment's first character. `corollary` MUST be the label with every `.` replaced by `_`, and corollaries are unique. `statement` is the theorem's universal statement on one line and passes the statement gate. `bridges` is an array of export names, each declared in `sourceBridges` and listed at most once; the producer lists every source function the statement mentions when all of them have a bridge, and leaves it empty otherwise.

The package's `Laws.lean` proves `AverCert.Laws.<corollary> : (statement) ∧ Holds manifest`, inside the model theorem's own namespace. A claim with a nonempty `bridges` also proves `AverCert.Laws.<corollary>_bridged`, the same conjunction plus the rendered statements of the listed bridges in order. The `_bridged` name is derived, never transported.
...
## 10. Checker witness and axiom guard

After the package builds, the verifier writes `CheckerWitness.lean`, which is never read from the package. It imports `Lean`, `AcceptedArtifact`, `ArtifactBytes`, `Manifest`, `Artifact`, `Laws` and `Bridge` when declared, and `ArtifactCertificate`, and contains:

- one bridge pin per `sourceBridges` entry, at the root: `theorem _root_.AverCertChecker.bridge_pin_<i> : (<rendered statement>) ∧ (_root_.AverCert.Schema.Holds _root_.AverCert.manifest) := _root_.AverCert.Bridge.<export>_certified`;
- one law pin per `laws` entry, inside `namespace <prefix>` for the model theorem's namespace: `theorem _root_.AverCertChecker.law_pin_<i> : (<statement>) ∧ (… Holds …) := _root_.AverCert.Laws.<corollary>`, and, for a claim with bridges, `_root_.AverCertChecker.bridged_law_pin_<j>` at the bridged conjunction, numbered over the bridged claims. The namespace matters: `open <prefix> in` at the root resolves names differently from inside the namespace;
- `rfl` pins that `Artifact.data.modBytes`, `modLen`, `manifest` and `wasip2ComponentEnvelope` are the checker's bytes, `AverCert.manifest` and the declared envelope;
- `rfl` pins of `subject.artifactHash` (to the recomputed hash), `artifactRoot`, the obligation export names, `subject.exports`, the policies, the termination witnesses, `contracts`, `declaredUncertified`, `capabilities`, `start`, `hostRoleTable`, `stringHostRoles`, `target`, `profile` and `abi`;
- pins of `ClaimAxes.reportEntries` and `ClaimAxes.reportFacets` to the JSON's names, class and facets, by `first | rfl | decide +kernel`;
- `theorem AverCertChecker.checked : AverCert.AcceptedArtifact.accepted AverCert.Artifact.data := AverCert.Artifact.certificate`;
- the axiom guard.

The guard is a `run_cmd` that runs `Lean.collectAxioms` on `AverCertChecker.checked` and on every checker-named pin, never on the package's bare corollary names. The whitelist is `[propext, Classical.choice, Quot.sound]`, matched as exact `Lean.Name`s. For the root the guard throws on any other axiom, which fails elaboration and rejects the package. For each pin it logs one line instead: `AVER_LAW_AUDIT`, `AVER_LAW_BRIDGE_AUDIT` or `AVER_BRIDGE_AUDIT`, then the pin name, then `ok` or `axioms <name>[,<name>…]`. The verifier reads the lines back and requires exactly one well-formed line per declared pin; a missing, renamed, repeated or malformed line declines the package. Only an `ok` line grants credit.

The guard runs on every `verify` and `check`, because the witness is written after any cache restore and is never cached. The final replay re-checks terms but enforces no axiom policy, so the guard's place in the always-fresh witness matters. The package cannot ship a competing guard, because `run_cmd` is on the token list of stage 7. That list is a hardening layer; the guard and kernel acceptance are what the verdict rests on.
...
- **Caches as evidence.** Package caches are ignored; local caches cannot skip the witness or the replay.
...
- **Canonical Int inputs.** `SRepr` requires every Int, at any depth of an argument, to be a canonical carrier. The runtime builds only canonical carriers: the i64 fast paths build a `Small` directly, and every path that can produce a limb-carrying result ends in the normalization epilogue. A host that fabricates its own carrier words is outside the claim.
...
- **Configured cache directories**, as trusted local state. Their integrity manifests detect accidental corruption, not a writer who replaces the `.olean` files and the Lake traces together.
...
| 2 Package layout | `aver-cert/src/engine/render_package.rs` (`write_project`, `render_plans`, `render_manifest_lean`, `render_artifact`, `render_artifact_plans`, `render_final`), `aver-cert/src/engine/source_bridges.rs` (`render_bridge_lean`, `MODEL_PACKAGE_DIR`), `aver-cert/src/verifier.rs` (`assemble_build`, `is_checker_owned`, `checker_witness`) |
