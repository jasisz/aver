...
- `AVER_YIELD_DUMP=1` (env var, any door that type-checks a program) prints the
  generated Aver for every lowered `yield` function to stderr — a debugging
  aid for the state types, request/outcome sums and answer functions it built
...
`--write` rewrites exactly the lists the report names, on functions and on module boundaries, to that minimum, rendered through the formatter's own layout so `aver format --check` stays clean afterwards. It is idempotent, and a tree whose lists the report calls minimal is left byte-identical: the report is what you read to decide whether to run the rewrite at all, so the rewrite never touches a list the report said nothing about. Four rules keep it a mechanical step:
...
- A namespace entry you wrote, such as `Disk`, stays as written while the computed set is covered by it, including when you wrote a method beside it. Narrowing it to `Disk.readText` is a decision about the contract; `effect-granularity` raises that as a warning instead.
...
A second, per-turn budget is off by default. `[verify] turn-budget = N` makes each case count VM steps since its last wait — `Tcp.poll` or `Wait.poll` — (or since the case began), and a turn that runs past N is reported once per case as `warning[turn-budget]: <fn> case <k> <case>: one turn ran N steps without waiting; deepest function on the stack at the limit: <fn>`, where `<k>` is the case's 1-based position in the block, so two cases with the same expression stay distinguishable. It is a warning, never a failure: the case was answered, it just did not wait often enough while doing so. Only VM steps count — time spent inside a provider, live or stubbed, is not measured — and with the budget off nothing is counted at all. The count is checked on the same 256-opcode cadence as the step limit, so a turn shorter than that cadence cannot be measured and a reported length is the length at the check that caught it. The wasm-gc verify lane (`--wasm-gc`) does not measure turns: it runs on fuel, not opcodes, and has no `Tcp.poll` seam to reset on. `aver check` reports the static shape behind most of these as `warning[serve-path]`.
...
the proof ratchet; `--compare-manifest <earlier proof_manifest.json>` (Lean,
check mode) says, for each law that did not close, whether its own script
changed and which definitions it opens changed since that manifest;
`--minimize` (Lean, implies `--check`) collapses each auto-proof to the single
tactic that actually closed it. See [docs/lean.md](lean.md) for the proof
workflow, the `--check` summary fields, `--compare-manifest` and `--minimize`.
