...
pub(super) fn candidate(
    ...
    for citation in law.using.as_ref()? {
        ...
        for id in &theorem.function_cone {
            ...
            {
                finite.insert(induction::lean_name(fd, ctx));
            }
        }
    }
    ...
    Some(format!(
        "(simp only [Bool.and_eq_true, decide_eq_true_eq, beq_iff_eq, {finite}] at *; all_goals (repeat' first | (simp_all +zetaDelta [{finite}, {}, {excluded}]) | split); all_goals ({lemmas}grind [List.drop_cons, List.drop_drop, List.length_drop, List.append_assoc]); done)",
