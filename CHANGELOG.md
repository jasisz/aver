...
### Fixed
- **Vector aliasing — clone-on-write across VM + wasm-gc.** `Vector.new(n, inner)` previously shared one heap entry across `n` cells, so `Vector.set` on a row from `Vector.get(outer, i)` rewrote every alias. VM disables the `last_use`-driven owned-mutate path for `Vector.set` / `Map.set` and deep-clones compound defaults in `Vector.new`. Wasm-gc reserves a per-`Vector<T>` scratch local in the slot table and emits `array.new_default` + `array.copy` + `array.set` on the copy in both `Vector.set` paths (boxed and fused-with-`Result.withDefault`). Plus discovery walker now registers `Vector<T>` instantiations from `Vector.new` call sites, fixing nested-vector compile errors. Cures repros `01_vector_aliasing` and `02_arena_panic`. Legacy `--target wasm` and `--self-host` already routed through clone paths in this shape; no change needed there.
