...
- **The JSON example now proves end-to-end on Lean — escaped-string parse/serialize roundtrips close universally.** The two parser-workhorse laws (`escapeJsonString.parseStringRoundtrip`, `parseStringChunk.escapedStringRoundtrip`) — for every string, escaping and re-parsing returns the original string at the exact final position — used to be the last caught `sorry`s in `examples/data/json.av`; they now close kernel-genuine (axioms = `[propext, Quot.sound]`, no `native_decide`, no Mathlib), taking the example's Lean budget to zero. The prover recognizes the whole escape producer/consumer pair structurally — a per-char classifier fold with a two-char escape table and hex control escapes, a segment-chunking scanner with an escape dispatcher whose table must align arm-by-arm — and synthesizes the suffix-invariant proof from a machine-checked template, with the scanner's carried state (segment start, collected chunks) universally quantified. Any escaper/parser pair with that shape qualifies, whatever its names, delimiters or escape table; misaligned pairs decline honestly, and a template regression degrades to caught `sorry`s, never a build error.
...
- **Algebra laws over Int-component records now prove universally on Lean — exact-rational arithmetic ships as a worked example.** A library of fractions as plain data (`Int` numerator/denominator record, no normalization, equality by cross-multiplication) carries its full ring algebra as `verify ... law` blocks — commutativity, associativity, distributivity, identity elements, negation/subtraction laws — and every one of them now closes kernel-genuine on the Lean backend (previously most degraded to caught `sorry`s; Dafny/Z3 already proved them). The prover recognizes the shape structurally — any record with all-`Int` fields, pure `+`/`-`/`*` arithmetic, and an integer-equality comparator qualifies — and normalizes both sides with a fixed package of core ring lemmas, no Mathlib. See `examples/data/rational.av`.
- **Floor-division laws — binary exponent search, digit extraction, power-of-two windows — now prove universally on both backends.** A recursive function that shrinks an `Int` argument by `Result.withDefault(Int.div(p, k), d)` with a literal divisor (halving in a binary exponent search, base-10⁹ digit peeling) used to fall outside the proof subset entirely: Lean emitted a kernel-opaque `partial def` (nothing about it was provable), and Dafny either declined or synthesized a `requires p >= 0` that broke every caller. The compiler now validates the shrink — literal divisor ≥ 2, and the function's own guards must prove the argument stays positive at every recursive call (never guessed; an unvalidated guard declines honestly) — and emits genuine well-founded definitions on both backends. On top of that class, `when`-laws about power-of-two windows close as true universal proofs: the power function's positivity and sum law, the scaled-significand window `2^(n-1) <= sig(a,b,n) < 2^n` of an integer ratio, and the m-bit×n-bit product window. Lean proves them kernel-genuine from the core floor-division bridges plus functional induction (no Mathlib, no `native_decide`); Dafny proves them from an emitted-and-proved division-window lemma prelude with branch-split helpers. See `tests/fixtures/floor_window.av`.
...
- **`aver proof --check-json` reports explicit law counts: `universal_laws` and `bounded_laws`.** Consumers could already pin sorries/errors/axioms as exact counts, but "how many law theorems does this file prove universally" required knowing which degradation path moves which counter. The Lean summary now carries `universal_laws` (law theorems classed universal whose kernel certificate passed the axiom whitelist) and `bounded_laws` (law theorems stated only over the finite sample grid). Both come from the same statement-class markers and `#print axioms` audit the `universal` flag already keys on; the flag's meaning is unchanged.

### Changed
- **`aver check` catches verify-case type mismatches on compound expected sides earlier.** A case whose expected side is a list/map/vector that cannot match the function's return type (e.g. `=> ["a"]` against a `List<Int>` fn) is now a check-time type error instead of a guaranteed runtime verify mismatch.
...
- **Conditional `when` laws export honestly to both proof backends, and proof checks no longer stall on concrete samples.** Dafny law samples respect the `when` premise (premise-violating combinations from the given grid are no longer asserted as false facts), sample assertions reuse the already-proven universal lemma instead of pushing Z3 into minutes-long symbolic unfolding, Lean sample theorems pin their premise numerals to `Int` (no more false-as-stated theorems from truncated `Nat` subtraction), and nonlinear-arithmetic laws (`x * x`, error-squaring polynomials) degrade to honest caught `sorry`s instead of failing the Lean build with broken tactics.
...
- **wasm-gc: Option-returning functions now verify on the wasm-gc backend, including nested and compound payloads.** A `verify ... => Option.None` case (and a `f(x) == Option.None` comparison written in a fn body) failed wasm-gc compilation with "Option constructor: instantiation `Option<T>` was not registered" while `aver check` and VM verify passed — a bare `Option.None` has no payload to fix its `T`, and the type checker stamped it generically instead of adopting the other side of the comparison. Equality against `Option.None` now infers the concrete side first and propagates its type. The same applied to four more shapes that type-check and pass VM verify: nested Options (`Option.Some(Option.None)` under `Option<Option<Int>>` — the constructor emitter also mistook an Option-typed payload for the full instantiation), tuple payloads (`Option<Tuple<Int, Int>>` was registered but looked up under a differently-spaced name), list literals with an `Option.None` element (`[Option.None, Option.Some(n)]` on the expected side of a verify case), and `Option.None` passed as a `Map.set` value argument. Verify cases now propagate the called function's type into the whole expected expression, and the backend resolves Option instantiations regardless of payload nesting or spacing. Still out: a bare `Option.None` inside a non-empty map literal or inside a tuple literal (rejected at `aver check` on every backend alike), and — wasm-gc only, pre-existing — `Vector<Option<T>>` values and `Option` values inside tuple-typed results (VM verify runs them; wasm-gc reports a backend compile error), plus two runtime divergences the wider wasm-gc verify coverage now reaches: `String.len` counts UTF-8 bytes on wasm-gc but Unicode scalars on the VM, so cases over multi-byte characters disagree between the backends (`examples/data/json.av`), and `Result` equality traps at runtime when two record types in one module each carry their own `Result<Record, String>` (`examples/core/order_total.av`).
- **Large given-domain grids now survive every Lean export mode.** The bounded-chunk split of the domain cross-check (see the `tests/fixtures/large_domain_law.av` entry below) applied only to the default export; `--verify-mode sorry` and `--verify-mode theorem-skeleton` still emitted one giant conjunction whose statement alone exceeds the elaborator's recursion depth, so the stub export of a large grid failed to build. All three modes now emit the same bounded parts.
- **wasm-gc: `Bool == Bool` now compiles — `verify` cases against `true`/`false` run on the wasm-gc backend.** Comparing two Bools (written in a fn body, or synthesized by every `verify ... => true` case) emitted a 64-bit equality on 32-bit Bool values, so the whole module failed wasm validation ("type mismatch: expected i64, found i32") even though `aver check` and VM verify passed. Affected any file whose verify block compared a Bool-returning call against a Bool literal.
- **`aver verify --wasm-gc` reports backend compile failures honestly** — a file the wasm-gc backend cannot compile is listed as a wasm-gc backend error, instead of being mislabeled "type errors (run aver check for details)" on a file that type-checks cleanly.
...
- **Proof exports stay honest on shapes the prover cannot handle, instead of producing broken files.** Lean: a law whose helper chain uses a `Result`/`Option` builtin (e.g. `Result.withDefault`) degrades to an honest caught `sorry` — its proof used to cite a Lean name that doesn't exist and the whole export failed to build (`tests/fixtures/result_default_cone.av`); laws over large given-domain grids (hundreds of combinations) no longer kill the export with an elaborator recursion-depth error — the domain cross-check is split into bounded chunks (`tests/fixtures/large_domain_law.av`). Dafny: recursion outside the recognized termination patterns (e.g. doubling/halving on a rational pair) is exported as an opaque declaration and its laws reported as omitted, instead of guessing a `decreases` measure that fails verification on a correct function and synthesizing a `requires` that breaks every caller (`tests/fixtures/expo_outside_subset.av`); the countdown `decreases` pick is now validated against the function's actual self-calls, which also fixes red-black tree and RNG exports that previously carried guessed-measure errors.
