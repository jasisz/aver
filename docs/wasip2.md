...
| `Tcp.poll` / `Tcp.readSome` | `poll` accepts the common `Map<Int, Tcp.Socket>` API but admits only `Tcp.Socket.Connected` values on wasip2 — the other two variants carry a `Tcp.Dial` or a `Tcp.Listener`, which only the compile-rejected operations below can mint, so on this target a program has no way to build one. It subscribes every candidate input stream, adds one duration pollable for timeout, maps ready dense indices back to caller-owned keys, sorts them with Aver's arbitrary-precision `Int` order, and drops every pollable. A `Listening` or `Dialing` value returns an explicit `Result.Err`, never a trap. `readSome` performs one bounded `input-stream.blocking-read`, returning empty `Bytes` only for clean EOF. Actual read errors poison the pool slot; argument validation and polling do not. |
