...
> Status: v0, certification level **L1** (conditional on named runtime contracts). Nine certified function classes (four integer, four ADT, and cross-function composition); everything else is **declined fail-closed** and listed with a reason. This document is the contract; the emitted `cert-manifest.json` is its machine-readable form.
