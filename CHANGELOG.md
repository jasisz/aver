...
- **`aver effects` reports declared effect lists against the minimum the checker computes, and rewrites them on request.** The report names, per module, every function whose `! [...]` is missing or carries an unused entry, and does the same for the module's `effects [...]` boundary; modules with nothing to say get one summary line. `--write` rewrites every list to the computed minimum across the whole module graph, so changing which primitive a leaf calls no longer means editing every function above it by hand; the rewrite goes through the formatter's layout, is idempotent, and leaves an already-minimal tree byte-identical. `yield` and the callback-forwarding marker `_` are copied verbatim, a namespace entry such as `Disk` keeps the granularity its author chose, and a module without a boundary does not get one. `--since <rev>` compares the effect surface at a git revision with the working tree and separates the functions whose list moved with an unchanged body from the ones whose own body changed. `--json` emits the versioned deterministic envelope. Granularity stays `Namespace.method`; nothing here lets a list be omitted, named or abbreviated. See `docs/cli.md`.
