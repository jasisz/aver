...
- **`aver compile FILE --explain-mir-coverage`** reports how much of a program lowers to MIR (per function, with the dominant blocking shape for any that don't) — the meter used to drive the VM onto MIR. Add **`--target wasm-gc`** to switch the meter to the wasm-gc backend's reach: how many lowered functions the wasm-gc emitter renders straight from MIR versus the resolved-HIR fallback — the progress tracker for porting that backend onto the shared middle-end.

### Changed

- **Function values are now restricted to call-argument position — a `Fn(...)` type is allowed only as a function parameter.** Aver has no closures (functions are top-level, capture nothing), and a function value can already only be a named function, builtin, or constructor passed directly as an argument (`HttpServer.listenWith(port, ctx, handler)`). That rule is now enforced uniformly: using `Fn(...)` as a **return type**, a **record or sum-variant field**, a **collection / map-value / tuple element**, or nested inside another `Fn`, and binding a function value to a local (`g = double`, or wrapped as `gs = [double, inc]`), are type errors. Previously some of these (a `-> Fn(...)` return, a `Fn` record field, a `List<Fn(...)>`) type-checked but then failed to compile on every backend; the error now lands at `aver check` with a clear message. This keeps the concrete callee at every call — and therefore the effects it may perform — statically knowable, which is what the effect system, the Oracle, and `aver verify` depend on. To choose between functions dynamically, branch at the call site or model the choice as a sum type.

### Fixed

- **`--target wasm-gc`: a tuple-destructure binding that follows `_` is no longer dropped.** `match pair { (_, value) -> value }` compiled to wasm-gc never extracted `value`, returning the field's zero default instead of the bound element (e.g. `second((7, 42))` returned `0`). Tuple fields are now paired with their binding by position, so every binding is extracted regardless of where the wildcards sit. The VM and other backends were already correct.

### Compiler internals

- **`match` on `Int.div` / `Int.mod`'s `Result` lowers on every backend.** The directly-consumed (boxed) form — `match Int.div(a, b) { Result.Ok(q) -> … ; Result.Err(_) -> … }`, not just the fused `Result.withDefault(Int.div(a, b), default)` — now materialises the concrete `Result<Int, String>` on wasm-gc (mirroring `Int.fromString`) and exports faithfully to the Lean/Dafny proof backends (guarding the zero divisor so the `Err` arm is reachable, rather than modelling the result as an unconditional `Ok`). The natural Ok/Err idiom is no longer VM-only.
- **The wasm-gc and wasip2 backends now run the shared Core-MIR optimizer.** Until now only the VM applied the six MIR optimize passes (inline → const-fold → algebraic-simplify → bool-match-to-if → branch-collapse → dead-code); wasm-gc and wasip2 emitted from the raw, unoptimized lowering. The pass nest is hoisted into a single `ir::mir::optimize()` entry and wired into the shared wasm-gc/wasip2 seam, so one optimizer pass now improves every executable backend at once — the foundation for the perf work. The wasm-gc body emitter learned `MirExpr::IfThenElse` (emitted by `bool-match-to-if`); any optimized shape it doesn't yet cover degrades to the resolved-HIR emission, never to wrong code. The two wasm-gc differential tests are reframed accordingly: the resolved-HIR build is now the unoptimized *baseline* (a perf A/B reference), and they assert both paths compile + MIR coverage holds rather than byte-identity (optimization diverges by design); correctness of the optimized output is covered behaviourally by `wasm_gc_spec` / `wasm_gc_capture_output`, size/speed by `aver bench`.
- **The wasm-gc backend now emits function bodies from Core MIR for most of the shapes it covers, falling back to the resolved-HIR emitter for the rest.** The wasm-gc → MIR port follows the same incremental, fallback-driven path the VM took: literals, local reads, numeric and `String` binary ops, negation, `let` bindings (named and statement-sequencing), direct and tail calls, registered builtins, structured `match` (primitive, `Option` / `Result` / `List`, user sum-type, single-arm tuple-destructure, and multi-arm tuple-of-`Result`-constructors subjects), structural `==` / `!=` on sum / record / carrier types (the per-type `__eq` helper), constructors, record create / update / project, tuple / map / list literals, `?` propagation, string interpolation, the native scalar `Float` / `Int` / `Bool` builtins, the `List` operations (`prepend` / `reverse` / `concat` / `take` / `drop` / `contains` / `zip` / `fromVector` / …), the `Vector` operations (`len` / `new` / `get` / `set` / `fromList`), the `Map` operations (`set` / `get` / `has` / `len` / `keys` / `values` / `remove` / `entries`) including the fused no-allocation `match Map.get(m, k) { Some(v) -> … }`, `Char.toCode`, the custom-inline `String` ops (`length` / `byteLength` / `split` / `join`), the `Args.get()` inline expansion, independent products (`(a, b, c)!` / `?!`), `Result.withDefault` (both the fused `Result.withDefault(Int.mod(a, b), …)` and the boxed tag-dispatch form) and `Option.withDefault` (the fused `Vector.get(v, i)` / `Vector.set(v, i, x)` leaves and the boxed `Option<T>` tag-dispatch form), and effect calls (`Console.*` / `Disk.*` / `Tcp.*` / `Http.*` / `Random.*` / `Time.*`) are all walked straight off `MirExpr`; anything still uncovered resets the function and re-runs the existing emitter, so the whole corpus and game suite stay byte-for-byte unchanged. Two byte-differential tests — one over every single-file example, one over the multi-module games — compile each program both ways and assert the modules are identical, so any covered function whose MIR emission drifts by a single byte fails before merge.
- **wasm-gc modules are now byte-reproducible across builds.** Carrier type slots (`Option` / `Result` / `List` / `Vector` / `Map` / `Tuple` discovered from record fields) were registered in `HashMap`-iteration order, which the standard library randomizes per process — two builds of the same program could emit the module with its type section in a different order. Discovery now walks record names in a stable sorted order, so the emitted bytes are deterministic.
