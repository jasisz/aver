...
- **Artifact certificates now cover comparisons between computed integers.** Programs that decide an order or an equality — `a.top * a.bottom >= 0`, `a.top * b.bottom < b.top * a.bottom`, `a.top * b.bottom == b.top * a.bottom` — now get a certificate for the exact bytes the compiler already emits, instead of being reported as source-level only. The two comparison helpers are stated over carriers in the runtime's normal form, which is every carrier the runtime builds, and the three arithmetic helpers now also state that their result is in that form. `projects/k5_fdiv` went from six certified exports to ten.
