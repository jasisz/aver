...
- **The wasm-gc backend now emits function bodies from Core MIR for the shapes it covers, falling back to the resolved-HIR emitter for the rest.** First wave of the wasm-gc → MIR port (the same incremental, fallback-driven path the VM took): literals, local reads, numeric binary ops, negation, and named `let` bindings are walked straight off `MirExpr`; everything else resets the function and re-runs the existing emitter, so the whole corpus and game suite stay byte-for-byte unchanged. A byte-differential test compiles every single-file example both ways and asserts the modules are identical, so any covered function whose MIR emission drifts by a single byte fails before merge.
