...
| `Tcp.poll` / `Tcp.readSome` | `poll` accepts the common `Map<Int, Tcp.Socket>` API but currently admits only `Tcp.Socket.Connected` values on wasip2. It subscribes every candidate input stream, adds one duration pollable for timeout, maps ready dense indices back to caller-owned keys, sorts them with Aver's arbitrary-precision `Int` order, and drops every pollable. A `Listening` or `Dialing` value returns an explicit `Result.Err`, never a trap. `readSome` performs one bounded `input-stream.blocking-read`, returning empty `Bytes` only for clean EOF. Actual read errors poison the pool slot; argument validation and polling do not. |
| `Tcp.{beginConnect, dialled, listen, accept, peerAddress, closeDial, closeListener}` | **Level B:** these operations compile with their exact resource/result types but return an explicit `Result.Err` saying native dial/listener resources are unavailable on this target. Blocking `Tcp.connect` and connected-socket polling remain native. This keeps portable source catchable without pretending that the WASI backend implements resources it does not yet own. |
...
### Why `Process.stopRequested` / `Terminal.*` / `Env.set` are rejected, not stubbed
