...
function makeHostImports(memory) {{
    ...
    <p>
        ...
        without a JS host — let the CLI do the bundling. <code>--bridge
        wasi</code> merges the runtime and <code>aver_to_wasi.wasm</code>
        into a single self-contained binary whose only import is
...
<span class="rt-comment"># Runs. No other host wiring needed.</span></pre>
    <p>
        The artifacts on this page (<code>aver_runtime.wasm</code> +
        <code>aver_to_wasi.wasm</code>) are what that command bundles
        internally — published here so that consumers who want to
        deploy a thin user.wasm against a separately-hosted runtime
        (CDN-cached, version-pinned) can wire the modules up at
        instantiate time instead.
    </p>
    <p>
        WASI bridge coverage today: <code>Console.*</code> and
        ...
        provide them through the host directly until coverage lands.
    </p>
