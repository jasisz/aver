...
### Added
- **Persistent HAMT for `Map`, polymorphic from day one.** The linked-list association map is replaced by a hash array mapped trie (branching factor 16, max depth 8) with structural sharing. `Map.get` / `Map.has` are O(log N) instead of O(N) cons-walks, and `Map.len` is now an O(1) header read instead of an emitted loop. Polymorphic over `Int` / `Float` / `Bool` / `String` / `Heap` via a runtime kind-tag dispatch — the emitter normalizes the key to `i64 + kind`, the runtime hashes/compares via the matching path. Building 12k entries under wasmtime now takes <5 ms.
- **User-defined types as `Map` keys.** `Map<MyVariant, V>`, `Map<(Int, Str), V>`, `Map<List<Int>, V>` — anything except functions and `Unit` is hashable. The runtime walks heap values structurally through `rt_deep_hash` / `rt_deep_eq`, dispatching on the `OBJ_KIND` header byte and reading the per-field pointer mask from the meta byte. Type checker, parser, and Map literal inference all relax to the same rule: only `Fn(_)` and `Unit` are rejected.
- **Cross-backend uniformity for hashable keys.** VM, Rust codegen, and WASM all accept the same set of types as `Map` keys and produce identical results. The VM's `NanValue::map_key_hash` was previously hashing heap values by arena pointer (silent lookup miss for equal-but-different-arena values); the new `map_key_hash_deep` uses the structural `hash_in` walker so two equal `Color.RGB(255, 0, 0)` instances always land in the same bucket.
- **`--target edge-wasm`.** Emits a thin `user.wasm` whose `aver_runtime` import is left unresolved — pair it with a separately-hosted runtime module to ship just your program logic. With `--optimize size`, a trivial program drops to ~280 B; the seven playground games sit between 5 and 28 KiB of pure program logic.
- **`--bridge wasi`.** Bundles the `aver_to_wasi.wasm` translation shim alongside `--target wasm`, producing a self-contained binary whose only import is `wasi_snapshot_preview1.fd_write`. Run anywhere with a WASI host (wasmtime, wasmer) without a JS host shim.
- **`--optimize size|speed` pipeline.** Replaces the single `wasm-opt -Oz` invocation with a multi-stage flow: `wasm-metadce` first (with a graph naming `_start` / `main` as the only outside-reachable entries, so dead exports collapse), then `wasm-opt -Oz --converge --strip-producers --strip-target-features`. Snake bundled drops 18.5 KiB → 6.5 KiB. The legacy `--wasm-opt` spelling stays as a clap alias.
- **Runtime CDN artifacts at `/runtime/`.** Every release publishes `aver_runtime.wasm` (~10 KiB optimized) + `aver_to_wasi.wasm` (~684 B) + `.wat` companions + `CHECKSUMS.txt` + per-version README under `tools/website/runtime/v{X}/`, mirrored at `latest/`, with a `manifest.json` and a static listing page documenting browser host shims, WASI hosting, and edge packaging. Stable URLs: `/runtime/latest/aver_runtime.wasm` floating, `/runtime/v{X}/...` immutable per-version pin.
- **Playground games default to edge packaging.** Games are now thin `user.wasm` modules that share a single cached runtime — second game you click downloads only the program logic. Worker auto-detects bundled vs edge shape via import inspection, so dragging in a self-contained `.wasm` from `aver compile --target wasm` keeps working.
- **Emit-time WASM validation.** `aver compile --target wasm` runs `wasmparser::Validator::validate_all` on the produced bytes before declaring success, so emit-time bugs (Map<Int,V>'s `expected i32, found i64` from the linked-list era was the original motivator) become compile errors with a real diagnostic instead of runtime "module error" messages.
- **WAT-source-of-truth runtime.** All 64 runtime functions (alloc, GC compactor, obj inspection, str/list/map/vec ops, formatting) now live in `src/codegen/wasm/runtime/wat/*.part.wat` instead of hand-written `wasm-encoder` instruction streams. Source is readable, editable, and the binary is parsed once via `wat::parse_str` and cached. Disassembly through `wasm-tools print` shows real names (`$rt_alloc`, `$rt_map_set`, `$rt_deep_hash`, `$buildMap`, `$x`) instead of `call 18`; `wasm-opt -Oz` strips the name section automatically for production.
- **Imported-runtime architecture.** `user.wasm` declares `(import "aver_runtime" "rt_alloc" ...)`, `(import "aver_runtime" "memory" ...)`, etc.; the runtime is a separate WASM module instantiated first by the host and wired into every program at instantiate-time. `--target wasm` post-merges the runtime into a single artifact (existing soft-dep on `wasm-merge`); `--target edge-wasm` ships them as separate files.
- **`aver→wasi` as a deployment bridge, not a compile-time backend.** `user.wasm` always emits Aver-flavored host imports (`aver/console_print`, `aver/print_value`, …) regardless of how it's later deployed; the bridge is a separate WAT module that translates them to `wasi_snapshot_preview1` calls when needed. Adding new bridges (Cloudflare, Fastly, …) is now "write a shim" rather than "fork the compiler".

### Changed
- **`--wasm-opt oz|o3` is now `--optimize size|speed`.** The flag drives a multi-tool pipeline (`wasm-metadce` + `wasm-opt --converge --strip-*`), not a single binary, so the old name was misleading. The legacy spelling stays as a clap alias and existing scripts keep working.
- **`Map` runtime ABI is polymorphic.** `rt_map_get(map: i32, key: i64, kind: i32) -> i32`, `rt_map_set(map, key: i64, kind: i32, value: i64, value_ptr_flag: i32) -> i32`, `rt_map_has(...)` likewise. The emitter extends scalar keys to i64 (sign-extended for `Int` / `Bool`, reinterpret for `Float`, zero-extended for pointer kinds) and pushes the kind tag at every call site.
- **GC compactor learns four new `OBJ_KIND` values** (12 `OBJ_HAMT`, 13 `OBJ_HAMT_NODE`, 14 `OBJ_HAMT_LEAF`, 15 `OBJ_HAMT_COLLISION`). The wrapper and leaf join the existing record-style branch (meta byte = pointer mask); node and collision get an all-fields-pointer branch. All four use the standard `8 + low32 * 8` size formula via standard header layout.

### Removed
- **`--target wat` and `--target wasm+wat`.** The name section in the produced `.wasm` plus standard `wasm-tools print` cover the same use case via standard tooling — keeping a parallel target inflated the build matrix without earning its keep.
- **`--adapter` flag alias.** `--bridge` has always been the canonical name in user docs; the alias was leftover pre-1.0 parity-for-its-own-sake. There is one CLI spelling now.
- **`OBJ_MAP_ENTRY` (kind 11)** is no longer emitted — HAMT replaced the linked-list cons cells. The constant stays in `value.rs` as a reserved slot (the GC still recognizes the layout in case any pre-0.14 in-memory module instance needs to be unmapped during instantiation).

### Fixed
- **`Map<Int, V>` and `Map<Float, V>` no longer crash at validation.** The pre-HAMT runtime's `emit_map_set` ABI took `key: i32` and used `str_eq` for comparison, so non-`String` keys produced invalid bytecode (`expected i32, found i64`). HAMT's polymorphic dispatch handles every kind correctly.
- **`Map<Bool, V>` no longer silently miscompiles.** Pre-HAMT, both `true` and `false` keys collapsed to the same string-pointer interpretation under `str_eq`, so `Map.set(m, true, 100); Map.set(m, false, 200); Map.get(m, true)` returned `200`. Now they're distinct.
- **VM `Map` lookup respects equality.** `NanValue::map_key_hash` previously hashed heap values by their arena pointer, so two equal `(1, 2)` tuples allocated in different slots hashed to different buckets — `Map.get` would silently miss. The new structural hash matches `eq_in`.
...
- **Oracle invariant documentation in proof export.** The trust header in every `aver proof --backend lean` / `--backend dafny` artifact now documents the per-effect bounds the runtime guarantees: `Random.int` stays in `[min, max]`, `Random.float` in `[0.0, 1.0]`, `Time.unixMs ≥ 0`, `Disk.exists` is total. User-side theorems carry these as hypotheses for now; subtype-encoded oracle types that surface them as free side-conditions land in 0.14.
