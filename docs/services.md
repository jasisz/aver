...
The table answers "already taken" and "job cancelled" out of a bounded number of dead slots — at most 4096, oldest forgotten first. A program that starts more jobs than that over its life gets `Err("work: unknown job")` for a handle whose slot has been forgotten, rather than a table that grows for as long as the program runs.
