...
- **`--bridge wasi`** — bundles the `aver_to_wasi.wasm` shim with `--target wasm`. The result imports only `wasi_snapshot_preview1.fd_write`, runs under wasmtime / wasmer with no JS host.
...
- **`HttpResponse.headers` returned by `Http.*` is empty under `--bridge fetch`.** Same caveat as `req.headers`: response status and body flow through, response headers from the upstream don't yet make it back into the guest `Map<String, List<String>>`. Lands in 0.15 alongside the request-headers bulk transfer.
- **`Http.*` under `--bridge wasi` returns a transport error.** WASI preview 1 has no native HTTP client and `wasi-http` preview 2 isn't in mainstream wasmtime yet, so the bridge stub returns `Result.Err("Http.* not available under --bridge wasi")`. Programs that need real HTTP under standalone wasmtime should run via a JS host (Workers, Deno, Bun) that satisfies `aver/http_send` itself.
