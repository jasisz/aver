...
### Added
- **`aver run --expr '<call>'`** — record/replay now targets any function, not just `main`. Pass a function call (e.g. `aver run src/tax.av -e 'loadTaxRate("PL")' --record dir/`) and the recording's `entry_fn` and `input` are populated from the call; `aver replay` picks them up unchanged via the existing entry-args path. Repeat `-e` to batch several calls in one invocation; each produces its own recording file. `--input-file PATH` (use `-` for stdin) reads a single call expression from a file for long inputs. Conflicts with `--verify`, `--self-host`, and `--wasm`. Argument shapes supported: literals (`String` / `Int` / `Float` / `Bool` / `Unit`), list / tuple literals, and ADT constructors — built-in (`Result.Ok(5)` / `Result.Err("msg")` / `Option.Some(x)` / `Option.None`) and user-defined (`Shape.Circle(1.0)`, `Shape.Rectangle(3.0, 4.0)`), including nested. Function calls, arithmetic, variables, and record constructions in arg position stay out of scope for now; wrap them in a helper function and call that instead.
