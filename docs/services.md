...
`task`, `started` and `landed` are the three ends of the seam between a job kind and the answer state the turn already holds. `T` is the `begin` task type and `R` the `take` payload type. `task = "Module.function"` is `(S) -> Option<T>`, where the turn takes its next task from. `started = "Module.function"` is `(S, T) -> S`. The turn calls it with the offered task as soon as `begin` has answered `Ok(job)` for it. The start is recorded, so the next `task` ask is offered a different task than the one just started. `landed = "Module.function"` is `(S, Result<R, String>) -> S`, where a job that is over goes. A job that finished arrives as `Result.Ok(r)`. A job that never will finish (cancelled, stopped, or forgotten by the engine) arrives as `Result.Err(reason)`, because losing a job does not stop the run. One start happens in this order: ask, `begin`, `started`. Every task `task` offers in a turn is begun in that turn: the loop counts nothing against the job limit, because the engine queues a job begun at it. So `task` must run out once `started` has recorded what it offered, or the turn never ends. A `begin` that answers `Err` records nothing, so the task stays in the answer state and is offered again in a later turn.
...
A program whose loop is generated states one law about the seam itself. The law goes on the `started` function and is named exactly `aStartedTaskIsNotAskedAgain`: `when task(state) == Option.Some(t)`, `task(started(state, t)) != Option.Some(t) holds`. The generated loop cites it by that name on its own `__consumed<K>` law. What it guarantees is exactly what it says: the task just started is not the next task offered, so the same task is never started twice in a row. It does not say that a task started earlier is never offered again later, and it does not say the offers run out. A `started` that moves the task to the back of a queue instead of removing it satisfies the law and still starts the same task again, and it keeps the turn starting jobs for ever. Remove the task in `started`, and state a stronger law of your own when the program depends on more. A `[run]` program whose `started` function states no law by that name is `error[run-binding]`, and the message prints the block to write.
...
`[run]` is the table that asks for the loop itself to be generated. Its answer modules, job kinds, `Wait` and `Work` are loaded from the manifest, so the entry lists only its own source dependencies. When none of the four keys is set, the defaults take seated ids in slot order, admit every askable id, and stop on the flag or when no process is seated and no job runs. `__View` and `__Pending` are generated. The loop belongs to the entry module: a yielding function in a dependency is a library helper that a process enters, and the loop never seats it, whichever module a command checks or runs.

To customize a policy, name all four keys. All four name things in one module: the entry module, which is where the loop is generated and what it can see. `order`, `admit` and `stop` are three pure policies: `order(view: View) -> List<Int>`, `admit(view: View, id: Int) -> Bool` and `stop(view: View) -> Bool`. `view` names the record they read, which the program declares and the loop fills: `pending`, `ready`, `askable`, `jobs` and `stopping`. With custom policies the run also ends as soon as no process is seated, whatever `stop` says, and the jobs still running then are cancelled without reaching `landed`. `ready` is what the turn's one wait reported. `askable` is the ids this turn may ask, in slot order, which means every seated slot whose wake has fired. `order` and `admit` see the whole view, but the turn asks `admit` only about askable ids and never serves a slot that is not askable, whatever `order` returned.

Some mistakes are refused when `aver.toml` is read: a `[run]` table with only some of the four keys, a key that is not `Module.function` / `Module.Type`, or four names that do not share a module. Four names in a module other than the one the entry is loaded as (`Slice.Node` for an entry the module root loads as `Node`) generate no loop, so an entry without `main` would run nothing; every door refuses that with `error[run-binding]`. Problems in what the loop is generated from (a process with parameters, a process written in another module of the program, an answer module with no `fresh`, a policy with effects) are `error[run-binding]`. A view record or marker sum that does not have the shape the loop fills is `error[view-shape]`, which prints the declaration it wants.

A program whose loop is generated may declare any number of job kinds, each with its own `task`/`started`/`landed` seam. The generated run keeps one job table, a `Map<Int, __Job>` whose `__Job` sum has one variant per kind. The kinds share one engine and one `[work] max-jobs` limit, so `max-jobs = 3` bounds all running bodies together, not three per kind. The turn first takes every kind's ready jobs and then begins every task each kind offers, kind by kind in manifest order. The engine starts queued jobs in the order they were begun, so a kind listed first that always has a task keeps the kinds after it waiting behind its jobs. The job limit is not in the generated source at all, so the program means the same on every machine that builds it.

A job can also be the answer to a request, with no seam. An answer module begins the job itself, keeps its handle in its state, and parks the request on it with `Later(Wait.Wake.Item(Wait.Item.Job(job)))`. The turn watches that job in its one wait and asks the request again when the job settles, and the module then takes it and answers `Now`. That needs no `task`, `started` or `landed`, and no law. The answer module performs only `begin` and `take`, which return at once, so `answer-shape` does not warn about it. When the run ends, the generated loop cancels every job a parked request is still waiting on, as it cancels the jobs in its own table, so no job outlives its run. `tests/fixtures/run_job_request` is a worked example.
