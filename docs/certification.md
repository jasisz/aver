...
4. authors a **kernel witness** binding the computed artifact hash, obligation count and export names. For every non-expression family, audited `CertDecode.lean` computes the `WCode` entries and carrier index directly from checker-regenerated `ArtifactBytes`; construct and projection claims also compute their struct field counts there. These kernel results are equated to the manifest-bound obligations. Expression fragments retain their existing checked-plan/canonical-code-entry equality. Each obligation's **self index** (which module function the fuel semantics reasons about) is pinned into the byte-decoded export section by the whole-module `exportsAccounted` conjunct — the checker no longer splices a Rust `self` list; only host-role classification remains a later standalone sub-leg. The witness also pins the obligation's **typed face** to the standard form of its byte-derived class: a `Nonempty Dom` proof for every obligation (so a `Dom := Empty` vacuity is rejected), and per class the codomain representation (`intRepr`/`verbatimRepr`), the integer domain representation with the byte-bound arity, and the projection domain representation — each by `HEq.rfl`. A manifest that weakens `Dom`/`Cod`/`domRepr`/`codRepr` fails one of these checks;
...
As of schema 50, the `cert_goals` matrix has **23 certified exports and 0
