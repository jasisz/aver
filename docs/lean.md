...
- **fallback** — bounded evidence only: per-sample and `_checked_domain` theorems via `native_decide`, plus the universal theorem with an honest `sorry`. One attempt stands before that `sorry` for an unconditional law whose earlier laws speak about the functions in its call cone: `grind` over those laws, with the recursive functions kept folded — this is how a round trip stated over a countdown fed an expression (`digits(magnitudeOf(value), [])`) closes from the countdown's own laws, which fuel induction proved one law above. `when`-laws get a guarded-domain enumeration instead; their bounded statements carry an explicit statement class and are never credited as universal.
