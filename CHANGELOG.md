...
- **Persistent TCP sessions now have a usable event-loop receive path.** `Tcp.poll : (Map<Int, Tcp.Connection>, Int) -> Result<List<Int>, String>` reports the caller-owned peer IDs that can make progress, sorted and duplicate-free, while `Tcp.readSome : (Tcp.Connection, Int) -> Result<Bytes, String>` consumes the available chunk without waiting to fill an exact frame. Empty readiness means timeout; empty bytes means clean EOF. Both operations live in the standard Tcp capability/provider contract, participate in replay and hostile verification, preserve arbitrary-precision peer IDs, and run through the VM, generated Rust, wasm-gc host bridge, and native WASI 0.2 sockets. Reported by Robin Owens ([@n1bor](https://github.com/n1bor)).
...
- **A type declared in a module nobody imports no longer changes what a name means everywhere else.** A bare type reference — `Step` rather than `Domain.State.Step` — was resolved against every type in the program, so adding a second declaration of that name anywhere, even in a module the file never imports, made the reference ambiguous. Nothing said so: `aver check` passed, `aver verify` passed, and `aver compile --target rust` exited 0 having written `compile_error!` into the generated crate for every function that touched the type; on the first external project one injected record left 76 functions unrendered against a baseline of none, and the failure surfaced only at `cargo build`. The VM refused the same program with an internal error. A bare name is now answered only by the modules the asking one can see — itself, what it names in `depends [...]`, and the standard modules — so an unimported declaration cannot participate. A collision between two modules a file really does import is still an error, and it names both candidates. Separately, `aver compile --target rust` now exits non-zero when the generated code carries a deliberate compile error, instead of reporting success. Found on the first external project, where it cost a day and was mis-diagnosed as a problem with records and recursion. Reported by Robin Owens ([@n1bor](https://github.com/n1bor)).
