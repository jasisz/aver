...
`--target wasm-gc`, `--target wasip2` and `aver run --wasm-gc` / `--wasip2` run a program with a job kind, the one wait of a turn, `Work.cancel` and the generated coordinator. A component and a wasm-gc module are single-threaded, so there is one honest difference and it is the wall clock: a job runs **inline at `begin`**, inside the turn, rather than beside it. `begin` calls the bound function at once and answers a handle that already carries the result, so a `take` asked in the very next expression answers `Ok(Some(r))` where the VM would still answer `Ok(None)`. Everything a program can read is the VM's: a second `take` answers `Err("work: job already taken")`, a take after a cancel answers `Err("work: job cancelled")`, and a handle offered to another kind's `take` answers `Err("work: this job was not started by job kind 'K'")`. The handle is the whole job — its identity, its state, the kind that minted it and the answer — so two copies of one handle are one job, and the module keeps no table beside it.
