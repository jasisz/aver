...
- **Every ordinary standard effect now has one source-owned capability contract.** `Args`, `Console`, `Env`, `Http`, and `Terminal` move their 25 operations beside `Disk`, `Process`, `Random`, `Tcp`, and `Time` under `stdlib/capabilities/`. Their signatures, Oracle/replay semantics, hostile profiles, represented boundary types, contract hashes, and model hashes now come from Aver source. VM and generated Rust invoke exact-contract native providers; wasm-gc and wasip2 bind the same contracts through explicit target rows, including operation-level rejection of `Env.set` on wasip2 and whole-capability rejection of `Terminal`. Compiler-shipped target adapters for one standard capability form an explicit replay-compatibility family: target identities remain visible for audit, fingerprints must match, and traces remain portable between native, wasm-gc, and wasip2; custom providers remain identity-exact. The parallel checker signature table, effect classifier, hostile fallback, VM builtin dispatch, generated-Rust service emitter, and wasip2 effect-name rejector are deleted. Structural tests pin all 40 standard operations to the capability registry and reject any standard capability operation that reappears as a VM builtin. Closes [#1123](https://github.com/jasisz/aver/issues/1123).

- **Incoming HTTP is now ordinary Aver code, with one explicit deployment boundary.** The pure `HttpWire` module frames bounded content-length HTTP/1.1 requests and renders responses over `Bytes`; `HttpServer` owns the native listener, one typed `Tcp.Socket` poll map, sessions, keep-alive/pipelining, per-peer failure isolation, and cooperative `Process.stopRequested` cleanup. Request headers retain repeated values, paths and queries are split, conflicting content lengths, transfer encoding, and `Expect` are rejected explicitly, and malformed streams receive one bounded 400 before close. Native code calls `HttpServer.listen(port, handler)` or `serve(listener, handler)`; Cloudflare/wasm-gc and `wasi:http/proxy` keep the same `Fn(HttpRequest) -> Http.Response` handler and select it only through `--handler`. There is no provider-owned request token and no host-to-VM callback bridge. Addresses [#1124](https://github.com/jasisz/aver/issues/1124).
...
- **The HTTP response type is now owned by the `Http` capability.** Replace every `HttpResponse` annotation or constructor with `Http.Response`; handler signatures are now `Fn(HttpRequest) -> Http.Response`. The fields and wire behavior are unchanged. Qualified record construction also accepts the same multiline layout as an unqualified record, so migration does not force wide one-line handlers.
