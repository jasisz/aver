...
pub fn call(name: AverStr, args: &aver_rt::AverList<Val>) -> Result<Val, AverStr> {
    ...
    { let __dispatch_subject = name.clone(); if &*__dispatch_subject == "List.prepend" { builtinListPrepend(args) } else { if &*__dispatch_subject == "List.len" { builtinListLen(args) } else { if &*__dispatch_subject == "List.take" { builtinListTake(args) } else { if &*__dispatch_subject == "List.drop" { builtinListDrop(args) } else { if &*__dispatch_subject == "List.reverse" { builtinListReverse(args) } else { if &*__dispatch_subject == "List.concat" { builtinListConcat(args) } else { if &*__dispatch_subject == "List.contains" { builtinListContains(args) } else { if &*__dispatch_subject == "List.zip" { builtinListZip(args) } else { if &*__dispatch_subject == "List.head" { builtinListHead(args) } else { if &*__dispatch_subject == "List.tail" { builtinListTail(args) } else { Err(aver_rt::AverStr::from({ let mut __b = { let mut __b = aver_rt::Buffer::with_capacity((38i64) as usize); __b.push_str(&AverStr::from("unknown list builtin: ")); __b }; __b.push_str(&aver_rt::AverStr::from(aver_rt::aver_display(&(name)))); __b })) } } } } } } } } } } }
}
...
pub fn builtinListPrepend(args: &aver_rt::AverList<Val>) -> Result<Val, AverStr> {
    ...
    { let (v, lstV) = pair; builtinListPrependInner(&v, &lstV) }
}
...
pub fn builtinListTake(args: &aver_rt::AverList<Val>) -> Result<Val, AverStr> {
    crate::cancel_checkpoint();
    let pair = crate::aver_generated::domain::builtins::helpers::twoArgs(args)?;
    { let (lstV, nV) = pair; builtinListTakeInner(&lstV, &nV) }
}
...
pub fn listTake(items: &aver_rt::AverList<Val>, count: i64) -> aver_rt::AverList<Val> {
    crate::cancel_checkpoint();
    if (count > 0i64) { aver_list_match!(items.clone(), [] => aver_rt::AverList::empty(), [item, rest] => aver_rt::AverList::prepend(item, &listTake(&rest, (count - 1i64)))) } else { aver_rt::AverList::empty() }
}
...
pub fn builtinListDrop(args: &aver_rt::AverList<Val>) -> Result<Val, AverStr> {
    crate::cancel_checkpoint();
    let pair = crate::aver_generated::domain::builtins::helpers::twoArgs(args)?;
    { let (lstV, nV) = pair; builtinListDropInner(&lstV, &nV) }
}
...
pub fn listDrop(mut items: aver_rt::AverList<Val>, mut count: i64) -> aver_rt::AverList<Val> {
    loop {
        crate::cancel_checkpoint();
        return if (count <= 0i64) { items } else { aver_list_match!(items, [] => aver_rt::AverList::empty(), [_item, rest] => { {
...
pub fn builtinListConcat(args: &aver_rt::AverList<Val>) -> Result<Val, AverStr> {
    crate::cancel_checkpoint();
    let pair = crate::aver_generated::domain::builtins::helpers::twoArgs(args)?;
    { let (aV, bV) = pair; builtinListConcatInner(&aV, &bV) }
}
...
pub fn builtinListContains(args: &aver_rt::AverList<Val>) -> Result<Val, AverStr> {
    crate::cancel_checkpoint();
    let pair = crate::aver_generated::domain::builtins::helpers::twoArgs(args)?;
    { let (lstV, needle) = pair; builtinListContainsInner(&lstV, &needle) }
}
...
pub fn builtinListZip(args: &aver_rt::AverList<Val>) -> Result<Val, AverStr> {
    crate::cancel_checkpoint();
    let pair = crate::aver_generated::domain::builtins::helpers::twoArgs(args)?;
    { let (aV, bV) = pair; builtinListZipInner(&aV, &bV) }
}
...
#[inline(always)]
pub fn zipListsAcc(mut a: aver_rt::AverList<Val>, mut b: aver_rt::AverList<Val>, mut acc: aver_rt::AverList<Val>) -> aver_rt::AverList<Val> {
    loop {
        crate::cancel_checkpoint();
