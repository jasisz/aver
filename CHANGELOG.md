...
> _Iron in the frame — the type checker stops lying to itself about negation, recursion, and identity; the test bench gains a fuzzer that shakes the rest._

### Added
- **Duplicate `fn` name in a module is now a type error.** Defining the same name twice surfaces `Function 'X' is already defined in this module` at the second site. Pre-Iron the duplicate silently replaced the first definition — a typo could swap your function's body without any signal.
- **Polymorphic recursion that would need `T := F<...T...>` is rejected.** Shapes like `fn nest(v: A) -> Unit; nest([v])` (where the recursive call asks the checker to bind `A` to `List<A>`) surface as a normal type-incompatibility error instead of silently typechecking with a circular binding that later confused downstream backends.

### Improved
- **Unary minus on the VM skips runtime tag-dispatch.** Two new typed opcodes (`NEG_INT` / `NEG_FLOAT`) replace the generic `NEG`'s `is_int / is_float` branch when the operand's source type resolves to `Int` or `Float`. Continues the typed-arith pipeline established in 0.17.2 (`ADD_INT`, `MUL_FLOAT`, …) — `-x` in tight numeric loops now dispatches the same shape as `+`, `*`, `/`.

### Fixed
- **`aver replay` no longer panics on JSON containing multi-byte UTF-8 in unexpected positions.** A 113-byte malformed recording with an em-dash (`—`, 3 UTF-8 bytes) landing where the codec expected a JSON keyword used to abort with `byte index N is not a char boundary` because the keyword parser sliced the `&str` source instead of the byte buffer. Switched to byte-level comparison so the slice is never asked about codepoint boundaries. Found by AFL on the new replay-codec fuzz target inside three minutes of its first PR-smoke run.
- **Pattern parser bounds its own recursion.** A deeply-nested match-arm pattern like `((((...x...))))` used to unwind the parser stack because `parse_pattern` self-recursed through its LParen / tuple arm with no depth limit. Shares the same `MAX_PARSE_DEPTH = 64` cap as `parse_expr`; surfaces as `Expression too deeply nested` rather than a runtime abort.
- **Long unary-minus and postfix chains are a typed parse error instead of a stack-overflow abort.** `------...x` (chained unary minus) and `f()??????...` (chained `?` error-propagation, plus any other postfix `.field` / `(args)` chain) used to abort `aver` with `fatal runtime error: stack overflow` — the parser produced an N-deep AST that downstream walkers (`run_type_check`, `resolve_program`, codegen visitors) then recursed into and exhausted the worker-thread stack on. Both shapes now surface as ordinary `error[parse-error]` diagnostics at the offending source span, capped at 64 entries per chain. Found by the first matrix-mode AFL nightly on main after the recursion-depth guard below; both shapes have their minimised reproducer under `tests/regressions/parser/`.
- **Deeply-nested expressions are a typed parse error instead of a stack-overflow abort.** Inputs the AFL nightly routinely produces — 2500+ nested `(...)`, `Option.Some(...)`, or `{...}` literals — used to crash the whole `aver` invocation with `fatal runtime error: stack overflow, aborting` because the recursive-descent parser had no depth bound. The parser now caps recursion at 64 levels and surfaces `Expression too deeply nested (max 64 levels). Refactor with named bindings or smaller sub-expressions.` as an ordinary `parse-error`. 64 sits comfortably above any hand-written Aver shape (the deepest production expression in the example corpus measures ~12 levels) and comfortably below `cargo test`'s 2 MiB worker-thread stack limit — the binding constraint, not the 8 MiB main-thread default. Three minimised reproducers landed under `tests/regressions/parser/` as the regression net.
- **`aver replay` can load `Vector` values.** `aver run --record` emitted a `$vector` marker, but the replay decoder had no matching arm — recordings touching a Vector value failed to load with `unknown replay marker '$vector'`.
- **`aver replay` can load `Map`s with a single string key.** A 1-entry string-keyed Map encoded as `{"key": value}` collided with the marker-wrap shape; replay tried to interpret `key` as an unknown marker. The decoder now only treats `$`-prefixed keys as markers.
- **`-0.0` keeps its sign bit on every backend.** Pre-Iron `-x` desugared to `0 - x` in the parser, and `0.0 - 0.0` collapses to `+0.0` under IEEE 754. Unary minus is now a first-class AST node (`Expr::Neg`) and every backend negates at the float bit level, so `-0.0` round-trips through compile + eval + replay.
- **`String.fromFloat(x)` on `--wasm-gc` no longer traps on finite floats around `1e4`–`1e18`.** The hand-rolled WAT printer's shortest-roundtrip loop multiplied `abs_val * 10^N` until `i64.trunc_f64_s` overflowed and the module trapped with `wasm trap: integer overflow`. The loop now bails one iteration before overflow, accepting fewer fractional digits at very large magnitudes instead of a crash. Repro that died pre-fix: `String.fromFloat(((-0.5772) * (-61.8024)) * (-877.8128))` (≈ `-31313.64`).
- **A malformed compile no longer panics the `aver` process.** Sources that registered two different meanings under one symbol — duplicate variant constructor with conflicting payload, a constant rebound under a name already used as a function, an empty namespace path — used to unwind the VM compiler with `panicked at vm/symbol.rs`. They now surface as `Compile error: VM symbol 'X' already exists as ...` and point at the offending source. No legal program could trip these panics; only hand-edited intermediate files, fuzz finds, or future codegen bugs hit this path.
- **A single source error no longer fans out into a cascade of "got Invalid" diagnostics.** `let z = add(unknownFn(1), unknownFn(2))` used to report the two real unknown-fn errors plus two follow-on `Argument N of 'add': expected Int, got Invalid` reports — once per downstream consumer that received the recovery sentinel. The matcher now treats `Type::Invalid` as a wildcard ("already reported, don't compound"), so callers see only the originating diagnostic.
- **Two modules can't accidentally share a type just because they spell its name the same.** A project that imports `A` (exposing `type Shape { Circle, Square }`) and `B` (exposing `type Shape { Triangle, Hexagon }`) used to silently let a `B.Shape` value satisfy a parameter typed `A.Shape` — the matcher's `ends_with` fallback collapsed both onto the same key. References now resolve to canonical `Module.Type` form before comparison, so the crossed call is rejected with `expected A.Shape, got B.Shape`. The legitimate same-name-same-type case (calling `Types.open()` against an annotation of `Types.Status`) still type-checks.
