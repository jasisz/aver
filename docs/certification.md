...
> Status: v0, certification level **L1** (conditional on named runtime contracts). Seven certified function classes (three integer, three ADT, and cross-function composition); everything else is **declined fail-closed** and listed with a reason. This document is the contract; the emitted `cert-manifest.json` is its machine-readable form.
