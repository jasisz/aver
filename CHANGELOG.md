...
- **A bound on the length of a countdown writer's output now certifies from the bound one digit below it.** Write the ladder about a base-256 digit writer from its floor up — no digits below 1, then at most one digit below 256, at most two below 65536, at most three below 16777216 — and each rung now certifies from the rung under it. Only the floor rung used to certify; every rung above it stopped at its samples, because the rung below was handed to the proof as an implication whose own condition nobody discharged, and a condition on the shrunk value (`value / 256 < 256` when `value < 65536`) is arithmetic, not rewriting. A rung is now used at the shrunk value with its condition discharged from the condition of the law being proved, and a rung whose condition does not hold at the shrunk value is left out rather than assumed — which is why the ladder needs its floor: without a rung that holds below 1, the bottom rung has nothing under it and keeps its sampled evidence. Refs [#1284](https://github.com/jasisz/aver/issues/1284).
