...
`--target wasm-gc`, `--target wasip2` and `aver run --wasm-gc` / `--wasip2` still refuse a program with a job kind, with `error[work-target]`. A component and a wasm-gc module are single-threaded, so a job on those targets would run inline at `begin` rather than beside the turn; the lowering that does so is not in this build. A program that performs `Wait.poll` or `Work.cancel` without declaring a job kind is refused earlier and with a different slug — `error[capability-target-unsupported]`, `reason[standard-binding-unavailable]` — because neither wasm target binds the `Wait` and `Work` contracts at all, so there is nothing for the job gate to reach. What those two targets do run since this change is a capability the program answers with `answer = "Module"`: the state types, the reply sums and the answer functions the lowering leaves behind are data and pure functions, and the job handle `Work.Job` that every reply sum reaches through `Wait.Wake` now has a wasm representation of its own — one job kinds will mint when their own lowering lands.
