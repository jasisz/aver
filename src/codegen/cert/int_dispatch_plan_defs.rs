...
fn int_dispatch_plan_from_cert(c: &Cert) -> Option<IntDispatchRawPlan> {
...
mod int_dispatch_plan_gate_tests {
    ...
    fn int_dispatch_plan_reproduces_stage0_pins() {
        ...
        let box_plan =
            int_dispatch_plan_from_cert(&box_int_cert(box_int_entry.clone())).expect("boxInt plan");
        let hosts = IntDispatchHostTable {
            box_idx: 34,
            add_idx: None,
            sub_idx: None,
        ...
        assert!(
            int_dispatch_plan_from_cert(&gauge_cert(gauge_entry.clone())).is_some(),
        ...
        noisy[0] += 1;
        assert!(
            int_dispatch_plan_from_cert(&gauge_cert(noisy)).is_none(),
            "a body the canonical plan cannot reproduce must not carry a claim"
        );
        // Empty code entry: no claim.
        assert!(
            int_dispatch_plan_from_cert(&box_int_cert(Vec::new())).is_none(),
