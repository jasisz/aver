...
### Breaking changes
- **`Console.print` / `Console.error` / `Console.warn` / `Terminal.print` / `Terminal.setColor` now take `String`.** Their previous polymorphic signature (a `Printable` type variable that accepted any value and let the runtime stringify per-type) is gone. Stringification is now the caller's job — use interpolation `"{x}"` for primitives, write a per-type render helper for compound shapes (`fn showResult(r: Result<T, E>) -> String` pattern-matching into `"Result.Ok({n})"` / `"Result.Err(\"{msg}\")"`). Programs that called `Console.print(record)` / `Console.print(some_list)` etc. will now fail typecheck with `Argument 1 of 'Console.print': expected String, got <Type>` — the message tells you which call site to migrate. Reason: every backend (legacy wasm, wasm-gc, VM, Rust, self-host) loses its per-type format dispatch. Replay/record stores plain string bytes per call. Effect ABI becomes trivial across hosts. Examples migrated in this commit cover the typical patterns; see `examples/core/calculator.av` for the Result-render shape and `examples/core/lists.av` for List-render via direct recursion.
...
- **`aver run --wasm-gc <file>`** — embedded wasmtime host with engine GC + tail calls. Walks the module's import section once, dispatching `aver/*` names through a single `dispatch_aver_import` switch (Args, Console.print/error/warn, Time.now/unixMs/sleep, Random.int/float, Float.{sin,cos,atan2,pow}). Strings cross via the binary's `__rt_string_*` LM transport bridge. Imports we don't have a real impl for fall back to a typed-zero stub so programs that declare effects without exercising them still link. Lives in `src/main/run_wasm_gc.rs` (commands.rs is already 6 KLoC — keep new entries out of it). 13 of 18 single-file `examples/core/` + `examples/data/` cases run identically to the VM today; 5 known backend bugs (recursive sum eq dispatch, IndependentProduct lowering, generic `E` in Result, List<T> generic, ref/eqref mismatch) remain — each is a separate ticket. Services examples need more host imports (Disk/Http/Tcp/Console.readLine) — out of run-host MVP scope.
- **String comparison BinOps** (`< / > / <= / >=`) lower to a new `__wasmgc_string_compare(a, b) -> i32` helper (lex byte compare, returns -1/0/1) followed by an `i32 op 0`. Discovery force-registers the helper whenever a String-subject ordering BinOp shows up. Previous backend fell through to `i64.lt_s` on `(ref null $string)` operands — invalid wasm.
- **Numeric BinOp emit promotes `Int op Float` to `f64` ops** with `f64.convert_i64_s` on each I64-typed operand. Fixes unary minus on Float (parser desugars `-273.15` to `0 - 273.15` where the `0` is `Literal::Int(0)`).
- **Interpolation lowering covers Float and Bool** (`Float.toString` / `String.fromBool`); discovery force-registers all three per-type stringifiers when any interpolated expression exists.
- **`String.fromInt` / `String.fromFloat`** recognised in `BuiltinName::from_dotted`, routed through the same `Int.toString` / `Float.toString` helpers.
- **`emit_default_value` covers I64 / F64 / I32** in addition to ref types — newtype-flattened `Option<RecordWithSinglePrimitiveField>` emits the matching primitive zero instead of bailing.
