...
| `work-target` | error | A program with a Work-shaped capability, or one that answers a capability with `answer = "Module"`, was compiled or run on a wasm backend (`--target wasm-gc`, `--target wasip2`, `aver run --wasm-gc`, `aver run --wasip2`, `aver verify --wasm-gc`). An answered capability's generated reply types carry `Wait.Wake`, which reaches the job handle `Work.Job`, and neither wasm target has a representation for one. The bytecode VM and `--target rust` both run such a program. | Run the program on the VM or compile it with `--target rust`; the wasm-gc and wasip2 backends follow in a later change. |
