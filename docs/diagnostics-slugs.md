...
| `wait-key` | error | This program keys one wait set by one type and another by another type. A turn has one wait, its keys are how the program says what it is waiting for, and the keys travel out of every backend through one set of helpers built from that type. | Name both kinds as constructors of one type and key every wait by it; `Wait.poll` accepts any key a map accepts, so the type can say whatever the program needs it to say. |
