...
user.instance.exports._start();</pre>
    ...
    <h2>Browser host shim</h2>
    <p>
        A minimal <code>aver/*</code> import set for browser hosts.
        Strings and other heap values arrive as
    ...
    <pre><span class="rt-comment">// Smallest viable shim — covers Console.* and one Time effect.</span>
...
function makeHostImports(memory) {{
    ...
    const readString = (ptr, len) =>
        decoder.decode(readBytes(ptr, len));
    ...
    <h2>Standalone with WASI</h2>
    <p>
        For wasmtime / Cloudflare Workers / Fastly Compute — anywhere
        without a JS host — bundle <code>aver_to_wasi.wasm</code>
        alongside the runtime. It translates <code>aver/*</code>
        imports into <code>wasi_snapshot_preview1.fd_write</code>
        calls, so the only host the result needs is WASI itself.
    </p>
    <pre><span class="rt-comment">// One-shot bundle (binaryen):</span>
<span class="rt-prompt">$</span> wasm-merge \\
      aver_runtime.wasm  aver_runtime \\
      aver_to_wasi.wasm  aver         \\
      dist/mygame.wasm   program      \\
      --rename-export-conflicts        \\
      --enable-bulk-memory --enable-multivalue --enable-tail-call \\
      -o mygame-standalone.wasm

<span class="rt-prompt">$</span> wasmtime mygame-standalone.wasm
<span class="rt-comment"># Runs without any other host wiring.</span>

<span class="rt-comment"># Or skip the merge and just wire all three modules at instantiate time
# in any wasmtime::Linker / @bytecodealliance/preview2-shim setup.</span></pre>
    <p>
        Coverage today: <code>Console.*</code> and
        <code>Format.value</code> /
        <code>Print.value</code>. Other <code>aver/*</code> effects
        (<code>Disk.*</code>, <code>Tcp.*</code>, etc.) trap as
        <code>unreachable</code> in the bridge — wire them through
        the host directly until coverage lands.
    </p>
    ...
    <ul class="rt-list">
        ...
        <li><strong>aver_to_wasi.wasm</strong> — translation shim that
            ...
            Optional — needed only for wasmtime / Cloudflare Workers /
            Fastly Compute, not for browser hosts.</li>
