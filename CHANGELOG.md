...
### Proof export — Lean
- **Private tail-recursive Int countdown workers (Fibonacci's `fibTR`-shape) emit as native recursive defs instead of fuel-encoded helpers.** The compiler reads the worker's single external caller's surrounding guards and uses them as the aux's precondition — `fib`'s `false`-arm of `match (n < 0)` yields `(h_dom : n ≥ 0)`, nested `match (n > 2) { true -> match (n < 500) { true -> worker(n) }}` yields `(h_dom : n > 2 ∧ n < 500)`. Recursive callsites discharge preservation via `(by omega)`; termination on `Int.natAbs n`. Lean can now `decide` / unfold through these workers in proofs instead of treating them as opaque fuel helpers. Exposed fns, multi-caller fns, callers passing non-identity args, and bodies with leading `let` bindings keep the fuel encoding. (#84)
- **Verify `when` clauses that are stronger than a refinement-lifted given's type invariant stay as theorem premises instead of being silently dropped.** Pre-fix every `when` got dropped whenever a given was refinement-lifted (`given a: Int` becoming `∀ (a : Natural)`) on the unchecked assumption it was just restating the type's invariant. A user-written `when a >= 10` over `Natural` (invariant `a.val >= 0`) would lose the `>= 10` part. Both Lean and Dafny now do a commutator-relaxed syntactic match (`a >= 0` ≡ `0 <= a`) against the conjunction of lifted invariants and only drop when the match is bijective. Equivalent predicates still drop cleanly so the `natural.av`-style `when a >= 0 + when b >= 0` cases keep the one-line `∀ (a b : Natural), add a b = add b a` shape; compound `Bool.and(n >= 0, n <= 100)` invariants flatten on both sides so `IntRange` works the same way.
