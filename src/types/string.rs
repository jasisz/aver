...
pub fn register_nv(global: &mut HashMap<String, NanValue>, arena: &mut Arena) {
    let methods = &[
        "len", "byteLength", "startsWith", "endsWith", "contains",
        "slice", "trim", "split", "replace", "join", "charAt", "chars",
        "fromInt", "fromFloat", "fromBool", "toLower", "toUpper",
    ];
...
fn nv_str(v: NanValue, arena: &Arena) -> Option<&str> {
    if v.is_string() { Some(arena.get_string(v.arena_index())) } else { None }
}

fn length_nv(args: &[NanValue], arena: &mut Arena) -> Result<NanValue, RuntimeError> {
    if args.len() != 1 { return Err(RuntimeError::Error(format!("String.len() takes 1 argument, got {}", args.len()))); }
    let s = nv_str(args[0], arena).ok_or_else(|| RuntimeError::Error("String.len: argument must be a String".to_string()))?;
...
fn byte_length_nv(args: &[NanValue], arena: &mut Arena) -> Result<NanValue, RuntimeError> {
    if args.len() != 1 { return Err(RuntimeError::Error(format!("String.byteLength() takes 1 argument, got {}", args.len()))); }
    let s = nv_str(args[0], arena).ok_or_else(|| RuntimeError::Error("String.byteLength: argument must be a String".to_string()))?;
    Ok(NanValue::new_int(s.len() as i64, arena))
}

fn starts_with_nv(args: &[NanValue], arena: &mut Arena) -> Result<NanValue, RuntimeError> {
    if args.len() != 2 { return Err(RuntimeError::Error(format!("String.startsWith() takes 2 arguments, got {}", args.len()))); }
    if !args[0].is_string() || !args[1].is_string() {
        return Err(RuntimeError::Error("String.startsWith: both arguments must be String".to_string()));
    }
...
fn ends_with_nv(args: &[NanValue], arena: &mut Arena) -> Result<NanValue, RuntimeError> {
    if args.len() != 2 { return Err(RuntimeError::Error(format!("String.endsWith() takes 2 arguments, got {}", args.len()))); }
    if !args[0].is_string() || !args[1].is_string() {
        return Err(RuntimeError::Error("String.endsWith: both arguments must be String".to_string()));
    }
    let s = arena.get_string(args[0].arena_index());
...
fn contains_nv(args: &[NanValue], arena: &mut Arena) -> Result<NanValue, RuntimeError> {
    if args.len() != 2 { return Err(RuntimeError::Error(format!("String.contains() takes 2 arguments, got {}", args.len()))); }
    if !args[0].is_string() || !args[1].is_string() {
        return Err(RuntimeError::Error("String.contains: both arguments must be String".to_string()));
    }
    let s = arena.get_string(args[0].arena_index());
...
fn slice_nv(args: &[NanValue], arena: &mut Arena) -> Result<NanValue, RuntimeError> {
    if args.len() != 3 { return Err(RuntimeError::Error(format!("String.slice() takes 3 arguments (s, from, to), got {}", args.len()))); }
    if !args[0].is_string() { return Err(RuntimeError::Error("String.slice: first argument must be a String".to_string())); }
    if !args[1].is_int() { return Err(RuntimeError::Error("String.slice: second argument must be an Int".to_string())); }
    if !args[2].is_int() { return Err(RuntimeError::Error("String.slice: third argument must be an Int".to_string())); }
...
fn trim_nv(args: &[NanValue], arena: &mut Arena) -> Result<NanValue, RuntimeError> {
    if args.len() != 1 { return Err(RuntimeError::Error(format!("String.trim() takes 1 argument, got {}", args.len()))); }
    let s = nv_str(args[0], arena).ok_or_else(|| RuntimeError::Error("String.trim: argument must be a String".to_string()))?;
...
fn split_nv(args: &[NanValue], arena: &mut Arena) -> Result<NanValue, RuntimeError> {
    if args.len() != 2 { return Err(RuntimeError::Error(format!("String.split() takes 2 arguments, got {}", args.len()))); }
    if !args[0].is_string() || !args[1].is_string() {
        return Err(RuntimeError::Error("String.split: both arguments must be String".to_string()));
    }
    let s = arena.get_string(args[0].arena_index()).to_string();
    let delim = arena.get_string(args[1].arena_index()).to_string();
    let parts: Vec<NanValue> = s.split(&*delim)
        .map(|p| {
...
fn replace_nv(args: &[NanValue], arena: &mut Arena) -> Result<NanValue, RuntimeError> {
    if args.len() != 3 { return Err(RuntimeError::Error(format!("String.replace() takes 3 arguments (s, old, new), got {}", args.len()))); }
    if !args[0].is_string() || !args[1].is_string() || !args[2].is_string() {
        return Err(RuntimeError::Error("String.replace: all arguments must be String".to_string()));
    }
    let s = arena.get_string(args[0].arena_index()).to_string();
...
fn join_nv(args: &[NanValue], arena: &mut Arena) -> Result<NanValue, RuntimeError> {
    if args.len() != 2 { return Err(RuntimeError::Error(format!("String.join() takes 2 arguments, got {}", args.len()))); }
    if !args[0].is_list() { return Err(RuntimeError::Error("String.join: first argument must be a List".to_string())); }
    if !args[1].is_string() { return Err(RuntimeError::Error("String.join: second argument must be a String".to_string())); }
    ...
    for item in &items {
        if !item.is_string() {
            return Err(RuntimeError::Error("String.join: list elements must be String".to_string()));
        }
...
fn char_at_nv(args: &[NanValue], arena: &mut Arena) -> Result<NanValue, RuntimeError> {
    if args.len() != 2 { return Err(RuntimeError::Error(format!("String.charAt() takes 2 arguments, got {}", args.len()))); }
    if !args[0].is_string() { return Err(RuntimeError::Error("String.charAt: first argument must be a String".to_string())); }
    if !args[1].is_int() { return Err(RuntimeError::Error("String.charAt: second argument must be an Int".to_string())); }
    let s = arena.get_string(args[0].arena_index());
...
fn chars_nv(args: &[NanValue], arena: &mut Arena) -> Result<NanValue, RuntimeError> {
    if args.len() != 1 { return Err(RuntimeError::Error(format!("String.chars() takes 1 argument, got {}", args.len()))); }
    if !args[0].is_string() { return Err(RuntimeError::Error("String.chars: argument must be a String".to_string())); }
    let s = arena.get_string(args[0].arena_index()).to_string();
    let items: Vec<NanValue> = s.chars().map(|c| {
        let cs = c.to_string();
...
fn from_int_nv(args: &[NanValue], arena: &mut Arena) -> Result<NanValue, RuntimeError> {
    if args.len() != 1 { return Err(RuntimeError::Error(format!("String.fromInt() takes 1 argument, got {}", args.len()))); }
    if !args[0].is_int() { return Err(RuntimeError::Error("String.fromInt: argument must be an Int".to_string())); }
...
fn from_float_nv(args: &[NanValue], arena: &mut Arena) -> Result<NanValue, RuntimeError> {
    if args.len() != 1 { return Err(RuntimeError::Error(format!("String.fromFloat() takes 1 argument, got {}", args.len()))); }
    if !args[0].is_float() { return Err(RuntimeError::Error("String.fromFloat: argument must be a Float".to_string())); }
...
fn from_bool_nv(args: &[NanValue], arena: &mut Arena) -> Result<NanValue, RuntimeError> {
    if args.len() != 1 { return Err(RuntimeError::Error(format!("String.fromBool() takes 1 argument, got {}", args.len()))); }
    if !args[0].is_bool() { return Err(RuntimeError::Error("String.fromBool: argument must be a Bool".to_string())); }
...
fn to_lower_nv(args: &[NanValue], arena: &mut Arena) -> Result<NanValue, RuntimeError> {
    if args.len() != 1 { return Err(RuntimeError::Error(format!("String.toLower() takes 1 argument, got {}", args.len()))); }
    if !args[0].is_string() { return Err(RuntimeError::Error("String.toLower: argument must be a String".to_string())); }
...
fn to_upper_nv(args: &[NanValue], arena: &mut Arena) -> Result<NanValue, RuntimeError> {
    if args.len() != 1 { return Err(RuntimeError::Error(format!("String.toUpper() takes 1 argument, got {}", args.len()))); }
    if !args[0].is_string() { return Err(RuntimeError::Error("String.toUpper: argument must be a String".to_string())); }
