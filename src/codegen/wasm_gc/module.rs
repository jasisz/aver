...
//!    value; every user fn gets its body from the MIR body emitter
//!    (`body::emit_fn_body_via_mir`), the only body emitter (decision
//!    `MirEmitterIsTheOnlyEmitter`). A residual function gets an `unreachable`
//!    trap stub for the rare shape MIR does not cover (a first-class fn value
//!    with no funcref-table slot — a builtin/variant `FnValue` or a let-bound
//!    fn-value call).
//!
...
pub(super) fn emit_module_with(
    ...
    if registry.bignum
        && effect_registry
            .iter()
            .any(|effect| effect == EffectName::TcpSendBytes)
    {
        builtin_registry.register(BuiltinName::StringFromInt);
    }
    ...
    match target {
        ...
        super::TargetMode::Wasip2 => {
            // Populate the wasip2 registry from each effect that lowers on
            // this target. One Aver effect may need several canonical-ABI
            // imports (`Console.print` caches an output-stream handle and
            // then writes through it), and several effects share one import,
            // so the slot list per effect lives beside the effect and the
            // registry deduplicates while preserving first-use order.
            for name in effect_registry.iter() {
                if !name.lowers_on_wasip2() {
                    continue;
                }
                for slot in name.wasip2_slots() {
                    wasip2_imports.register(*slot);
                }
            }
    ...
    // the MIR emitter runs here so its caller_fn registrations match
    // the real code-emit loop. Trap-stub bodies emit no calls.
    //
    // It doubles as the liveness signal for the two comparison helper
    // exports. The MIR emitter reports through the body it is about to
    // emit and nothing else: it sets a flag on `fn_map` at the one lookup
    // every `__aint_cmp` / `__aint_eq` call site goes through. That is not
    // a second reachability analysis over the module — a helper only
    // reachable from another runtime helper (a
    ...
    for (i, _fd) in fn_defs.iter().enumerate() {
        ...
        // fn. Cheaper than threading a separate pre-pass. The MIR/stub
        // decision (`mir_dispatch[i]`)
    ...
    if let Some(tr) = &tcp.read_some {
        let (_, parse_id_fn) = tcp
            .parse_id
            .expect("tcp.read_some gated on tcp_parse_id allocation");
        let cabi_realloc_fn = cabi_realloc.as_ref().map(|c| c.fn_idx).ok_or_else(|| {
            WasmGcError::Validation("tcp.read_some emit requires cabi_realloc fn idx".into())
        })?;
        let blocking_read_fn = wasip2_imports
            .lookup_wasm_fn_idx(super::wasip2_imports::Wasip2ImportSlot::InputStreamBlockingRead)
            .expect("tcp.read_some gate requires blocking-read slot");
        let drop_input_stream_fn = wasip2_imports
            .lookup_wasm_fn_idx(
                super::wasip2_imports::Wasip2ImportSlot::IoStreamsResourceDropInputStream,
            )
            .expect("tcp.read_some gate requires drop-input-stream slot");
        let drop_output_stream_fn = wasip2_imports
            .lookup_wasm_fn_idx(
                super::wasip2_imports::Wasip2ImportSlot::IoStreamsResourceDropOutputStream,
            )
            .expect("tcp.read_some gate requires drop-output-stream slot");
        let drop_tcp_socket_fn = wasip2_imports
            .lookup_wasm_fn_idx(
                super::wasip2_imports::Wasip2ImportSlot::SocketsTcpResourceDropTcpSocket,
            )
            .expect("tcp.read_some gate requires drop-tcp-socket slot");
        let result_ok_fn = factory_exports
            .result_bytes_string_ok
            .ok_or_else(|| {
                WasmGcError::Validation(
                    "tcp.read_some emit requires Result<Bytes,String> Ok factory".into(),
                )
            })?
            .fn_idx;
        let result_err_fn = factory_exports
            ...
            .ok_or_else(|| {
                WasmGcError::Validation(
                    "tcp.read_some emit requires Result<Bytes,String> Err factory".into(),
                )
            })?
            .fn_idx;
        let aint_from_i64_fn = registry.aint_from_i64_fn_idx.ok_or_else(|| {
            WasmGcError::Validation(
                "tcp.read_some emit requires __aint_from_i64 for List<Int> result".into(),
            )
        })?;
        let tcp_pool_global = wasip2_globals
            .as_ref()
            .and_then(|g| g.tcp_pool)
            .ok_or_else(|| {
                WasmGcError::Validation("tcp.read_some emit requires tcp_pool global".into())
            })?;
        let helpers = super::wasip2_tcp::TcpReadSomeHelperFns {
            parse_id_fn,
            cabi_realloc_fn,
            blocking_read_fn,
            result_ok_fn,
            ...
            bump_alloc_ptr_global: wasip2_globals
                .as_ref()
                .map(|g| g.bump_alloc_ptr)
                .expect("tcp.read_some emit requires bump_alloc_ptr global"),
        };
    ...
    for tp in [tcp.poll.as_ref(), tcp.wait.as_ref()].into_iter().flatten() {
        ...
        let lookup = |slot: super::wasip2_imports::Wasip2ImportSlot,
         ...
         -> Result<u32, WasmGcError> {
            wasip2_imports.lookup_wasm_fn_idx(slot).ok_or_else(|| {
                WasmGcError::Validation(format!("tcp.poll emit requires {name} fn idx"))
            })
        };
        let cabi_realloc_fn = cabi_realloc.as_ref().map(|c| c.fn_idx).ok_or_else(|| {
            WasmGcError::Validation("tcp.poll emit requires cabi_realloc fn idx".into())
        })?;
        let aint_cmp_fn = builtin_registry
            .lookup_wasm_fn_idx(super::builtins::BuiltinName::AintCmp)
            .ok_or_else(|| {
                WasmGcError::Validation("tcp.poll emit requires __aint_cmp fn idx".into())
            })?;
        let tcp_pool_global = wasip2_globals
            .as_ref()
            .and_then(|globals| globals.tcp_pool)
            .ok_or_else(|| {
                WasmGcError::Validation("tcp.poll emit requires tcp_pool global".into())
            })?;
        let helpers = super::wasip2_tcp::TcpPollHelperFns {
            parse_id_fn,
            cabi_realloc_fn,
            ...
            bump_alloc_ptr_global: wasip2_globals
                .as_ref()
                .map(|globals| globals.bump_alloc_ptr)
                .expect("tcp.poll emit requires bump_alloc_ptr global"),
        };
        codes.function(&super::wasip2_tcp::emit_tcp_poll(tp, &helpers));
    }
    ...
    if let Some(ts) = &tcp.send_bytes {
        let lookup = |slot: super::wasip2_imports::Wasip2ImportSlot,
                      name: &'static str|
         -> Result<u32, WasmGcError> {
            wasip2_imports.lookup_wasm_fn_idx(slot).ok_or_else(|| {
                WasmGcError::Validation(format!("tcp.sendBytes emit requires {name} fn idx"))
            })
        };
        let instance_network_fn = lookup(
            super::wasip2_imports::Wasip2ImportSlot::SocketsInstanceNetworkInstanceNetwork,
        ...
        let str_to_lm_fn = bridge.as_ref().map(|b| b.to_lm_fn).ok_or_else(|| {
            WasmGcError::Validation(
                "tcp.sendBytes emit requires bridge (__rt_string_to_lm fn idx)".into(),
            )
        })?;
        let string_from_lm_fn = bridge.as_ref().map(|b| b.from_lm_fn).ok_or_else(|| {
            WasmGcError::Validation(
                "tcp.sendBytes emit requires bridge (__rt_string_from_lm fn idx)".into(),
            )
        })?;
        let cabi_realloc_fn = cabi_realloc.as_ref().map(|c| c.fn_idx).ok_or_else(|| {
            WasmGcError::Validation("tcp.sendBytes emit requires cabi_realloc fn idx".into())
        })?;
        let blocking_write_fn = lookup(
            super::wasip2_imports::Wasip2ImportSlot::OutputStreamBlockingWriteAndFlush,
            "OutputStreamBlockingWriteAndFlush",
        ...
        let network_handle_global = wasip2_globals
            ...
            .ok_or_else(|| {
                WasmGcError::Validation(
                    "tcp.sendBytes emit requires network_handle global (Phase 4.1b wireup gate)"
                        .into(),
                )
            })?;
        let bump_alloc_ptr_global = wasip2_globals
            .as_ref()
            .map(|g| g.bump_alloc_ptr)
            .expect("tcp.sendBytes emit requires bump_alloc_ptr global");
        let aint_from_i64_fn = if ts.aint_struct_type_idx.is_some() {
            Some(registry.aint_from_i64_fn_idx.ok_or_else(|| {
                WasmGcError::Validation(
                    "tcp.sendBytes emit requires __aint_from_i64 for List<Int> result".into(),
                )
            })?)
        } else {
            None
        };
        let aint_to_string_fn = if ts.aint_struct_type_idx.is_some() {
            Some(
                builtin_registry
                    .lookup_wasm_fn_idx(BuiltinName::StringFromInt)
                    .ok_or_else(|| {
                        WasmGcError::Validation(
                            "tcp.sendBytes emit requires String.fromInt for byte range errors"
                                .into(),
                        )
                    })?,
            )
        ...
        let helpers = super::wasip2_tcp::TcpSendBytesHelperFns {
            instance_network_fn,
            network_handle_global,
            resolve_addresses_fn,
            resolve_next_address_fn,
    ...
    // It names the map insert helpers, the only bodies that can trap,
    // so a stop inside one prints `Map.set Map<K,V> (table grows; a
    // stop here is a resize bug)` in the engine backtrace instead of
    // `<wasm function N>`. Emitted only when the program instantiates
    // a map, so a map-free module's bytes are unchanged.
...
struct FactoryExports {
    ...
    /// `Tcp.poll` host/replay builders for `Result<List<Int>, String>`.
...
fn allocate_factory_exports(
    ...
    if effect_registry.iter().any(|e| {
        ...
        *next_fn_idx += 1;
        ...
        types.ty().function([], [list_ref]);
        fx.list_int_nil = Some(FactorySlot {
            type_idx: *next_type_idx,
            fn_idx: *next_fn_idx,
        });
        *next_type_idx += 1;
        ...
        || (!external_wait && effect_registry.iter().any(|e| e == EffectName::WaitPoll))
    {
        let result_idx =
            registry
                ...
                .ok_or(WasmGcError::Validation(
                    "Tcp.poll factory requires Result<List<Int>,String> slot".into(),
                ))?;
        let list_idx = registry
            .list_type_idx("List<Int>")
            .ok_or(WasmGcError::Validation(
                "Tcp.poll factory requires List<Int> slot".into(),
            ))?;
        let int_idx = registry.aint_struct_idx.ok_or(WasmGcError::Validation(
            "Tcp.poll factory requires the AverInt slot".into(),
        ))?;
        let string_idx = registry
            .string_array_type_idx
            .ok_or(WasmGcError::Validation(
                "Tcp.poll factory requires String slot".into(),
            ))?;
        let result_ref = ref_null(result_idx);
        let list_ref = ref_null(list_idx);
        let int_ref = ref_null(int_idx);
        ...
        // List<Int> builders are shared with byte-returning effects. Reserve
        // them before the poll Result factories when poll is their sole
        // consumer, matching the common factory body/func-section order.
        if fx.list_int_cons.is_none() {
            types.ty().function([int_ref, list_ref], [list_ref]);
            fx.list_int_cons = Some(FactorySlot {
                type_idx: *next_type_idx,
                fn_idx: *next_fn_idx,
