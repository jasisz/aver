...
- **Effect propagation** is statically enforced (blocks `check`/`run`/`verify`), but runtime-agnostic: no capability passing, no sandboxing. Violations are type errors, not warnings. The effect system has no runtime semantics — it is a static lint that prevents undeclared side effects from crossing function boundaries. Effect names (`Console`, `Io`, `Network`, …) are **tags**, not namespaces: `print` is a global builtin, not `Console.print`. The analogy is `+` rather than `Arithmetic.add` — builtins live outside the namespace system by definition.
