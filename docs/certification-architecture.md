...
| `Module.lean` | Verifier | Rendered from the artifact hash on every run; a package file of that name is ignored |
...
4. Generate `ArtifactBytes.lean` from the bytes read in step 1, and `Module.lean` (`CertModule.wasmSha256`) from their hash. The package cannot supply another numeral. The wall's `Schema` imports `Module`, so with `Module` checker-owned no package module is in the wall's import closure, and a package declaration cannot become what a name in the wall resolves to.
...
7. Write `CheckerWitness.lean` and elaborate it. It pins the Lean data to the JSON envelope and to the staged bytes by `rfl`, pins `ClaimAxes.reportEntries` and `ClaimAxes.reportFacets` as theorems, requires `AverCert.Artifact.certificate` at exactly the accepted type, and pins every declared law-claim and source-bridge at a statement the checker controls. Every statement is read at the root namespace, never in the namespace the manifest's law `theorem` names: that namespace is the package's choice, and inside it a package constant `Evil.Tiny.addTwo` would be what the text `Tiny.addTwo` means. The producer writes each law statement with every model name `_root_.`-qualified, and a law that lists bridges must name each bridged model exactly as `_root_.<model>`. The statement gate refuses `set_option` and `open` in a statement, so no statement can change the options or the names it is elaborated with. Each statement is elaborated alone as a checker definition and the pins conjoin the definitions, so no statement text can re-associate a pin's conjunction. The witness imports no `Lean`, runs no code, names everything `_root_`-qualified and spells numerals with `nat_lit`, so no package namespace or instance takes part in what a pin says. `_root_` alone does not stop a package from redirecting a pin: Lean resolves a dotted name to the longest prefix that is a declared constant and reads the rest as field accesses, so `_root_.AverCert.manifest.subject.contracts` would mean a package constant `AverCert.manifest.subject` if one were declared. So a pin never continues a dotted name past a package constant. It names the package constants `AverCert.manifest` and `AverCert.Artifact.data` in full and reads their fields through the wall structures' projection functions, for example `_root_.AverCert.Schema.Subject.contracts (_root_.AverCert.Schema.Manifest.subject _root_.AverCert.manifest)`. Those projections are wall constants in wall namespaces, where the audit (step 8) refuses every package declaration. A pin that does not elaborate declines the package.
8. Run the checker's audit program (`lake env lean --run CheckerAudit.lean`). It is compiled with only the Lean toolchain in scope and loads the built witness environment when it runs. It declines a package that declares under the reserved `AverCertChecker` prefix or under a wall namespace (any `AverCert.*` other than exactly `AverCert.manifest` and `AverCert.subject` or a name inside the producer's own `Plans`, `Artifact`, `Final`, `Bridge` and `Laws`, nested ones such as `AverCert.AcceptedArtifact.AverCert.*` included), or a constant under `AverCert` whose name extends another declared constant (`AverCert.Artifact.data.manifest`), extends the parser, declares a scoped instance or an instance outside the admitted forms (the class is read off the elaborated declaration), or whose bridge encoders do not list a record's fields or a sum's constructors exactly. It also declines a bridged law whose elaborated statement does not use every model constant its bridges are about. A package constant such as `Evil.Tiny.addTwo` beside the model `Tiny.addTwo` needs no rule of its own: the witness reads the statement at the root, where `_root_.Tiny.addTwo` means only the model. It rejects any axiom outside `propext`, `Classical.choice` and `Quot.sound` under the accepted root or a report pin, and logs one audit line for every law, bridged-law and bridge pin; a pin whose axioms leave the whitelist loses only its own credit.
...
Build caches are off by default, and only `check` uses them: `verify` ignores a configured cache and says so, so its verdict never rests on local cache state. For `check`, a configured data or prelude cache never replaces the witness or the audit program, but its directory is trusted local state: its integrity manifest detects accidental corruption, not a writer who replaces the `.olean` files, the Lake traces and the manifest together. Caches shipped inside a package are ignored.
...
- for `check` only, any explicitly configured build-cache directory.
