...
- **`Console.print/error/warn` and `Terminal.print/setColor` now take `String`** (not the polymorphic `Type::Var("Printable")` wildcard). Stringification of non-String values is the caller's job — interpolation `"{x}"` for primitives, per-type render helpers for compound shapes (`fn showResult(r: Result<T, E>) -> String` etc.). Effect ABI stays trivial across every backend; replay stores plain string bytes per call. Examples carrying compound prints migrated.
