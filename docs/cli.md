...
- `--optimize size|speed`: post-process with binaryen `-Oz` (size) or `-O3` (speed). It drops the name section, so a wasm trap reports `<wasm function N>` instead of the name of the function that trapped — see the `Map` capacity note below.
...
**`Map` capacity on wasm-gc.** A `Map<K, V>` compiled to `wasm-gc` (and therefore to `wasip2`, which wraps it) has **16384 buckets and never grows**. Storing the 16385th distinct key traps: the entry has nowhere to go, and the table cannot resize. The trap names the limit — `Map.set Map<Int,Int> in place (fixed capacity 16384, no resize)` in the engine backtrace — unless the module was built with `--optimize`, which strips the names and inlines the helper; `aver compile` prints a note when that applies to your program. Reads are unaffected: looking up an absent key in a full map returns `Option.None` / the supplied default, and `Map.has` returns `false`. On `--target wasip2` this is reachable from a peer, since incoming HTTP headers are accumulated into a `Map<String, List<String>>` — more than 16384 distinct header names traps the guest. The VM and Rust targets have no such limit.
