...
- Script a stub by call index within one function body. An exported proof numbers one body at a time, so it cannot follow a run across a call into an effectful helper, on each turn of a recursion, or past a `match` whose arms call the operation a different number of times; `aver proof` declines a law over such a function with a named reason instead of numbering it differently, and sampled cases still export. `docs/oracle.md` names the four shapes
