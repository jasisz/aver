...
pub fn classifyFastExpr(
    ...
    match crate::aver_generated::domain::resolver::fast::classifyFastLeafExpr(expr) {
        Some(leaf @ _) => crate::aver_generated::domain::ast::FnFastPath::FastLeaf(leaf),
...
pub fn classifyFastListArmPair(
    ...
    match leaf1 {
        Some(v1 @ _) => match leaf2 {
            Some(v2 @ _) => {
                crate::aver_generated::domain::resolver::fast::classifyFastListPatterns(
                    slot,
                    &arm1.pattern,
                    &arm1.bindingSlots,
                    &v1,
...
pub fn classifyFastListCons(
    ...
    match bindingSlots.get(&head).cloned() {
        Some(headSlot @ _) => match bindingSlots.get(&tail).cloned() {
            Some(tailSlot @ _) => {
...
pub fn classifyFastForwardCall(
    ...
    match crate::aver_generated::domain::resolver::fast::classifyFastForwardSlots__collected(
        args.clone(),
        aver_rt::int_list_builder_new((aver_rt::AverInt::from_i64(0)).to_usize().unwrap_or(0)),
    ) {
        Some(slotArgs @ _) => {
...
pub fn classifyFastForwardSlots(
    ...
    loop {
        ...
        crate::aver_generated::domain::ast::Expr::ExprSlot(slot) => {
            {
            let __tco0 = rest;
            let __tco1 = aver_rt::AverIntList::prepend(slot, &acc);
            args = __tco0;
            acc = __tco1;
            continue;
        }
...
pub fn classifyFastForwardSlots__collected(
    ...
    loop {
        crate::cancel_checkpoint();
        aver_list_match!(args, [] => { return Some(aver_rt::int_list_builder_finalize(acc)); }, [arg, rest] => { match arg {
        crate::aver_generated::domain::ast::Expr::ExprSlot(slot) => {
            {
            let __tco0 = rest;
            let __tco1 = aver_rt::int_list_builder_push(acc, slot);
            args = __tco0;
            acc = __tco1;
            continue;
        }
