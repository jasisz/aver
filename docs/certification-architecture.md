...
7. Write `CheckerWitness.lean` and elaborate it. It pins the Lean data to the JSON envelope and to the staged bytes by `rfl`, pins `ClaimAxes.reportEntries` and `ClaimAxes.reportFacets`, requires `AverCert.Artifact.certificate` at exactly the accepted type, and pins every declared law-claim and source-bridge at a statement the checker controls.
8. Collect the axioms of `AverCertChecker.checked` and reject any outside `propext`, `Classical.choice` and `Quot.sound`. Audit each law and bridge pin the same way; a pin that leaves the whitelist loses only its own credit, and a pin that does not elaborate declines the package.
9. Replay `ArtifactCertificate`, the module holding the root, and its import closure with `lake env leanchecker --fresh`. Only then is the report printed.

`aver cert check` runs steps 1 to 8 and skips step 9. It trusts the locally built or cached `.olean` files and prints `CHECKED`, never `CERTIFIED`. The witness is still written and elaborated on every run, so the pins and the axiom audit always run.
...
The wall is one hash-addressed unit of 21 Lean files. By module:
...
The verifier rejects invalid Wasm, hash, version or wall mismatches, unsafe package files, ill-typed plans, lowerings that differ from the code entry, a type table that does not match the bytes, obligations that differ from the derived ones, a wrong contract list, Lean build failures, a mismatched root, non-whitelisted axioms and a failed replay.
