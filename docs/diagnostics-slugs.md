...
| `answer-binding` | error | A capability is marked `answer = "Module"` in `aver.toml` and the module cannot answer it: the capability is one the compiler ships (`Console`, `Disk`, `Tcp`, `Time`, `Wait`, `Work`, …), which the runtime's own providers answer and the generated turn calls itself; or the program has no such module; or an operation of the capability has no answer function; or an answer function's signature is not `(S, a1: T1, …) -> Tuple<S, Cap.__<Op>Reply>`; or one module answering several capabilities threads more than one state; or two answered capabilities of one module share an operation name, so one function would have to answer both. `answer` beside `crate`/`package`/`factory`/`version`/`path` or beside `work`, `task`/`landed` beside `answer`, and an `answer` value that is not one module name, are rejected by the manifest itself. | Declare a capability of your own rather than marking one the compiler ships; write one answer function per operation over one state, returning that state beside the operation's generated reply sum; rename an operation when two answered capabilities collide. |
...
| `work-target` | error | A program with a Work-shaped capability was compiled or run on a backend other than the bytecode VM (`--target rust`, `--target wasm-gc`, `--target wasip2`, `aver run --wasm-gc`, `aver run --wasip2`, `aver verify --wasm-gc`). | Run the program on the VM; the Rust, wasm-gc and wasip2 backends follow in a later change. |
