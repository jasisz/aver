...
| `ArtifactLayout.lean` | The declared module layout (`DeclaredLayout.Layout`): the imported function count; for every defined function its type index and the byte offset and length of its code entry, as packed tables; the exact function type of every planned function; and every plan entry's name, as characters, with its export position. Producer data: `layout_ok` confirms it against the staged bytes (section 7.7). |
...
The producer never emits, and MUST NOT emit, `ArtifactBytes.lean`, `ArtifactComponentBytes.lean`, `CheckerWitness.lean`, `CheckerAudit.lean`, `lakefile.lean`, `lean-toolchain`, any wall source, any build cache, or plan sidecars (`*.plan`, JSON plan trees).
...
After the package builds, the verifier writes `CheckerWitness.lean`, which is never read from the package. It is data for the kernel: theorems only, no `import Lean` and no command that runs code. It imports `AcceptedArtifact`, `ArtifactBytes`, `Manifest`, `Artifact`, `Laws` and `Bridge` when declared, and `ArtifactCertificate`. Every name in it is `_root_`-qualified, so a package declaration placed where an unqualified name would resolve first (for example `AverCertChecker.AverCert.AcceptedArtifact.accepted`, under the witness's own namespace) cannot be reached. Every numeral is a `nat_lit` and every `Int` is built from `Int.ofNat` or `Int.negSucc`, so no `OfNat` instance takes part in what a pin says. It contains, in order:

- one bridge pin per `sourceBridges` entry, at the root with no `open`: `theorem _root_.AverCertChecker.bridge_pin_<i> : (<rendered statement>) ∧ (_root_.AverCert.Schema.Holds _root_.AverCert.manifest) := _root_.AverCert.Bridge.<export>_certified`;
...
1. declines a package that declares any constant under the reserved `AverCertChecker` prefix;
...
3. checks every record and sum a bridge encoder reads (section 9.2);
...
- **Caches as evidence.** Package caches are ignored; local caches cannot skip the witness, the audit or the replay.
