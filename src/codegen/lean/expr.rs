...
fn emit_match(
    ...
    // Nonrecursive bodies never carry named equations: a dependent matcher
    // around a helper result can prevent simp from reducing its nested
    // observations, and a body must not change because a law was stated
    // about it (#1404) — only recursive bodies retain the equations their
    // termination proofs consume. A constructor-case proof that needs the
    // equation asks the tactic for it (`split` names one per branch).
