...
- Script a stub by call index within one function body. An exported proof numbers one body at a time, so it restarts every operation at 0 across a call into an effectful helper and on each turn of a recursion while a run keeps counting; such a law passes `aver verify` and fails to certify. `docs/oracle.md` names the three shapes
