...
theorem construct_canonical_discharges
  ...
  intro S add sub mul stringEq stringConcat toIndex cmp eq
    _hAdd _hSub _hMul _hStringEq _hStringConcat _hToIndex _hCmp _hEq fuel x args w hDom hRun
  rcases hSemantic S x args hDom with ⟨hLen, hCod⟩
  cases fuel with
  | zero => simp [wFuncN] at hRun
  | succ fuel =>
      have hCall := ConstructVerbatimSoundness.generic_construct_certified
        structIdx plan code (host add sub mul stringEq stringConcat toIndex cmp eq) self 1
        hCheck body hLow hCode args hLen
      have hFuel := construct_run_succ_eq_one
        structIdx plan code (host add sub mul stringEq stringConcat toIndex cmp eq) self
...
theorem constructUnary_canonical_discharges
  ...
  intro S add sub mul stringEq stringConcat toIndex cmp eq
    _hAdd _hSub _hMul _hStringEq _hStringConcat _hToIndex _hCmp _hEq fuel p args w hDom hRun
  subst args
