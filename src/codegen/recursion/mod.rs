...
/// target fn into a call to `aux_name` carrying an extra
/// `OMEGA_PROOF_SENTINEL` ident. The proof-mode `IntCountdownGuarded`
/// shape stores the rewritten arms on `ProofIR` (now resolved), so
/// the rewriter that lifts `fn(args)` to `aux_name(args,
/// OMEGA_PROOF_SENTINEL)` works in resolved space too.
///
