...
- **`aver proof` declines a law it cannot number the way a run numbers it, instead of exporting a theorem about a different function.** An exported proof numbers the calls of one function body at a time, and four shapes leave it unable to follow a run: a call into an effectful function, a recursive call, a second operation inside a `Process.stopRequested` poll loop, and a call after a `match` whose arms call that operation a different number of times. A law is checked on samples and proved for every input, so a law over such a function could pass `aver verify`, prove in the kernel and still be false of the run — a false certificate reached with every step passing. Each such law is now declined with a reason naming the call and the operation, in the report and in the emitted Lean or Dafny, and counted among the declined claims, so nothing is proved about it. `aver verify` is unchanged, and so are sampled `verify` cases: a case is one concrete evaluation the run has already checked, so the exported model either agrees with it or fails the proof visibly. A poll loop reaching no second operation stays exact and keeps its theorem, as do `match` arms that call an operation equally often. See "Where a law is declined instead of exported" in `docs/oracle.md`.
