...
- **An optimisation can no longer change what gets proven.** The passes that make a program faster rewrite the program's own code — the string-building rewrite above replaces a loop and the call that consumes it — and the Lean and Dafny exports read that code to state their theorems. Keeping the two apart was a convention: every path that exports a proof had to remember to switch the optimisations off first, and one that forgot would have exported theorems about the rewritten program instead of the one you wrote. The compiler now snapshots your program before the first optimisation runs and hands that snapshot to the exporters, so a proof always describes the source you wrote, whatever the same build did on its way to a binary. Every `.lean` and `.dfy` Aver emits for `examples/` and its own fixtures is byte-for-byte what it was.
