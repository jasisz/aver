...
| `Wait.poll` | `(Map<Int, Wait.Item>, Int) -> Result<List<Int>, String>` | One wait over sockets and jobs together. Returns sorted caller keys; `[]` means timeout. A timeout beyond a century is capped at a century: past that it means "no deadline within reach", which is the wait such a number asked for. |
...
A generated coordinator runs on both targets. On wasip2 its signal stop flag stays `false`: it stops through the program's policy or normal exhaustion, with no host signal subscription. Explicit `Process.stopRequested` calls still fail the wasip2 capability check. VM, generated Rust and wasm-gc continue to observe their host stop flag.

`--target wasm-gc`, `--target wasip2` and `aver run --wasm-gc` / `--wasip2` run a program with a job kind, the one wait of a turn, `Work.cancel` and the generated coordinator. Aver currently executes jobs on both wasm targets **inline at `begin`**, inside the turn. This is the current backend implementation, not a requirement that all WebAssembly execution be single-threaded. `begin` calls the bound function at once and answers a handle that already carries the result, so a `take` asked in the very next expression answers `Ok(Some(r))` where the VM would still answer `Ok(None)`. Everything a program can read is the VM's: a second `take` answers `Err("work: job already taken")`, a take after a cancel answers `Err("work: job cancelled")`, and a handle offered to another kind's `take` answers `Err("work: this job was not started by job kind 'K'")`. The handle is the whole job — its identity, its state, the kind that minted it and the answer — so two copies of one handle are one job, and the module keeps no table beside it.

`Work.cancel` drops the answer of a job nobody collected and changes nothing for one already taken or already cancelled. Because the job has already run when the cancel lands, cancelling on these targets never stops work in flight; what a program observes is the answer, and the answer is the VM's.

`Wait.poll` splits one wait set the way the VM does: a `Socket` item goes to the same socket poll the target already has — the host reactor on wasm-gc, `wasi:io/poll` on wasip2 — and every `Job` key is ready at once, because a job here is done the moment it began. A wait set holding any job therefore returns immediately with those keys merged in and sorted ascending; a set of sockets alone honours the timeout exactly as `Tcp.poll` does. An empty wait set reports `[]` without sleeping, which is the one case where the timeout is not waited out.

`[work] max-jobs` decides nothing here: at most one job is ever running, because it is over before the next expression. A manifest that sets the key for a wasm compile gets `warning[work-max-jobs-ignored]` at the program door, naming the key and the target, and the program runs. A program written against the limit sees the difference — a second `begin` at `max-jobs = 1` succeeds here and answers `Err("work: job limit 1 reached")` on the VM.

Recording on wasm-gc has the VM's shape. `begin` records with its task and the handle it minted, `take` with the answer it gave, `Work.cancel` and `Wait.poll` with their own, so a recording made here replays on the VM and a VM recording replays here, effect for effect. Replay hands back the recorded answers in the recorded turns — a wait that reported one job on the VM reports one job here — while the bound function still runs beside them, which is the recomputation the VM makes too. `--record` stays refused on wasip2: a component records nothing.
