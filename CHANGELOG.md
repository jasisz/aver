...
- **Artifact certificates now cover plain arithmetic on Int arguments.** A function that computes an integer from its own integer arguments — `double(n) = n * 2`, `addPair(a, b) = a + b`, `combineSurrogates(hi, lo) = 65536 + (hi - 55296) * 1024 + (lo - 56320)` — used to be reported as source-level only unless it happened to be exactly `n + k`. Such bodies now get a certificate for the exact bytes the compiler already emits, through the same face that already covered record arithmetic, and their report line shows the expression the bytes compute. `n + k` and the two Int comparisons (`a >= b`, `a == b`) move onto that face as well: their certificate now holds for every canonical carrier rather than only for the small-value encoding, and `aver-cert explain` prints that domain on the export's own line.
