...
pub struct CodegenContext {
    ...
    /// Whether a `match` on a local name is emitted with a named equation
    /// (`match h_NN : x with`). Off by default: only the body of a recursive
    /// function turns it on, for its termination proof. Law statements,
...
fn codegen_ctx_fn_sig(ctx: &CodegenContext, name: &str) -> Option<crate::verify_law::FnSigInfo> {
    ...
    match name {
        ...
        "__byt_new" | "__byt_push" => Some(FnSigInfo {
            return_type: crate::types::Type::named(crate::ir::INTERNAL_BYTE_BUILDER_TYPE),
            is_pure: true,
        }),
        "__byt_finalize" => Some(FnSigInfo {
            return_type: crate::types::Type::Result(
                Box::new(crate::types::Type::named(
                    crate::ir::INTERNAL_BYTE_PAYLOAD_TYPE,
                )),
