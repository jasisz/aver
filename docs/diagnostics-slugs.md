...
| `work-target` | error | A program with a Work-shaped capability, or one that performs `Wait.poll` or `Work.cancel`, was compiled or run on a wasm backend (`--target wasm-gc`, `--target wasip2`, `aver run --wasm-gc`, `aver run --wasip2`, `aver verify --wasm-gc`). A job kind is answered by a function of the program and the one wait of a turn is the `Wait` contract, and neither wasm target binds those yet. The bytecode VM and `--target rust` both run such a program. A capability answered with `answer = "Module"` is no longer refused anywhere. | Run the program on the VM or compile it with `--target rust`; the wasm-gc and wasip2 backends follow in a later change. |
