...
pub enum BuiltinIntrinsic {
    ...
    LstFinalize,
    /// `__byt_new(<capacity>)` — `Int -> __ByteBuilder`, a fresh byte
    ...
    BytNew,
    /// `__byt_push(<builder>, <elem>)` — `(__ByteBuilder, Int) ->
    /// __ByteBuilder`, the builder with `elem` appended when it is in
    ...
    BytPush,
    /// `__byt_finalize(<builder>)` — `__ByteBuilder ->
    /// Result<__BytePayload, String>`, what `Bytes.fromList` would answer for
    /// the pushed elements: `Ok` of validated packed octets, or `Err` of the
    /// standard library's own message about the first non-octet. The retarget
    /// wraps the `Ok` payload into the `Bytes` record in synthesized AST.
    BytFinalize,
}
