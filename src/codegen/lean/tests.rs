...
fn transpile_proves_sortedness_of_insertion_law_as_universal() {
    ...
    // = true` by synthesizing the head-of-insert lower-bound lemma the bare list IH
    // cannot supply, then inducting on the list given.
    let mut ctx = ctx_from_source(
        r#"
module SortedInsert
    intent = "sorted xs implies sorted (insort x xs)"
...
"#,
    ...
    assert!(
        lean.contains("-- aver:law-class insort_law_sortedInsort universal insort.sortedInsort")
    );
    // The TRUE-universal conditional statement.
    assert!(lean.contains(
        "theorem insort_law_sortedInsort : ∀ (x : Nat) (xs : List Nat), sorted xs = true -> sorted (insort x xs) = true := by"
    ));
    // The synthesized head-of-insert lower-bound support lemma (law-scoped uid, no
