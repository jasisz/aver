...
> _The compiler grew a real pipeline. Every IR-level transform is now a named, ordered, individually-observable stage; every consumer of derived facts reads from one place; every backend's relationship to the IR is now a documented contract._

### Added
- **Ordered compiler pipeline (`src/ir/pipeline.rs`).** Seven stages in fixed order — `tco → typecheck → interp_lower → buffer_build → resolve → last_use → analyze`. Single source of truth replaces five inline duplications of the canonical sequence (`cmd_run_vm`, `build_codegen_context`, `load_module_recursive`, `replay_cmd`, every playground entry). Each stage has both a per-stage public function (`pipeline::tco`, `pipeline::resolve`, …) and a corresponding `PipelineConfig` flag (`run_tco`, `run_resolve`, …) so callers can run the whole sequence or just one stage. No bundled "traversal lowering" toggle — `run_interp_lower` and `run_buffer_build` are independent flags that match the pipeline gates 1-to-1.
- **`Analyze` stage centralises five IR analyses** that codegen, dump, and the type checker were each computing on their own: `compute_alloc_info`, `classify_thin_fn_def`, `tailcall_scc_components` (mutual-TCO members), `find_recursive_fns`, `recursive_callsite_counts`. Result lands in `PipelineResult.analysis: AnalysisResult` carrying per-fn `FnAnalysis { allocates, thin_kind, body_shape, local_count, mutual_tco_member, recursive, recursive_call_count }` plus global `mutual_tco_members: HashSet<String>` and `recursive_fns: HashSet<String>` sets. Backend-specific bits (alloc policy) are caller-provided via `cfg.alloc_policy`; policy-independent facts run unconditionally.
- **`LastUse` stage** is its own thing now, not folded into the resolver. Annotates each `Expr::Resolved` slot with `last_use: bool` so backends can MOVE instead of COPY (VM `MOVE_LOCAL`, Rust skips `.clone()`, owned-mutate fast paths). Splitting it out means `--emit-ir-after=resolve` shows IR with slots but no ownership flags, `--emit-ir-after=last_use` shows the diff — every consumer of unowned vs owned slots is now individually observable.
- **`aver compile FILE --emit-ir-after=PASS` observability flag.** Runs the canonical pipeline up through the named stage and prints the IR snapshot. Eight legal stage names (`parse` for the AST as the parser produced it, plus the seven pipeline stages). Output uses Aver-like surface syntax with explicit pass markers — `<tail-call:fn>(args)` (TCO output), `<name:last>` (LastUse output), `__buf_*` / `__to_str` (interp_lower / buffer_build intrinsics) — so a `diff -u` between two stages shows exactly which expressions a pass rewrote. After the analyze stage runs, FnDef headers also carry per-fn fact tags: `[no_alloc, locals=2, recursive×2, body=block:3 (direct), mutual-tco]`.
- **`aver bench scenario.toml [--target={vm,wasm-local,rust}] [--json]` scenario harness.** Three targets:
  - `vm` (default) — in-process `vm::compile_program_with_modules` + `VM::run`. Single µs-µs measurement, no spawn cost.
  - `wasm-local` — `aver compile --target wasm --bridge wasip1` produces a WASI-bundled `.wasm`; wasmtime Engine + Module are built once, each iteration creates a fresh Store + Instance and invokes `_start`. Mirrors the cargo bench shape so VM/WASM numbers are directly comparable. Requires the `wasm` feature. WASI imports are stubbed in-process — bench programs return Int and never touch the host.
  - `rust` — `aver compile --target rust` + `cargo build --release` produces a native binary; each iteration spawns it once. Includes process spawn overhead (~1-2 ms on macOS) — for programs that take <1 ms in pure compute the spawn dominates, just like in cargo bench measurements.
  Manifest fields: `entry`, `iterations`, `warmup`, `args`, optional `[expected]` response-byte constraints, optional `[tolerance]` regression budgets. Human-readable text by default; `--json` produces the structured shape downstream tools consume. Directory mode (`aver bench bench/scenarios/`) globs every `*.toml`, runs them alphabetically, and emits one report per scenario — NDJSON when combined with `--json` for streaming consumers. Thirteen scenarios ship in `bench/scenarios/`: the eleven cargo-bench programs (fib, countdown, record, map_build, map_lookup, match_dispatch, string_interp, vector_ops, three newtype variants) plus `factorial` (smoke) and `fractal_seahorse` (deforestation regression). The cargo bench (`benches/comparison_bench.rs`) and `aver bench` read from the same `.av` files via `include_str!`, so VM/WASM/Rust/self-host comparison and the in-process scenario harness measure the exact same programs. Release script (`tools/release.py`) runs the full scenario suite in directory mode as a smoke gate before publishing. Reports carry `backend` (aver version, build profile, optional wasmtime version) and `host` (os/arch/cpus) blocks so downstream tools join on `host.os + host.arch + backend.name` to compare like-for-like across runs. Full reference: `docs/bench.md`.
- **`aver bench --save-baseline PATH` / `--compare PATH` / `--fail-on-regression`.** Capture a baseline locally and compare; current p50_ms / p95_ms vs. stored values, exit non-zero when any gated metric exceeds its tolerance budget. Per-scenario tolerances configurable via `[tolerance] wall_time_p50_pct = 25.0` (default 20%), `wall_time_p95_pct = 35.0` (default 30%). 0.15.1 ships the harness as a local-use feature — no checked-in baselines, no CI gate yet. CI integration (workflow + cross-machine baseline calibration) lands in 0.15.2.

### Fixed
- **Proof export traversal leak on dep modules.** `aver proof bench.av --backend lean` already passed `apply_traversal_lowering: false` for the entry, but `load_module_recursive` had hardcoded `interp_lower` + `buffer_build` calls — every dep module ran the deforestation passes regardless of the proof flag, leaking synthesized `<fn>__buffered` variants into Lean/Dafny codegen (`warning: recursive function 'allRows__buffered' is outside proof subset`). Threaded the per-stage flags through `load_compile_deps` → `load_module_recursive` → its `PipelineConfig` so dep modules honor the entry-level decision. Pre-existing bug surfaced and fixed by the pipeline migration.
- **Playground bypassed deforestation by accident.** `compile_to_wasm`, `compile_project_to_wasm`, `compile_rust_files`, `compile_rust_files_project` predated 0.15 — when the deforestation passes landed for the CLI, the playground entry points were missed. Anyone downloading a WASM/Rust artifact from the playground got a slower binary than the CLI compile produced for the same source. Same for `playground::run_record` / `replay_run_inner` — recordings store effects, not IR shape, so running interp_lower + buffer_build during record/replay produces the same effect sequence as the un-lowered run. All these paths now use the canonical full pipeline.

### Changed
- **`PipelineConfig` flag shape: per-stage booleans instead of bundled `apply_traversal_lowering`.** Every stage has its own `run_*: bool` field (`run_tco`, `run_interp_lower`, `run_buffer_build`, `run_resolve`, `run_last_use`, `run_analyze`) plus typecheck slot (`typecheck: Option<TypecheckMode>`). Drops `stop_after`. Caller-level `apply_traversal_lowering: bool` (the proof-vs-runtime CLI distinction) translates to the two per-stage booleans at exactly one boundary (`build_codegen_context`); below that it's split-flags everywhere.
- **Per-stage public API: every IR analysis exposes a stable function in `pipeline::*`.** Direct calls into `tco::transform_program` / `resolver::resolve_program` / `lower_interpolation_pass` / `run_buffer_build_pass` / `run_type_check_full` are gone from the production tree (and from the test files where they lived) — every consumer goes through the `pipeline::*` namespace.
- **All codegen consumers read from `PipelineResult.analysis` / `CodegenContext.{mutual_tco_members,recursive_fns,fn_analyses}`.** Five ad-hoc analyses unioned into one centralised flow (`compute_alloc_info`, `tailcall_scc_components`, `find_recursive_fns`, `recursive_callsite_counts`, `classify_thin_fn_def`):
  - `compute_memo_fns` reads `recursive_fns` + per-fn `recursive_call_count`.
  - `codegen::build_context` builds `mutual_tco_members` / `recursive_fns` as the union of entry's `entry_analysis` plus each dep module's `module.analysis` (sound under Aver's module DAG — cross-module SCCs are mathematically impossible by construction; documented in `src/ir/analyze.rs` and pinned in memory).
  - Rust codegen reads `ctx.mutual_tco_members` (entry path) / `module.analysis.mutual_tco_members` (dep path) for trampoline emission membership.
  - Lean codegen's `recursive_pure_fn_names` reads `ctx.recursive_fns`. `transpile`/`transpile_for_proof_mode`/`transpile_with_verify_mode` take `&mut CodegenContext` and call `ctx.refresh_facts()` upfront — idempotent for production (build_context already populated), test stubs (32 sites using `empty_ctx()` + push-in-place) get fresh sets without per-test refresh calls.
  - VM compiler's `chunk.no_alloc` flag reads `analysis.fn_analyses[name].allocates`; falls back to in-place `compute_alloc_info` only for callers that didn't run the pipeline (test harnesses, `aver verify`).
  - WASM emitter's `no_alloc_by_canonical` map reads `ctx.fn_analyses[name].allocates`; same fallback shape.
  - VmAllocPolicy / WasmAllocPolicy / NeutralAllocPolicy agree on every name today, so the pipeline analyze runs with `NeutralAllocPolicy` and downstream readers get correct answers for both runtime backends.
- **VM compile API consolidated.** `compile_program` / `compile_program_with_modules` / `compile_program_with_loaded_modules` all take `analysis: Option<&AnalysisResult>` as a peer parameter (no special `_and_analysis` variant). Production callers pass `pipeline_result.analysis.as_ref()`; verify / REPL / test paths pass `None` and hit the in-place `compute_alloc_info` fallback.
- **`Expr::Resolved` printer (`checker::expr_to_str`).** Used to print every resolved local as the opaque string `"<resolved>"` regardless of name or ownership — now prints `"<name>"` or `"<name:last>"` when LastUse marked it terminal. Drives `--emit-ir-after=last_use` showing exactly which slots got the ownership annotation.

### Pipeline contract (new)
- WASM and Rust codegen no longer carry InterpolatedStr handlers. After closing the playground bypass paths, every WASM/Rust emit site sees post-`interp_lower` IR — the InterpolatedStr branches in `codegen::wasm::expr::emit::dispatch`, `codegen::wasm::expr::infer` (×2), `codegen::wasm::emitter::collect_strings_from_expr`, and `codegen::rust::expr::dispatch` are now `unreachable!()` with a comment naming the contract. Roughly 100 lines of dead code and helper enums (`emit_interpolated_str` / `emit_str_part` in WASM, `emit_interpolated_str` + `DisplayKind` enum + `expr_display_type` in Rust) deleted.
- VM keeps its `compile_interpolated_str` because the REPL is a real consumer of pre-lower IR (single-statement evaluation, full pipeline overhead per line not worth amortising). REPL is the lone legitimate VM bypass.
