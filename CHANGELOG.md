...
- **Jobs, the one wait and a generated coordinator run on wasm-gc and wasip2.** `aver run --wasm-gc`, `aver run --wasip2`, `aver compile --target wasm-gc` and `--target wasip2` accept a program that declares a job kind, performs `Wait.poll` or `Work.cancel`, or asks for its loop with a `[run]` table. `error[work-target]` is history. One honest difference stays, and it is the wall clock: a component and a wasm-gc module are single-threaded, so a job runs **inline at `begin`** rather than beside the turn. `begin` calls the function `work` binds at once and answers a handle that already carries the result, so a `take` asked in the next expression answers `Ok(Some(r))` where the VM still answers `Ok(None)`. Everything else is the VM's, word for word: a second take is `Err("work: job already taken")`, a take after a cancel is `Err("work: job cancelled")`, and another kind's handle is `Err("work: this job was not started by job kind 'K'")`. `Wait.poll` watches sockets through the poll each target already has and reports every job key as ready at once. `[work] max-jobs` decides nothing there — at most one job is ever running — and a manifest that sets it for a wasm compile gets `warning[work-max-jobs-ignored]` at the program door, naming the key and the target. `--target wasip2` still refuses a *generated loop* for a reason that has nothing to do with jobs: the turn reads `Process.stopRequested`, which WASI 0.2 cannot bind. See "On wasm-gc and wasip2" under "Jobs" in `docs/services.md`.
