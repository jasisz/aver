...
- **An answer module says what it answers in its own header.** `answers [Wire]` replaces `answer = "Sockets"` in `aver.toml`. The module is part of the program like any other: some module lists it in `depends`, usually the entry. A module that answers a capability the compiler ships is refused with `error[answer-binding]`, as before.
- **An answer function returns `Tuple<S, Result<R, Run.Wake>>`.** `R` is the operation's own result. `Result.Ok(v)` answers now and `Result.Err(wake)` asks to be asked again. The `<Op>Reply` sums a capability used to declare beside each operation are gone, and so is `Then`: a module that knows its answer early keeps it in its state and answers it on the next ask. Rewrite `Cap.OpReply.Now(x)` as `Result.Ok(x)` and `Cap.OpReply.Later(w)` as `Result.Err(w)`, delete the reply sums from the capability modules, and change the answer functions' result types.
- **`Run.Wake` replaces `Wait.Wake`.** It lives in the standard module `Run`, which needs no `depends`, and has two constructors. `Until(items, deadline)` waits for any of a list of sockets and jobs, or for a deadline in milliseconds, whichever comes first: `Item(i)` becomes `Until([i], Option.None)`, `After(ms)` becomes `Until([], Option.Some(ms))`, `Either(i, ms)` becomes `Until([i], Option.Some(ms))`, and `NextTurn` becomes `Until([], Option.Some(0))`. `Settled(deadline)` is new: the request is asked again once the same answer module has answered anything other than `Settled`, which is how a process waits for another process to change the module's state instead of polling on a short deadline. `Wait` keeps `Item` and `poll`, so its `contract_hash` moves; recordings of `Wait.poll` made by an earlier release and a vendored copy of the JavaScript adapter move with it once.
- **A job is the answer to a request.** The answer module begins the job itself, keeps its handle in its state, and parks the request on it with `Result.Err(Run.Wake.Until([Wait.Item.Job(job)], Option.None))`; the ask after the job settles takes it. The `task`, `started` and `landed` keys, the job table of the generated loop, and the required `aStartedTaskIsNotAskedAgain` law are gone. The manifest keeps `work = "Module.function"`, `[work] max-jobs` and host packages.
- **`[run]` is gone.** The loop is generated into the entry module when it writes a process and no `main`, or when its `main` calls `Run.all()`, which runs the loop and answers `Result<Unit, String>`, so a program can keep a command line of its own and run the loop from one subcommand. The policies are optional functions of the entry, found by name: `stop(view: Run.View) -> Bool` and `admit(view: Run.View, id: Int) -> Bool`. `order` is gone: askable slots are served in slot order. The program no longer declares `View` and `Pending`; `Run.View` and `Run.Pending` are generated, and `Run.View` has `pending`, `ready`, `askable`, `dropped` and `stopping`. `error[view-shape]` is no longer emitted.
- **A process can run once per key.** `process peer seated by Sockets.peers` in the entry seats one `peer(key)` for every key the pure function `Sockets.peers(state) -> List<K>` of an answer module lists. The loop reads the list at every turn boundary: it seats new keys in list order, drops an instance whose key has left the list (counted in `Run.View.dropped`), and does not seat a key whose instance returned until the key has been absent once. A request for a key the module no longer knows is answered by the module, never parked.
- **The loop generates no laws into a program.** Its invariants are stated once as laws over the generated functions of `tests/fixtures/run_schedule_cases/` and checked there on the VM and the Lean wall, and the history fold is gone. The source-trace observers of the entry module's processes are generated only when one of its own laws cites them. The slice fixture now generates about 980 lines and 130 functions, where it generated about 2,900 lines and 307 functions.

### Added — ending a run with a reason

- **`Run.fail(message)` ends a run of the generated loop.** A process or an answer module calls it from wherever it finds the fault, however deep in its own helpers, and declares the effect `Run.fail`. The turn it is called in is finished first; then the loop stops every process the way a stop does, cancelling the jobs parked requests wait on, and `Run.all()` answers `Err(message)`. A run that ends without a failure still answers `Ok(Unit)`. Only the first call of a run counts: a second one in the same turn, from a slot served later, changes nothing. A program with no `main` exits non-zero with the reason on stderr, on the VM, in generated Rust, on wasm-gc and on wasip2. A program that wrote a failure to a file for its `main` to read back after `Run.all()` can call `Run.fail` instead.
- **The loop's reading of the reason is recorded.** `Run.fail` and the loop's `Run.failure()` reading after every turn are recorded like `Process.stopRequested`, so a recording of a failed run replays to the same `Err`, and a recording made on the VM and one made on wasm-gc replay on each other. `Run` is now a capability module shipped with the compiler; `Run.Wake` is unchanged. A program that never calls `Run.fail` from a process or an answer module generates the same loop as before.
- **On wasm-gc, `Run.fail` and `Run.failure` import `aver.run_fail` and `aver.run_failure`.** The module keeps the reason itself; the imports exist for the recorder. `tools/wasm-work/host.mjs` answers both, so a vendored copy of the JavaScript adapter has to be refreshed before it runs a program that calls `Run.fail`. The certificate format does not admit these two imports yet, so a module that calls `Run.fail` cannot be certified.

### Added — how long a turn waited and worked

- **`Run.lastTurn()` says how long the generated loop waited and worked.** Since the loop does the waiting, a process that timed its own calls saw no wait at all and counted the whole wait as work. `Run.lastTurn()` answers a `Run.Turn` with `turn`, which turn this is, `waitedMs`, how long the loop's wait before the current turn took, and `workedMs`, how long the turn before it worked, from the return of its wait to the start of this one, in whole milliseconds of a monotonic clock. A process or an answer module calls it from anywhere and declares the effect `Run.lastTurn`. All three are 0 while the loop seats the processes, `turn` is 1 in the first turn after a wait and one more in each turn after it, and `workedMs` is 0 in the first turn. Every answer served in one turn reads the same numbers, so `turn` is how a program counts each turn once. It works on the VM, in generated Rust, on wasm-gc, on wasip2 and under the JavaScript adapter.
- **Only a program that reads it pays for it.** The loop reads the clock twice per turn, right before and right after its wait, only when some function of the program declares `Run.lastTurn`. The two marks it calls for that are internal: `Run` does not expose them, and a program that calls or names one is refused at check time. Every other program generates the same loop, the same wasm-gc and wasip2 modules and the same Rust code as before, and its recordings have no new entries. What does move, once more in this release as it did for `Run.fail`, is the `Run` capability's contract hash, because the capability has more operations: a generated crate's provider table, a recording's header and a certificate's source model name it.
- **A program with no generated loop cannot read it.** A `main` that declares `Run.lastTurn` in a program that runs no loop is refused at check time: there is no turn to report.
- **The readings are recorded.** `Run.lastTurn` is recorded like `Time.unixMs`, so a replay answers the recorded numbers, and a recording made on the VM and one made on wasm-gc replay on each other.
- **On wasm-gc, a program that reads it imports `aver.run_wait_starts`, `aver.run_wait_ends` and `aver.run_last_turn`.** `tools/wasm-work/host.mjs` answers all three, so a vendored copy of the JavaScript adapter has to be refreshed before it runs such a program. On wasip2 the loop reads `wasi:clocks/monotonic-clock.now`. The certificate format does not admit these imports yet, so a module that reads `Run.lastTurn` cannot be certified.

### Fixed — a wait on wasm-gc that has only a deadline

- **`aver run --wasm-gc` now sleeps out a wait that has only a deadline.** A program with no job kind whose wait watched no socket answered at once, so the loop turned hundreds of times until the deadline passed, using a whole core. It now sleeps until the deadline, as the VM does.

### Removed — the Dafny proof backend

- **`aver proof` exports Lean only.** The Dafny backend is gone: the laws only Dafny used to prove are covered by the core Lean export, without Mathlib, so a second prover no longer adds coverage. `aver proof --backend dafny` fails with "the Dafny backend was removed; use Lean (the default)". Drop the flag, or pass `--backend lean`, which still works; check the export with `--check` / `--check-json` instead of `dafny verify`.
- **`--error-budget` is removed.** It only counted Dafny errors. Use `--sorry-budget` for open Lean proofs and `--declined-budget` for claims the exporter refused to state.
- **`--check-json` no longer has the Dafny-only keys** `errors`, `timeouts`, `axioms`, `axiom_budget`, `omitted` and `claims`. A Lean run never wrote them, so a Lean summary and `proof_manifest.json` are unchanged, and a `--gate` baseline keeps working.
- **The playground download menu has no Dafny export**, and the Docker image no longer installs Dafny.

### Migration — a job begun at the limit is queued, not refused

- **`begin` never refuses at `[work] max-jobs`.** A job begun while that many bodies run is queued and gets its handle at once; it starts, in the order it was begun, as soon as a running body stops, and until then `take` answers `Ok(None)` and a wait does not report it ready. `Err("work: job limit N reached")` is gone. The limit is now only how much of the host a program uses: the same program gives the same answers on a host with one core and on one with sixty-four, and a recording replays under any limit, including on the VM, which used to fail a replay whose recording had more jobs running at once than the replaying host allowed. `Work.cancel` of a queued job takes it out of the queue so it never starts; a running job that is cancelled keeps its place until its body stops. A program that branched on the refusal has to stop doing so, and the headroom advice for sizing `max-jobs` is gone with it. The VM, generated Rust, the wasm-gc native runner and Wasmtime packs, and the JavaScript adapter all queue; `wasip2` still runs a job inline at `begin`.
- **The generated loop no longer carries the job limit.** A program built without `max-jobs` used to write the building machine's core count into its generated source, so the same program meant something else, and certified differently, on another machine.
- **A stop request ends a wait.** Once a program has asked `Process.stopRequested`, a `Wait.poll` on the VM, in generated Rust and on the wasm-gc native host returns within about 100 ms of SIGINT or SIGTERM with whatever is ready, possibly nothing, instead of sleeping out its timeout; a generated loop then stops at its next turn even when a request is parked on a long deadline. `[]` from `Wait.poll` now means "the timeout passed, or a stop was requested".
- **A socket the runtime no longer knows is reported ready by `Wait.poll` and `Tcp.poll`.** A connection the program closed, or one dropped after an I/O error, used to make the whole wait answer `Err`, and a generated turn, which propagates that `Err`, ended the run on it. It is now reported ready, the way a forgotten job is, and the operation run on it next gives the real error. `wasip2` is unchanged.

### Added

- **`aver check` warns when a loop updates a Map or Vector that a record still holds** (`warning[perf-shared-update]`). `Infra.Utxo.absorbed(setting.window.created, …)` inside a step that also passes `setting` on reads as "`Infra.Utxo.absorbed` updates `setting.window.created`, a Map that is still held by `setting`; each call copies the whole Map". The update is `Map.set`, `Map.remove` or `Vector.set` on a field of a record, or a call, followed into the modules it names, that hands the field to one of them and returns the result. The record still holds it when it is read after the call or was already passed whole to the enclosing call. Only functions that run repeatedly are checked: recursive ones, the ones a recursive function of the module reaches, and answer modules. A local collection read again after its own update is not reported, since keeping both versions is usually the point.
...
- **The generated loop hands its own Maps on at the run's last use.** `__bump` (the answer versions), `__park` (the slot table), `__take<Module>` and the family seating read everything else of the run first and update the run last, so `Map.set` on `versions`, `slots` and a family's retired keys finds them held by nothing else instead of copying them once per answer on the VM.
...
- **A negative deadline in a generated loop is due at once.** It used to be read as "no deadline yet", so the turn's timeout depended on which slot held it.
- **A yielding helper of a dependency is never seated by the loop**, at any door.
- **`work` bindings are judged only at a module that runs.** `aver check`, `aver verify` and the backend doors pointed at a module with no `main`, written or generated, such as an answer module or a library of helpers, no longer ask for the binding of a job kind it uses or refuse a binding that names one of its own functions. The program that runs the jobs is where the binding is checked.
- **wasm-gc lowers a constructor whose payload is a list no function names**, such as `Run.Wake.Until` in a program that returns a `Run.Wake` but never builds one. It used to fail with `cannot lower type`.
- **A wait deletes its socket registrations before its poller goes.** On Windows a registration left behind could take the readiness a later wait was sleeping on.
- **One law proof that fails in Lean no longer fails the whole `aver proof` build.** A proof that ended in an error its `first | … | sorry` portfolio could not catch (unsolved goals, a failed `rewrite`, a heartbeat timeout) failed `lake build`, so no law in the program was audited. Each proof theorem is now emitted behind `#guard_msgs`, and a proof that fails is charged as a sorry: it counts in `sorries`, `sorry_laws` and `build_errors`, is listed in the new `--check-json` field `isolated_errors`, and its manifest tier is `failed`. The other laws keep their credit. Sample and checked-domain theorems still fail the build.
- **A dependency's law proved from a leaf entry unfolds that dependency's own functions.** Several strategies spelled them under the entry's namespace (`LawsEntry.reciprocalBound` instead of `Domain.Recip.reciprocalBound`), so the arm failed and the law fell to `sorry`.
- **More laws close in core Lean.** A literal bit mask (`Bits.and(x, 128)`, `Bits.and(v, 4259839)`) against its `Int.div` / `Int.mod` form, a literal `Int` match against range predicates, a guarded claim that rests on the sign of a product or a square, and a guarded claim over a record and helpers that branch. Five bit lemmas ship in the `AverBits` prelude for the masks. No Mathlib.

### Migration — behaviour change for stubs that read their call index

- **The call index a `given` stub receives now counts the calls of its own operation.** The second argument of a generative stub used to count every effect the case reached, so a `Time.unixMs()` or a `Tcp.writeNow` between two `Random.int` reads pushed the second read from index 1 to index 2 and a stub written as `match call` answered the wrong line. Each operation now numbers its own calls from 0 on each branch path, in `aver verify` and in the Lean and Dafny that `aver proof` exports, so a scripted stub is a reply script for one operation and adding a clock read or a log line to the code under test does not rewrite it. Request operations of a generated coordinator follow the same rule: each request kind counts its own requests, in-place effects do not move them, and a self yield still charges nothing. A stub that ignores its index is unaffected, and so are recording, replay and `.trace` assertions. Any law whose stub branches on its index has to be rebased, and the value the law asserts can move with it: two laws in one yield fixture of this repository did, and a second fixture rebased the reply script of its stub. A single-operation law is not automatically safe, because the `?` rewrite used to spend an index on the error binding it generates and no longer does, so a law over one operation that uses `?` moves its exported index too. The hostile and honest profiles shipped with the standard capabilities keep their bodies, and the answers of the ones that read their index move: `Time.unixMsNormal` returns the base epoch on a program's first clock read whatever ran before it, and `Tcp.acceptOnceThenNothing` returns the peer on the first `Tcp.accept`, so an `aver verify --hostile` outcome can change for a law nobody touched. See "Stub signatures" in `docs/oracle.md`, which also gains a worked pattern for driving a socket state machine with a scripted peer.

- **`aver proof` declines a law it cannot number the way a run numbers it, instead of exporting a theorem about a different function.** An exported proof numbers the calls of one function body at a time, and five shapes leave it unable to follow a run: a call into an effectful function, a recursive call, a second operation inside a `Process.stopRequested` poll loop, a call after a `match` whose arms call that operation a different number of times, and a claim that reaches one operation through more than one effectful call. A law is checked on samples and proved for every input, so a law over such a function could pass `aver verify`, prove in the kernel and still be false of the run — a false certificate reached with every step passing. Each such law is now declined with a reason naming the call and the operation, in the report and in the emitted Lean or Dafny, and counted among the declined claims, so nothing is proved about it. `aver verify` is unchanged, and so are sampled `verify` cases: a case is one concrete evaluation the run has already checked, so the exported model either agrees with it or fails the proof visibly. The last shape is the quietest of the five, because it needs no helper and no recursion: `readOne() => readOneToo()` is two ordinary calls of two exact functions, and a run hands the peer index 0 on the left and index 1 on the right while the export writes 0 for both. A poll loop reaching no second operation stays exact and keeps its theorem, as do `match` arms that call an operation equally often, two calls in one claim that reach no operation in common, and the `because` lines of a claim, which a run never evaluates. See "Where a law is declined instead of exported" in `docs/oracle.md`.

- **Each arm of a `match` numbers its calls from the point the match was reached.** The exported proof carried one running index across the arms while a run charges only the arm it takes, so a function whose two arms read the same operation exported index 0 in the first arm and index 1 in the second, and the theorem for a case reaching the second arm asserted a value the model does not produce. Every arm now starts from the indices claimed before the match. A call after the match is numbered from the busiest arm, which is exact while the arms make the same number of calls, and a law over a function whose arms call that operation a different number of times is declined rather than exported at the busiest arm's index. `aver verify` is unchanged; a law over such a function exports different index literals than before. An arm beside a nested `match` with uneven arms is not a call after it, and keeps its theorem.
...
- **Writing a law about a function no longer changes that function's Lean definition, and a proof that stops closing can be compared against an earlier run.** The Lean export used to emit a function's `match` in a different form as soon as a law was stated about it, so adding a law elsewhere could make an unrelated proof that opens the function time out with its script unchanged. Now only a recursive function keeps that form, for its own termination proof; every other definition, and every law statement, is emitted the same way whether or not a law mentions it. To tell a broken proof from a changed model, `proof_manifest.json` now carries a hash per emitted definition and per law script, and `aver proof --check --compare-manifest <earlier proof_manifest.json>` reports, for each law that did not close, whether its own script changed and which definitions it opens changed since that manifest (`changed` in `--check-json`). See "Checking an export" in `docs/lean.md`.
- **One wait is keyed by whatever the program keys its map by, and a program that waits needs no job kind to run under an external host.** `Wait.poll` takes a `Map<K, Wait.Item>` for any key a map accepts and answers `Result<List<K>, String>` with the ready keys, in the order that map puts its keys in. `Int` is one such key, so every program written before this compiles and behaves exactly as it did. What the key buys is the program that waits on several kinds of thing at once: peers, the writes it still owes them, a dashboard client, a background job. Keyed by whole numbers those owners have to agree on an arithmetic convention that lives in comments, and two of them meeting delivers one peer's readiness to another with nothing to say so; keyed by a type of the program's own, a collision is a duplicate constructor and a ready key is matched on and handed down rather than decoded back out of an integer. The key of a wait is the key type the checker inferred for the map that wait was handed, so a wait set written at the call, bound to a name, or built by another function and passed in all key the same way and none of them has to be annotated. One program uses one wait key type: a program that keys one wait one way and another wait another way is refused with the repair, and so is a wait whose key type nothing in the program settles, which would otherwise be a key each backend named differently. `aver check` reads one file at a time, so both refusals are stated again at every `aver compile` door, which reads the entry module and the modules it depends on together; a program whose modules disagree about the key is refused there rather than compiled against one of them. A wait set written empty at the call holds nothing, so it names no key of its own and is read as keyed by `Int`; in a program that keys its waits by a type of its own, that second key is refused by name, and the repair is to bind the empty set to a name that writes the type down, `idle: Map<Watch, Wait.Item> = {}`, and hand that name to `Wait.poll`. `--target wasip2` carries the wait through its canonical ABI imports and keys it by `Int` there, so a wait keyed by another type is refused for that target by name, and a component carries none of the wait ABI helpers, exports or decimal bridges an external host needs. A key obeys every rule a map key obeys, so a `Float`, a `Map`, a `Vector`, a record with a `Float` field and a capability resource are all refused. Alongside it: a program that uses `Wait.poll` and binds no job kind at all now carries the wait half of the `aver:work/v1` descriptor and the ABI helpers that go with it, and the supplied JavaScript adapter accepts a module that declares no job kind and starts no worker for one — before, such a module carried no descriptor, `createWorkHost` refused it outright, and there was no way for a host to decode its wait set. The adapter reads the wait's type names out of the descriptor instead of assuming whole numbers. Recording: a whole-number key records as a number exactly as before, and a key of the program's own type records the way every other value of that type records on the bytecode VM and under `--target rust`; on wasm-gc the host is handed its keys as bare references it cannot read, so recording a wait keyed by anything but `Int` is refused there by name rather than written down as a guess. One thing does change for a recording made by an earlier release: on wasm-gc a wait records its set in the order the map puts its keys in, where before it recorded that set in the order the map's buckets happened to sit in, so a wasm-gc recording of a `Wait.poll` from an earlier release can mismatch on that argument and has to be made again. `Tcp.poll` already recorded in key order and is unchanged. This moves the certificate wall identity for a wasm-gc program that waits: `aver.wait_poll` changes shape — its wait-set parameter and its answer are now named from the program's key type rather than fixed at `Int` — the module gains the wait's own list and result factories, a bucket-key reader and a key-order export, and it registers the two decimal `Int` bridges an external host moves a full-ℤ key through, which shifts every function index after them. As with the earlier rotations, existing packages verify only with their paired release, or after regeneration with this one. See "Jobs" in `docs/services.md` and `stdlib/capabilities/wait.av`.
- **A job kind may name the data types of the modules it depends on.** A capability of Work shape can write `operation begin(task: Ledger.Request) -> Result<Work.Job, String>` instead of declaring a second `Request` field for field and converting between the two in both directions. It is the only capability that may: a job is answered by a function of the same program, so the job kind and the module whose type it names ship together, while every other capability stays closed on its own declarations and naming another module's type there is still refused. The module has to be in `depends` and has to expose the type, the same two gates an ordinary fn passes when it names another module's type, so a record its author kept back never ends up inside a published identity. The layouts of the named types, and of every type they reach, are inside that job kind's `contract_hash`, so adding a field to one invalidates earlier recordings and makes a deployment pack need rebuilding, exactly as adding a field to a record declared in the job kind's own module does; `aver capabilities` prints the one hash as before. Each reached layout is bound under the name of the module that declares it, whether the module holding the field wrote that name out or left it bare, so it crosses the boundary under that name on the VM and on wasm-gc alike. A named type that holds a capability resource anywhere inside it is refused at compile time, and the refusal names the field and the type; a name the owning module declares as a `resource` is refused as a resource. A Wasmtime pack carries the named modules beside the job kind's own contract source so its host recomputes the same hash without the project tree. See "Jobs" in `docs/services.md`.

- **The documentation shows the bounded outbox a non-blocking write needs.** Beside `Tcp.writeNow` it describes the outbox a program carries for itself: a bounded queue per connection with an exact partial-write cursor, the `Sending` key registered only while bytes remain, overflow charged to the slow peer, and a bulk producer that renders its next payload once the queue is under a watermark instead of enqueueing a whole answer at once, so a healthy peer is not dropped for reading slower than the program produces. See `Tcp.writeNow` in `docs/services.md`, and "Common patterns" in the language guide, whose sketch `llms.txt` and `aver agent-connect` carry as well.
...
- **The program declares the reply sum of every operation it answers; `Cap.__<Op>Reply` is gone.** Pre-1.0, and breaking for every program that answers a capability of its own. An answer module still answers `tick` with `fn tick(state: S) -> Tuple<S, Clock.TickReply>` and `Clock.TickReply.Now(v)` / `.Later(wake)`, but the sum is now yours: declare it in the capability module beside the operation, named after the operation with its first letter upper-cased, with exactly `Now` carrying the operation's result type and `Later` carrying `Wait.Wake`, and expose it like any other type — `type TickReply` / `Now(Clock.Tick)` / `Later(Wait.Wake)` beside `operation tick() -> Clock.Tick`. Naming `Wait.Wake` needs no `depends [Wait]`. The compiler generates nothing into a capability module any more, on any door or backend: a program that still writes `Clock.__TickReply` gets an ordinary unknown-type error, and a program that answers a capability without the declaration, or with one of another shape — a wrong payload, a record of that name, a missing or extra constructor — gets `error[answer-shape]` at `check`, `run` and `verify`, with the declaration to paste printed in the module's own names. A capability nothing answers is not held to it. The rule behind the change is the one that stands everywhere else in Aver: a program never writes a `__` name, and one declared sum per operation is what "no user generics" looks like here. See "Capabilities the program answers" in `docs/services.md` and "The coordinator" in `docs/language.md`.

### Added

- **`aver effects` reports declared effect lists against the minimum the checker computes, and rewrites them on request.** The report names, per module, every function whose `! [...]` is missing or carries an unused entry, and does the same for the module's `effects [...]` boundary; modules with nothing to say get one summary line. `--write` rewrites exactly the lists the report names to the computed minimum across the whole module graph, so changing which primitive a leaf calls no longer means editing every function above it by hand; the rewrite goes through the formatter's layout, is idempotent, and leaves a tree the report calls minimal byte-identical. `yield` and the callback-forwarding marker `_` are copied verbatim, a namespace entry such as `Disk` keeps the granularity its author chose even when a method is written beside it, a `//` comment written on a list is put back on the rewritten one, and a module without a boundary does not get one. A module the input did not name is called out as reached as a dependency, because another program can import it and the walk did not read that program. `--since <rev>` compares the effect surface at a git revision with the working tree, separates the functions whose list moved with an unchanged body from the ones whose own body changed, and names the functions the revision did not have and the ones it had and the working tree does not. `--json` emits the versioned deterministic envelope. Granularity stays `Namespace.method`; nothing here lets a list be omitted, named or abbreviated. See `docs/cli.md`.

- **Yield lowering has independent source/request-trace observers.** Compiler fixtures compare retained direct-style source with actual generated protocols using typed answers, dynamic positions and internal resumptions. Ordinary Aver laws check arbitrary finite prefixes, including local in-place effects and finite helper composition; corruption controls preserve return values while changing traces. Recursive imported helper splice invariants and imported in-place-effect composition remain open. See `docs/yield-request-traces.md`.
...
- **Process cases run through their lowered protocol with request stubs.** A local `verify process` accepts an exact `given` for each request operation and exercises nested helpers, self yields, `Unit` replies and error propagation on the VM. Oracle counters reset per case and include in-place effects; budgets and diagnostics retain the source name. Verification drivers add no live provider requirement and are omitted from Rust/WASM artifacts. Direct process laws, trace cases, WASM request stubs and proof export remain unsupported. Deterministic coordinator scenarios cover reversed order, grouping, premature readiness and stale completion/cancellation notifications.

- **The agent guides teach the coordinator, answer modules and jobs.** The language guide's "Yielding functions" section is now "Processes, answer modules and the coordinator": a process, an answer module with `Now` and `Later` and the four `Wait.Wake` shapes, a job kind with its `work`/`task`/`started`/`landed` binding, and the `[run]` table with the `Pending` sum, the `View` record and the three policies, each shown on the smallest program that checks and runs, with the hand-written coordinator kept as the door for a program without `[run]`. Two rules stand beside the rest: a loop that must not starve the others declares `yield`, and the declaration fixes where control is handed back without bounding how long one step takes; "atomic between two yields" means no other step runs in between, not atomicity of external effects and not a time bound. The toolchain guide gains the manifest keys with one example `aver.toml` and the diagnostics an agent meets on the way (`yield-*`, `intercept-outside-yield`, `answer-*`, `run-binding`, `view-shape`, `work-*`, `serve-path`, `turn-budget`), each with its recipe. `llms.txt`, `llms-full.txt`, `docs/cli.md` and `aver agent-connect` carry the same text. The program the blocks are cut from is `tests/fixtures/run_guide_example/`, and a test pins every block to it, so a guide block that stops checking fails CI.

- **The job seam has a third end, `started`, and the generated coordinator lifts its two limits.** A `work` binding that carries a seam now declares `task`, `started` and `landed` together — all three or none, all three pure functions of modules bound with `answer`: `task` is `(S) -> Option<T>`, where the turn takes its next task from; `started` is `(S, T) -> S`, called with the offered task the moment `begin` has answered `Ok(job)` for it; and `landed` is unchanged. `task` and `started` must name functions of one module, because the state one offers from is the state the other writes back to. The order of one start is ask, `begin`, `started`: a `begin` that answers `Err` records nothing, so the task stays in the answer state to be offered again. Because the start is recorded, `[work] max-jobs` may be any positive number — each slot of room starts a different task — and any number of job kinds may run under one generated coordinator over one shared table (`Map<Int, __Job>`, one variant per kind) and one shared limit. A `work` binding missing `started` is `error[work-binding]`, which prints the signature the seam wants, and a `started` of the wrong shape is the same slug. Two new generated laws pin the seam down: `aStartedTaskIsNotAskedAgain` per kind cites the program's own law of that name on its `started` function, and `aFullTableAsksNothing` per kind says a table with no room offers nothing. That first law is a requirement on the program: every job kind's `started` function states `verify <started> law aStartedTaskIsNotAskedAgain` — when `task(state)` offers a task, `task(started(state, task))` does not offer it again — and a `[run]` program whose `started` function has no law by that name is `error[run-binding]`, which prints the block to write. See "Capabilities the program answers" in `docs/services.md` and "The coordinator" in `docs/language.md`.

- **Jobs and the one wait run on both wasm targets.** wasm-gc delegates jobs to its host through `aver:work/v1`, with parallel execution in the native runner, Wasmtime packs, and the supplied JavaScript Worker adapter. WASI 0.2 keeps inline `begin`, immediate job readiness, and `warning[work-max-jobs-ignored]`. Both preserve typed handles, take-once and cancellation errors. Generated coordinators run on both targets; WASI 0.2 uses its stop policy without a signal subscription. See `docs/wasm-work.md`.

- **A recording of a job crosses backends in both directions.** `aver run --wasm-gc --record` writes `begin` with its task and the handle it minted, `take` with the answer it gave, `Work.cancel`, and `Wait.poll` with the keys it reported — the shape the VM records — so a recording made on wasm-gc replays on the VM and a VM recording replays on wasm-gc, effect for effect. Replay serves the recorded answers in the recorded turns, so a wait that reported one job on the VM reports one job on wasm-gc. `--record` stays refused on wasip2: a component records nothing. Admitting the four new host imports (`aver.wait_poll`, `aver.work_cancel`, `aver.work_begin`, `aver.work_take`) rotates the certificate wall identity, as the earlier rotations did: existing packages verify only with their paired release, or after regeneration with this one.

- **Two wasm-gc lowering gaps closed on the way.** A `Result.Ok(...)` whose payload is an effect that answers `Unit` — `Result.Ok(Console.print(line))`, the ordinary shape of a function whose answer wraps the effect it just performed — used to compile to a module that never performed the effect. And a sum reached through its qualified spelling (`Wait.Item`) found no equality or hash helper when it was a `Map` value, which stopped the module rather than emitting one.

- **A capability your program answers runs on wasm-gc and wasip2 too.** `aver run --wasm-gc`, `aver run --wasip2`, `aver verify --wasm-gc`, `aver compile --target wasm-gc` and `--target wasip2` no longer refuse a program that answers a capability of its own with `answer = "Module"`. The reason they used to is gone: the reply sums the compiler generates carry `Wait.Wake`, which reaches the job handle `Work.Job`, and the wasm backends now represent it — a handle is a compiler-owned resource like `Tcp.Dial`, two handles are the same job exactly when they are the same reference, and a sum that carries one compares and hashes like any other. Two smaller gaps closed with it: a variant field of type `Unit`, which is what an operation returning `Unit` is answered with, and `Result<Vector<T>, String>` behind a `?`. The target-specific scheduling of job kinds is described above. See "On wasm-gc and wasip2" under "Jobs" in `docs/services.md`.

- **A request can wait on a socket and a deadline at once.** `Wait.Wake` gains a fourth constructor, `Either(Wait.Item, Int)`. A module answering a capability parks on it to say "wake me when this socket or job is ready, and in any case not later than this many milliseconds from now", and the request is asked again on whichever comes first. The item goes into the turn's wait set exactly as `Item` puts it there and the deadline counts in the poll timeout exactly as `After` counts it, backwards clock and all — a reading that has fallen further back than the `ms` asked for makes the request askable rather than stranding it. Nothing tells the answer function which half woke it: a module that needs to know probes the socket again (`Tcp.readNow` answering `None` is "still nothing") or reads the clock in place, and `op(state, args)` is unchanged. Two more generated laws say the deadline half of the new gate — askable once the deadline has passed, and a wait no longer than the deadline asked for — and both close on the Lean wall, where the coordinator example now stands at thirty universal laws with nothing bounded and no `sorry`.

- **The coordinator example's reads honour their deadline, and the example ends on its own.** `Wire.read` always took a deadline and the example never used it; now `Sockets.read` records the clock reading a read falls due at on that read's first ask, parks on the peer's socket and on what is left of that deadline at once, and answers `Wire.Heard.TimedOut` on the ask that finds nothing once the deadline has run out — so the `TimedOut` arm the peer process always carried is reachable, and a peer that goes quiet is handed back instead of being waited on for ever. The same example no longer needs anyone on the other end to finish: its pool stops handing out work after a bounded number of idle asks, its chain ends where it stands once the pool is done, and its stop policy ends the run once the listener is the only process still seated (a listener never gives up on its own: a client that arrives late is served if there is work, and the run is over if there is none), so `aver run main.av --module-root . -- <port>` prints its summary and exits whether or not anyone connects.

- **A process can be split into named helpers.** A function whose effect list names `yield` may now call another yielding function of the same module, and the compiler puts the callee's machine inside the caller's instead of asking you to fold the two together by hand. A tail call to a helper stops with a `Yield` request carrying what the helper is entered with, and answering it calls the helper's own `__gStart`. A non-tail call nests the helper: the caller's state sum gains one variant per call site, `In<G>At<N>(<the helper's own state>, <the caller's live variables>)`, the caller's request sum gains the kinds the helper waits on, and the caller's answer function hands the answer down and routes what comes back — a result continues the caller's segment, another request leaves again with the new nested state. A nested call is not a stop by itself, so a helper that waits on nothing runs to its result in place. A coordinator, generated or hand-written, is unchanged: it seats and serves the caller's protocol, and the helper's requests reach it as the caller's. What is still refused, each with the construct named: mutual nesting between two yielding functions or a cycle written with tail calls only (the message prints the cycle either way), a request or a helper call inside an independent product `(a, b)!`, a yielding function calling itself outside tail position, and a yielding function of another module — call `Module.__fStart(...)` and answer its requests instead. The rule that an operation a process has to continue after must not answer `Unit` is gone with the reason for it. See "Helpers and nested state" in `docs/language.md`.

- **A parked request is not asked again until its wake fires.** In a program whose loop is generated, `Wait.Wake` no longer only decides how long the turn sleeps — it decides when the request is asked again. A request parked on `Item` is asked in a turn whose `Wait.poll` reported its key, and false readiness is still allowed. A request parked on `After(ms)` is asked once the turn's clock reading has reached the deadline that park set, `now + ms`; the turn reads `Time.unixMs` once per turn, after its one wait has returned and before it serves, so a deadline that fell due while the turn was asleep is asked in that same turn rather than the next one, one reading gates every slot and measures the next turn's wait, and a recording replays it. A slot also keeps the `ms` its `After` asked for, so a wall clock that steps backwards can neither strand a request behind a deadline it will never reach — a reading that has fallen further back than `ms` makes that request askable — nor put the turn to sleep for longer than the request asked for. A request parked on `NextTurn`, a freshly seated process and a process whose request was just answered are askable at once. The view the three policies read gains one field the loop fills, `askable: List<Int>` — the ids this turn may ask, in slot order — so a view that does not declare it is `error[view-shape]`, which prints the declaration the loop wants; `order` and `admit` still see the whole view, and the turn asks `admit` only about askable ids. The entry module's widened `effects [...]` gains `Time.unixMs`. Three more generated laws say the two halves of the deadline gate and the clamped wait, and all three close on the Lean wall. See "The coordinator" in `docs/language.md`.

- **The worked example of a partial write in `docs/language.md` is a real socket.** The coordinator example's `Wire` is answered by a module that owns a `Tcp.Listener` and the `Tcp.Connection` behind each peer key: it accepts from `Tcp.accept`, reads with `Tcp.readNow` and parks on `Connected` when nothing has arrived, writes with `Tcp.writeNow` and parks on `Sending` with the offset it got to in its own state, which is what a `Later` keeping the module's state is for. An answer module that performs effects is allowed and `warning[answer-shape]` says so, because an answer runs inside the turn and a slow one stalls every other process.

- **A `Later` keeps the state the answer module returned.** An answer function returns `Tuple<S, Cap.OpReply>`, and the generated coordinator used to keep that `S` on `Now` and throw it away on `Later`. It keeps it either way now, so a `Later` is where a module records its own progress — a partial write's offset, a retry count, a deadline of its own — and what a `Later` leaves alone is the *request*: the same instance number, the same request value, parked on the wake it named. The generated laws follow: a `Later` keeps the instance and keeps the request, and the per-module "a `Later` leaves this state alone" laws are gone, because they are no longer true. The reply sum itself is not touched by this, so no answer function needs rewriting for it; a module that was relying on its `Later` state being dropped now sees it kept.

- **A job that will never finish reaches the answer state as an error.** `landed = "Module.function"` now names `fn landed(state: S, outcome: Result<R, String>) -> S`. The generated turn hands it `Result.Ok(r)` for a job that finished, keeps the handle untouched for a job reported ready that has not finished, and hands it `Result.Err(reason)` for a job that was cancelled, whose body stopped, or whose id the job engine has forgotten — dropping that handle, because such a job has no second answer to give. A take that fails no longer stops the turn and ends the run: what a lost job means is the program's own business, written in `landed`. A `landed` of the old shape is `error[work-binding]`, which prints the signature the seam wants.

- **Jobs, the one wait and a generated coordinator run as a native binary.** `aver compile --target rust` now accepts a program that declares a job kind, answers a capability with `answer = "Module"`, or asks for its loop with a `[run]` table, and the binary it builds behaves as the VM does on the same program. `begin` starts the bound function on a thread of the job engine in `aver-rt`, `take` gives the same four answers in the same words, `Wait.poll` watches sockets through the same reactor and jobs through the same engine, and `[work] max-jobs` reaches the binary from `aver.toml` when it is compiled — a manifest that names no limit leaves the running host's own parallelism. Two differences from the VM: `Work.cancel` drops the job's answer but the thread runs to completion, because generated Rust carries no cancellation check, so cancelling detaches the work rather than stopping it — and a detached job keeps its slot, so it counts against `[work] max-jobs` until its body ends, which the VM's cancelled job does not; and replaying a recording serves the recorded answers without recomputing the job beside them, so the divergence check stays a VM check for now. A recording crosses in both directions: a recording made on the VM replays on the built binary, and a recording the binary makes replays on the VM, because the binary writes the module root it was compiled against — made absolute at compile time — and the program file relative to it into the recording's header. `--target wasm-gc` and `--target wasip2` still refuse a program with a job kind or a generated loop with `error[work-target]`. See "On the Rust backend" under "Jobs" in `docs/services.md`.

- **A running job is a capability shape the program declares and `aver.toml` binds.** Two reserved stdlib contracts arrive: `Work`, which owns the `Work.Job` handle and `Work.cancel`, and `Wait`, whose `Wait.poll(items: Map<Int, Wait.Item>, timeoutMs: Int)` is one wait over sockets and jobs together (`Wait.Item = Socket(Tcp.Socket) | Job(Work.Job)`). A *job kind* is a capability of your own that `depends [Work]` and declares exactly `begin(task: T) -> Result<Work.Job, String>` and `take(job: Work.Job) -> Result<Option<R>, String>`; anything else under that shape is `error[work-shape]`. Who runs the job is a manifest binding, `work = "Node.validate"` beside `capability = "Validation"`, naming a pure function of the same program whose parameter and result are exactly `T` and `R` — mutually exclusive with the Rust-package binding, and required: an unbound job kind is `error[work-binding]` at the program door of `aver run`, `aver check` and `aver verify`. The bytecode VM and `aver compile --target rust` both answer a job; the wasm-gc and wasip2 backends refuse such a program with `error[work-target]`. See "Jobs — `Work` and `Wait`" in `docs/services.md`.

- **Jobs run.** On the bytecode VM, `begin` starts the bound function on its own thread and hands back the handle at once, `take` answers `Ok(None)` while the job runs and `Ok(Some(result))` once it finished, `Work.cancel` stops it at the next cancellation check, and `Wait.poll` returns as soon as a socket is ready, a job settles, or the timeout elapses — a finished job wakes a waiting turn instead of letting it sit out the wait. `begin` never blocks the turn: at the limit it answers `Err("work: job limit N reached")`. A second take answers `Err("work: job already taken")` and taking a cancelled job answers `Err("work: job cancelled")`. A handle belongs to the job kind that started it: `Work.Job` is one type, so another kind's `take` type-checks, and the runtime refuses it with `Err("work: this job was not started by job kind 'K'")`. A new `[work] max-jobs` in `aver.toml` bounds how many jobs run at once; without it a program gets the host's available parallelism, and a limit below one is refused when the manifest is read. Jobs still running when the program ends are cancelled, and the runtime waits for them only briefly.

- **A recorded turn of jobs replays to the same turn, and a job that answers differently stops the replay.** `aver run --record` records `begin` with its task and the handle it minted, `take` with the answer it gave, and `Wait.poll` with the keys it reported. `aver replay` gives the program those answers back in the turns they were recorded in, and runs the bound function again beside them: a job is pure, so recomputing it is the check. When a recorded `take` said `Some(v)` and the recomputation produces something else, replay stops with a divergence naming the job kind, the job and both values.

- **A capability your own program answers.** Put `answer = "<Module>"` on a capability's `[[providers.bindings]]` entry in `aver.toml` and that capability is no longer performed: every call to one of its operations is a request a `yield` process hands back, and the named module computes the answer from one state. The capability module declares one reply sum per operation beside it — `type ClaimReply` with `Now(Pool.Assignment)` and `Later(Wait.Wake)` — and an answer function is written `fn claim(state: State, key: Int) -> Tuple<State, Pool.ClaimReply>`. `Wait.Wake` says what a `Later` is waiting for: `Item(Wait.Item)` for a socket or a job, `After(ms)` for a deadline, `NextTurn` to be asked again. Only a capability your program declares may be answered; `Console`, `Disk`, `Tcp` and the rest of the standard set are refused with `error[answer-binding]`. An operation of an answered capability is a request, so calling one outside a `yield` function is `error[intercept-outside-yield]`, which names the answer function to call instead. Every backend runs such a program — the bytecode VM, `aver compile --target rust`, `aver run --wasm-gc` and `aver run --wasip2`.

- **The coordinator is generated: write the processes, the answer modules and three policies, and nothing else.** Put a `[run]` table in `aver.toml` — `order`, `admit`, `stop` and `view`, all four naming things in the entry module — and the compiler writes the loop for you, into that module, in the `__` namespace: the slot table, one instance number per request, the seating of one of every process that module writes, the wait set, the poll timeout, the dispatch through the answer modules, the job seam, the stop observation, the turn, `__runAll`, `main`, and a shutdown that cancels every job still running rather than abandoning its handle. A `Later` parks its request where it stands, keeps the state the module returned, and names the wake that decides when the request is asked again; a `Wait.Wake.After(ms)` also bounds the turn's poll and a `NextTurn` makes that turn poll with zero. The view the three policies read is a record your program declares and the loop fills — `pending: Map<Int, Pending>`, `ready`, `askable`, `jobs`, `room`, `stopping`, with one `Pending` constructor per process carrying the instance it waits on and its wake — so a policy's law can sample it; a view that is not that shape is `error[view-shape]`, which prints the declaration it wants. The loop also emits its own invariants as `verify` laws over the generated functions, so you do not write those either. Each generated function carries its own effects rather than the program's, so a process that touches nothing keeps everything the loop generates for it pure; the entry module's `effects [...]` is widened to admit what the turn performs. Requirements the loop puts on your program are `error[run-binding]`: every process is written in that same module, takes no parameters and answers `Unit`; each answer module declares a pure `fresh()` and a name that does not collide with another answer module or with a field of the run table; the policies are pure; the module does not write its own `main`; and a program with a job kind keeps `[work] max-jobs = 1`, because the turn takes its next task from a state that starting a job does not change. `AVER_YIELD_DUMP=1` prints the whole generated loop after the protocol. The worked example is `tests/fixtures/run_all_slice/` — five processes, three answer modules, one job kind — with `examples/concurrency/README.md` pointing at it; see "The coordinator" in `docs/language.md` and the `answer` / `task` / `landed` / `[run]` keys in `docs/services.md`.

- **`yield` functions lower to plain data and pure answer functions.** A function whose effect list names `yield` is cut at every call to a capability this program answers itself and at every self tail call: the compiler generates one state sum per request kind (`__LoopClaimState`), a request sum (`__LoopRequest`), an outcome sum (`__LoopOutcome = Done | Waiting`), `__loopStart` and one `__loopAnswer<Kind>` per kind in the same module, removes the original function, and a coordinator you write answers the requests. The generated items are ordinary types and functions: `run`, `verify` and both proof exporters see them. The wasm backends do not yet: a stop is a call to a capability the program answers, and a program that answers a capability runs on the bytecode VM and on the Rust backend, as the entry above says. Calling the original directly is a type error with the recipe; a non-tail call between yielding functions is rejected in this phase. An operation nobody answers is not a stop: it runs inside the turn, where you wrote it, and the generated function holding it declares it in its own effect list. See "Yielding functions" in `docs/language.md`.

- **A module that exposes a yielding function exposes its protocol in its place.** `exposes [loop]` becomes the generated names, so a dependent module writes `Looper.__loopStart(...)`, matches `Looper.__LoopOutcome` and answers with `Looper.__loopAnswerClaim(...)`; a plain `Looper.loop(...)` is the same type error with the qualified recipe.

- **`examples/formal/knowledge.av` states knowledge that only grows as laws on a content-keyed merge.** An admission predicate kept apart from the merge, a merge with commutativity, idempotence and an append law over `mergeAll`, and stable-answer laws for the body and verdict queries, all under an explicit agreement premise that two plain verify cases show to be load-bearing. The kernel closes the append law and both stable-answer laws, each reason and its final step, and the step from the order-freedom reason to its claim; commutativity and idempotence are checked on the listed samples only. One obligation stays open in Lean: the order-freedom reason, because the map model states no order laws for its keys, so two sets under different keys do not commute as lists.

- **`aver check` warns when a `Tcp.poll` loop hands one of its turns to an effectful loop.** `warning[serve-path]` fires on the call from a function that polls into a loop that reads from `Disk` or `Tcp` on every step (`Disk.read*`, `Disk.size`, `Tcp.read*`, `Tcp.accept`, `Tcp.dialled`, `Tcp.peerAddress`), reached without going back through the poller or through another function that polls, and still recursive with the poller taken out of the call graph: that work runs to completion before the next wait, so peers that became ready in the meantime are not served until it returns. A loop that walks a list it was handed, one element per step, is the server shape and is not reported; neither is a loop that only writes what it already holds. One warning per (poller, loop) pair, suppressible with `[[check.suppress]]` and a reason.

- **`aver verify` can measure how long one turn of a case runs without waiting.** `[verify] turn-budget = N` in `aver.toml` (off by default) counts VM steps since the case's last `Tcp.poll`; a turn that crosses N is reported once per case as `warning[turn-budget]`, naming the case by its index and the innermost function at the limit. A warning, never a failure; only VM steps count, time inside a provider is not measured, the wasm-gc lane does not measure turns, and with the budget off nothing is counted.
...
- **`Tcp.Socket.Sending(connection)` polls a connection for write readiness.** `Tcp.poll` reports a `Sending` key when the next `writeNow` will accept at least one byte or fail; `Connected` keeps meaning read readiness, and a caller who wants both directions registers the same connection under two keys. Every exhaustive `match` on `Tcp.Socket` needs a new `Tcp.Socket.Sending(connection)` arm. Admitting the new host imports rotates the certificate wall identity, as the two earlier rotations did: existing packages verify only with their paired release, or after regeneration with this one.
...
- **Laws can explain their argument in Aver.** Optional ordered `because` expressions supply checked Boolean facts, including ordinary functions with `match` branches; `using [function.law, ...]` selects the available lemmas. Declared and hostile verification execute the explanations under the original guard. Lean checks each reason and the final implication separately, while retaining every obligation in the original law's axiom audit. Proof reports expose these steps without counting them as extra source laws. Direct recursive explanations with checked list descent also guide induction, preserving guards and earlier reasons at recursive calls. List descent takes precedence over a sibling counter fuel model for ordinary functions too. Dafny explicitly declines annotated laws. See [#1288](https://github.com/jasisz/aver/issues/1288).

- **A wasm-gc program can now carry its own native Wasmtime host and start from precompiled code.** `aver compile app.av --target wasm-gc --pack wasmtime -o out/` emits the canonical module, an optional distinct Binaryen result, a matching AOT-compiled `.cwasm`, a manifest binding the complete chain, and a stripped `aver-wasmtime-host` executable with Wasmtime and every custom Rust provider selected by `aver.toml` linked in. The destination runs that executable without Aver, Cargo, a Wasmtime installation, provider source, project tree, or first-start Cranelift compilation. `--certify --optimize` is now an honest three-artifact pipeline: the certificate binds `app.wasm`, Binaryen writes the explicitly unproved `app.optimized.wasm`, and Wasmtime executes `app.cwasm` derived from the latter. Before native-image deserialization the host checks every artifact hash, the precompiled envelope and engine fingerprint; before instantiation it also checks import names and signatures, capability contract/model hashes, linked provider provenance, and the build project's runtime effect policy. The host defaults to the zero-JIT AOT path, while `--artifact canonical|optimized -- <program args>` explicitly JITs either portable stage with its independently recorded import surface for differential diagnosis; it never falls back automatically, and a missing optimized derivative is an error. Provider-specific release hosts are content-addressed and cached, so changing only Aver source repacks without rebuilding Rust. The pack builds for the current OS/architecture and runs `main`. Closes [#1254](https://github.com/jasisz/aver/issues/1254) and [#1261](https://github.com/jasisz/aver/issues/1261).

- **`aver run --wasm-gc` can now optimize the in-memory module before Wasmtime compiles it.** `--optimize speed` and `--optimize size` use the exact same Binaryen pipeline and WebAssembly proposal flags as `aver compile --target wasm-gc`, including when a project is re-executed through its cached custom-provider host. The default run path is unchanged and does not require `wasm-opt`.

- **A certificate can now say, in the kernel, that the plan it certifies IS the function you wrote.** Exports certified through the compute face used to be certified against a plan the compiler derived from your source; that the plan and your function were the same thing rested on the compiler, not on a proof. Each such export now carries a source-bridge theorem — the plan's model applied to your arguments equals your function applied to them — and the verifier writes that theorem's statement itself. The certificate declares only the structure of a bridge (which export, which source function, one encoder per argument and result out of a closed set), so a package cannot word its own claim; the verifier renders the statement from that structure and requires the package to prove exactly it. `aver cert check` and `aver cert verify` report `source-bridges: N of M credited` beside the law-claims count, and `aver cert explain` prints `model: plan ≡ <Module>.<fn> (credited source-bridge; see SOURCE-BRIDGES)` on each bridged export with the rendered statement under that heading — the credit means that statement is proven without foreign axioms, so the statement, encoders included, is what to read. A law about fully bridged functions gets a second corollary conjoining the law, the bytes, and those identities, counted apart as `bridged-laws: N of M credited`, so a bridge the checker cannot finish costs the bridge and that corollary and never the law. Shapes the encoders do not reach (nested records, non-Int record fields, Float, String, ADT arguments) stay unbridged, and `explain` now prints the reason from the package instead of leaving it on the compiler's output.

- **Artifact certificates now cover plain arithmetic on Int arguments.** A function that computes an integer from its own integer arguments — `double(n) = n * 2`, `addPair(a, b) = a + b`, `combineSurrogates(hi, lo) = 65536 + (hi - 55296) * 1024 + (lo - 56320)` — used to be reported as source-level only unless it happened to be exactly `n + k`. Such bodies now get a certificate for the exact bytes the compiler already emits, through the same face that already covered record arithmetic, and their report line shows the expression the bytes compute. `n + k` and the two Int comparisons (`a >= b`, `a == b`) move onto that face as well, and with it onto its domain, which `aver-cert explain` prints on the export's own line: inputs in the runtime's normal form. For the two comparisons that is wider than before (they used to be certified only for the small-value encoding); for `n + k` it is narrower (it used to be certified for every represented carrier, canonical or not), a difference no value the runtime produces can observe. The certificate of such an export states that the bytes compute the plan shown in its report line, and — where the package carries a credited source-bridge (below) — that this plan is the function you wrote.

- **Artifact certificates now cover comparisons between computed integers.** Programs that decide an order or an equality — `a.top * a.bottom >= 0`, `a.top * b.bottom < b.top * a.bottom`, `a.top * b.bottom == b.top * a.bottom` — now get a certificate for the exact bytes the compiler already emits, instead of being reported as source-level only. A plain sign test on a record field (`f.num >= 0`) is covered too. The certificate for such an export holds for inputs in the runtime's normal form, which is every value the runtime produces, and `aver-cert explain` prints that domain on the export's own line so it is not something you have to look up. The three arithmetic helpers now also state that their result is in that form.

- **Installing Aver now installs the agent guide, and one command attaches it.** The language guide and the toolchain guide ship inside the binary, so `cargo install aver-lang` carries them with no repository checkout. `aver agent-connect` writes them into your project as `.claude/skills/aver/SKILL.md` and `.claude/skills/aver-tooling/SKILL.md` and creates or refreshes a short pointer section in `AGENTS.md`; `--global` installs the same two skills under `~/.claude/skills/` instead; `--print` writes the language guide to stdout for an agent that prefers one file. Bytes outside the `AGENTS.md` markers are never touched, a skill file the command did not write is refused by name rather than overwritten, and re-running produces no diff. The guide's `decision` block section now covers the full field list, symbol-versus-label references, a worked example, and `aver context --decisions-only`. Closes [#863](https://github.com/jasisz/aver/issues/863).

- **A program can now say when written bytes have to be on the disk.** `Disk.sync(path)` returns only after that path's bytes and metadata reach stable storage, so a program that appends a batch of records and then writes the file that names them can make the records durable first. The path may be a file or a directory, and both matter: syncing a file does not make its own directory entry durable, so a file that has just been created is made durable by syncing the file and then its parent directory. One sync after a batch of appends costs one flush, not one per append. Closes [#1229](https://github.com/jasisz/aver/issues/1229).

- **Artifact certificates now admit the exact WASI 0.2 imports emitted inside wasip2 components.** The producer analyzes the post-wrapper core bytes actually embedded in the delivered component, while the verifier and Lean wall select a closed 80-pair canonical-ABI registry only for `target = "wasip2"`. wasm-gc keeps its separate Aver host registry, custom capability namespaces retain their exact hashed grammar on both targets, and unknown interfaces, operations, or versions still fail closed. Closes [#1230](https://github.com/jasisz/aver/issues/1230).

- **Artifact certificates now bind wasip2 components end to end.** `aver compile --target wasip2 --certify` hashes the delivered `.component.wasm`, records a producer-declared prefix/core/suffix envelope, analyzes the exact post-wrapper core bytes embedded in that component, and emits the target/ABI and Lean `Artifact.data` envelope consumed by the existing verifier wall. The verifier still derives split positions only from declared lengths and never navigates the delivered component to find the core. Closes [#1146](https://github.com/jasisz/aver/issues/1146).

- **wasm-gc and wasip2 now lower fused `Bytes.fromList` producers directly into the nominal packed-u8 carrier.** The closed `__byt_*` sink grows geometrically, validates each arbitrary-precision `Int` while it is pushed, preserves the exact first offending value/index and standard-library error text, and finalizes without materializing or rescanning a temporary `List<Int>`. Its byte-only gate is independent of generic `list_build`, so unsupported `__lst_*` collectors remain fail-closed; certified artifacts retain source traversal until the helpers receive byte-level certificate-wall roles. Closes [#1211](https://github.com/jasisz/aver/issues/1211).

- **wasm-gc and wasip2 now lower fused String collectors into a growable GC byte buffer instead of building and reversing `List<String>`.** The closed `__buf_*` contract tracks logical length, physical capacity, and whether any fragment has been appended, so growth is geometric and separators remain correct even after an empty first fragment. Native fixed-part interpolation keeps its existing one-allocation variadic concat, while the implemented `__to_str` path is covered for Int, Float, Bool, and String. Generic list sinks remain fail-closed. Implements [#1212](https://github.com/jasisz/aver/issues/1212), split from [#1208](https://github.com/jasisz/aver/issues/1208).

- **wasm-gc and wasip2 now walk fused `String.chars` loops with a UTF-8 cursor instead of allocating a list of one-character strings.** The two backends implement the same internal cursor and Unicode codepoint-folding contract as the VM and generated Rust while continuing to refuse unsupported list-builder intrinsics. On the real 2,000-block btc-listener workload under Node/V8, five-run means fell from 0.627 s to 0.501 s for the indexed-output walk and from 0.739 s to 0.520 s for the UTXO walk; ancestry-attributed `Bytes.fromHex` profile samples fell from 10.06% to 3.40% and from 10.77% to 5.26%, respectively. Embedded Wasmtime improved by 18.3% and 15.1% on the same walks. This is the cursor first stage of [#1208](https://github.com/jasisz/aver/issues/1208); the String builder and byte sink follow above as separate stages.
...
- **A `yield` function now reports "The type of `x` is not settled" for a binding that stays live across a request.** The lowering has to carry that binding's value in a generated state type, and an open type has no fixed representation to put there; the diagnostic points at the binding and asks for an annotation instead of failing later with an opaque error.

### Changed

- **`aver context` now describes the lowered module.** A `yield` function's entry is reported under its generated protocol names (`__loopStart`, `__LoopOutcome`, and the rest) rather than the function the lowering removes, matching what every other door already sees.

- **A bare name in an effect list must be `yield` or a capability namespace.** A function's `! [...]` and a module's `effects [...]` used to accept any bare identifier in silence, so a misspelled `! [yeild]` bought neither the effect nor a diagnostic; it is now an error naming the unknown effect and saying how an effect is written. This rejects sources that compiled before.
...
- **Every embedded Wasmtime path now shares one wasm-gc engine configuration, with a 16 MiB initial copying-collector heap.** Wasmtime otherwise starts each GC heap at zero and allocation-heavy programs repeatedly grow and collect while discovering their working size. On a real btc-listener workload that decodes and validates a 1,911-byte, twelve-input Bitcoin Core transaction, executes all P2PKH/P2WPKH scripts and signatures, and applies its UTXO change, the seven-run median for 50 iterations fell from 389 ms to 162 ms (2.40x faster). Wasmtime documents the initial heap as virtual space committed lazily, so this does not eagerly consume 16 MiB of physical memory per store. `aver run --wasm-gc`, `aver run --wasip2`, wasm-gc verification, and the Wasmtime benchmark runner now use the same collector, proposal, and optimisation settings. Verification keeps Wasmtime's defensive 512 KiB wasm-stack limit so a hostile non-tail-recursive case traps before exhausting its worker thread's native stack; production hosts retain their 8 MiB allowance.

- **wasm-gc Maps now hash proof-packed sequence keys with an FNV-1a fold instead of DJB2.** Both are valid deterministic hashes, but DJB2's low bits clustered fixed-width binary keys badly in the power-of-two open-addressing table. On the btc-listener `Map<Bytes, Int>` workload (200,000 resident 37-byte keys and 2,000,000 probes, Node 26/V8, `--optimize speed`), seven-run medians fell from 139.5 ms to 64.8 ms for construction, 1052.0 ms to 805.7 ms for hits, and 1555.2 ms to 515.7 ms for misses. The key representation remains the proof-derived packed `i8` array; equality, iteration order, and the source API are unchanged.
...
### Fixed
- **A capability whose boundary names a type of a dependency compiles to wasm-gc again.** A module writes its own type bare, while the contract the host decodes against writes it out in full, and the wasm-gc provider ABI took the bare spelling as the whole name — so a job kind that names the data types of its dependencies stopped the compile with `capability ABI cannot resolve named type`, including where the bare name sat inside a list. A bare name in a contract layout is now read as a type of the module that declares the layout, and only when that module really declares it, so two modules sharing a bare name still resolve to their own.

- **SIGINT and SIGTERM reach a program run from a project that binds a provider.** When `aver.toml` binds a capability to a Rust package, `aver run`, `aver verify` and `aver audit` execute the program inside the cached provider host. That host used to be a second process the command started and waited on, so a signal sent to `aver` reached the waiting process alone: it died where it stood, and the program it had started ran on with `Process.stopRequested()` still false until it hit its own deadline, while the same program compiled with `--target rust` stopped at once. On unix the host now takes over the command's own process instead of running beside it, so the process id, the terminal, the signal and the exit status all belong to the program that is running. A signal is then the program's to answer: from its first `Process.stopRequested` call the two signals raise its flag and end nothing, so Ctrl-C returns the prompt once the program has stopped rather than at the keypress. An in-process `aver run` and a binary built with `--target rust` already behaved that way. Windows keeps the second process and the behaviour it had.
- **A call in tail position is charged with its target's effects.** A call to a peer of the caller's own recursion group becomes a tail call before the type checker runs, and the checker used to walk only that call's arguments, so the target's `! [...]` was never required of the caller. A self tail call charges a function its own effects, which its declaration already carries, so what this rejects is a recursion group whose members do not all declare what the group performs: a `loopB` that declares `! [Console.print]` and ends in `loopA(n)`, where `loopA` reads the clock, passed `aver check`, while the same call written one line earlier was rejected. A tail call now carries exactly the obligation an ordinary call carries. Because the call runs in both directions around a cycle, every function in a recursion group declares what the group as a whole performs. A program that only checked because of the gap now gets the ordinary missing-effect error at the tail call's line, `Function 'loopB' calls 'loopA' which has effect 'Time.unixMs', but 'loopB' does not declare it`. Add the named effect to that function's `! [...]`, and to the module's `effects [...]` when the boundary does not already list it. A tail call that hands a named function to a parameter marked `! [_]` is charged that function's effects the same way, reported as `Function 'loopX' passes callback 'stamp' with effect 'Time.unixMs' to 'loopY', but 'loopX' does not declare it`. The unused-effect lint counts both shapes, so an effect added for either reason is not then reported as unused.

- **`BranchPath.Root` stays readable in a verify case and in a `!` branch.** A module with more than one verify case runs its cases on separate execution states, and so does each branch of an independent product; each is built from the compiled program and a fresh value store holding the program's constants. `BranchPath.Root` is a value like any other, but the only one the program reaches through its symbol table rather than through a constant or a global, and nothing carried it across into those states. Reading a path built from the root — any `BranchPath.child(path, i)` whose `path` came from `BranchPath.Root` — then reached a value that was not there, and the command died with `index out of bounds` instead of giving a verdict: `aver verify examples/formal/oracle_independent_products.av` crashed on both of the blocks that hand `BranchPath.Root` to a spec function. A module with a single case, and a stub that receives its path from the effect dispatcher rather than from the root, were unaffected, which is why the crash looked narrower than it was. Fixes [#1400](https://github.com/jasisz/aver/issues/1400).
...
- **A tuple-subject match with list, literal or Option arms runs on wasm-gc and on the Rust backend.** `match (state.tasks, state.running)` with an arm such as `([], 0) -> …`, `([next, ..rest], n) -> …`, `(true, 0) -> …` or `(Option.Some(task), 0) -> …` — the shape an answer function takes to read two ends of its state at once — used to stop `aver run --wasm-gc` with a `wasm \`unreachable\` instruction executed` trap the moment the function ran, and `aver compile --target rust` refused the function outright with `MIR walker could not render fn`; only the VM answered. Both backends now take it: each element of a tuple arm tests its field, the first arm whose elements all hold is taken in source order, and its binds are written before the body. A tuple match whose arms carry binds and wildcards alone (`(a, b) -> …; _ -> …`) runs on wasm-gc as well, where it used to trap. A tuple arm that carries a `String` or `Float` literal, or a variant of your own, in one of its elements is still outside what wasm-gc emits, and the Rust backend still refuses it beside an `Int` literal or a list element.
...
- **A law comparing a map's size or a lookup across a store inside a record update now closes in Lean.** Most one-key updates are written as `R.update(r, field = Map.set(r.field, k, v))`, and a law about one of those cited no fact about `Map.set` at all: the walk that records which builtins a law reaches read record creations but not record updates, so the store was invisible to it. It is recorded now, and two facts join the ones the compiler ships — a store under a key the map already holds does not change the map's size, and a removal never grows a map — so a law that says "settling never grows the table" or "the slot written is the slot read" closes for every input when its `when` or its `because` supplies the membership. Closing the first of those needed the map model itself to be corrected: its `set` walked the entries and inserted before the first greater key, which on a key already present is only right for a sorted list, and law variables range over every list, so the size fact was not merely missing but false in the model. Storing under a key the map holds now replaces that entry where it stands. Every map a program can build is unchanged by this, and every fact the compiler already shipped about `Map.set` keeps its statement. `examples/formal/knowledge.av` closes its monotone count for every input, and the generated coordinator's twenty-seven laws are now all universal, the five per-process size comparisons included.

- **A job a wait reported ready while it was still running keeps its handle.** A wait may report a job ready before it has finished — the contract says so — and the generated coordinator dropped the handle before asking for the result, so a job that answered "nothing yet" ran on with nobody left to collect it. The turn now removes a job only once its result is in hand; a job that answers nothing keeps its place in the table and is collected in a later turn.

- **A generated loop with more than one job kind is refused instead of running one of them.** The generated turn crosses one job seam, while the checker admitted any number of `work` bindings, so a second job kind was accepted and then never started or taken. Such a program is now refused with `error[run-binding]` naming every kind it found and saying that one job kind per generated loop is the limit in this build.

- **`Wait.poll` counts as a wait for `aver check` and `[verify] turn-budget`.** Both keyed on `Tcp.poll` alone, so a program whose turns are cut by `Wait.poll` — every program whose loop is generated — got neither the `warning[serve-path]` for a loop that runs a whole queue inside one turn nor a turn measured against the budget, and a run of many short turns was measured as one long one. `Wait.poll` now ends a turn exactly as `Tcp.poll` does.

- **A large `Wait.poll` timeout no longer stops the program.** A timeout the contract admits, up to `i64::MAX` milliseconds, named an instant the host's clock cannot hold and the wait ended the turn with a crash. Timeouts are now capped at a century, which is the "no deadline within reach" such a number asked for.

- **A wait over one job no longer returns empty because another job finished.** The job engine's signal is per engine, not per wait set, so an unrelated job settling ended a wait that was watching only its own job, and the turn saw "nothing ready" long before its timeout. The wait now looks at its own set again after every wake and goes back to sleep until one of its own jobs is ready or the timeout is spent.

- **A long-running program no longer piles up a tombstone for every job it finished with.** A collected or cancelled job kept its slot for ever so a second `take` could say "already taken". At most 4096 of those dead slots are kept now, oldest forgotten first, and a handle whose slot has been forgotten answers `Err("work: unknown job")`. A job kind forgets the ids it minted on the same schedule, so a handle whose slot the engine still holds is never answered with "this job was not started by job kind" while it is alive.

- **Lean proof discovery uses the generated project's toolchain.** With Elan installed without a default toolchain, the availability check previously failed in the source directory and silently left provable conditional laws bounded; dependent explanations then stayed open. The speculative proof and `--minimize` checks now run beside the generated `lean-toolchain`, matching the proof build.

- **A recorded map argument no longer depends on the map's internal order.** A recording wrote a `Map` with non-string keys in the order the store happened to hold it, so `aver replay --check-args` on a program that passes such a map to an effect (a wait set keyed by `Int`, for one) could report an argument mismatch on a different machine or a different run of the same one. Entries are now recorded in key order, the order every other part of the language shows a map in.

- **Law proofs now cite what the Lean prelude knows about `Map.set`.** When a law's cone calls `Map.set`, proof search carries the prelude's facts that a get after a set finds the stored value under that key and is unchanged under another, that a second set under one key overwrites the first, and that a set never shrinks the map; the last two join the prelude as hand-proved lemmas. The reasons solver behind `because` and `using` and the generic simplification rung both cite them, and a law with a sum-typed argument is also tried by constructor split, so a fact behind a `match` is reached. A lookup that survives a store under another key, a store that never shrinks the map, and a store over a store under one key now close universally instead of staying open.
...
- **A bound on the length of a countdown writer's output now certifies from the bound one digit below it.** Write the ladder about a base-256 digit writer — at most one digit below 256, at most two below 65536, at most three below 16777216 — and every rung now certifies from the rung under it. Only the bottom rung used to certify; every rung above it stopped at its samples, because the rung below was handed to the proof as an implication whose own condition nobody discharged, and a condition on the shrunk value (`value / 256 < 256` when `value < 65536`) is arithmetic, not rewriting. A rung is now used at the shrunk value with its condition discharged from the condition of the law being proved, and a rung whose condition does not hold at the shrunk value is left out rather than assumed. The bottom rung has no rung under it, and now reads the writer's own empty case at the shrunk value instead. Refs [#1284](https://github.com/jasisz/aver/issues/1284).

- **A law in a module that shares a function name with another module now proves against its own function.** Two modules of one program can both define `littleEndian`; whether a law about one of them certified used to depend on which module came first in the dependency list, because a bare call name was looked up across the whole program and the first match won. The Lean export now reads a bare name as the function of the module the law lives in, and the pools of citable laws read that module's own blocks. On the btc-listener modules this is the difference between the script-number laws certifying from the root entry, the way the project runs `aver proof`, and failing there while certifying when the module is proven alone.
...
- **A law `given` now types every sample it expands into cases, and rejects one that contradicts it.** `given xs: List<Int> = [[], [1], [1, 2]]` says the element type once, for the whole domain, but the empty sample used to keep it only where the call around it happened to supply one: `allBytes(List.reverse(List.reverse(xs)))` was still rejected with `expected List<Int>, got List<T>`, and under builtins that accept any list (`List.len(List.reverse(xs))`) nothing was rejected while the expanded case carried an unresolved element type into the proof lane and the backends. Each substituted sample now takes the type its `given` declares, so a law reads the same whether or not `[]` is in its domain. The other side of the same annotation: a sample that contradicts it — `["a"]` under `given xs: List<Int>` — is now reported at the `given`, including in laws whose builtins accept any element type and therefore never objected to it.
...
- **Two more law shapes now certify in Lean instead of stopping at their samples.** A `when`-law whose function decides between two list layouts and whose reader matches on that decision (a sign byte placed in front of a byte list, read back as the signed sum) used to fall back to its sampled domain: the proof split the reader before the decision was made and lost track of which byte it was looking at. A round trip stated over a countdown fed an expression (`bigEndian(reverse(digits(magnitudeOf(value), [])), 0) => magnitudeOf(value)`) used to keep an honest `sorry`, although the countdown's own laws one block above were already proven. The first now closes by deciding the function's own guard first; the second now closes by composing the earlier laws about the functions it calls, as the last attempt before the `sorry`.

- **A `match` on a condition whose second arm is named instead of `false` now works on every backend.** `match x < 0` with `true -> -x` and `other -> x` is the same program as the one that spells the second arm `false` or `_`, but only the latter two were recognised as a two-way branch. The named form was left as a general `match` on a `Bool`, which each backend then answered differently: the VM ran it, wasm-gc built a module that crashed as soon as the function was called, and `aver compile --target rust` emitted a crate that would not build, with the message `MIR walker could not render fn` at the function. All four backends now agree. The name is still bound in that arm — it holds `false`, as it always did — and the condition is evaluated once, so a condition that does something is not done twice.

- **A function with no body is now reported as a mistake instead of a compiler failure.** `fn f()` with nothing under it is not a declaration in Aver, and there is nothing to run for it. Until now the file parsed and the failure surfaced much later, in words about the compiler rather than about the program: `aver run` said `internal error: fn 'f' did not lower to MIR`, and `aver compile --target rust` wrote a crate that would not build. It is now refused at the signature, with the line it is on. The mistake is easy to make in more than one way, and none of them look empty: a signature at the end of a file, a body indented under `?` and `!` but never written, `fn main() []` putting the effect list where `! [...]` belongs, or `fn f() 1 + 2` written on the signature line.

- **A function that binds an integer literal to a name no longer breaks the Rust backend.** `n = 1` followed by `n` is about as ordinary as Aver gets, and until now the crate `aver compile --target rust` emitted for it did not compile: the binding held a plain machine integer but was handed a boxed one, and `rustc` rejected the mismatch. The same applied to a binding whose value folds to a literal (`n = 1 + 2`) and to one that reaches the name through a `match`.

- **`Disk.sync` on a directory now works on Windows.** It used to fail there with an access error, so the crash-safe recipe — sync the file, then sync its parent directory — could not be written once and run everywhere. Windows offers no call that flushes a directory, so the second sync now returns `Ok` without touching the disk; NTFS journals the metadata that sync is asking about, so the durability the program asked for holds when the call returns. Syncing a file is a real flush on Windows as everywhere else, and a path that is not there is still an error. Closes [#1238](https://github.com/jasisz/aver/issues/1238).

- **Record fields now run in the order you wrote them, not the order the record declares them.** A record literal or `T.update(...)` whose fields are written in a different order than the type declares could hand a later field a value that an earlier field had already consumed, so the field ended up empty and the program either failed with `field access on non-record value (Unit)` or, worse, silently produced a wrong answer. Both forms now evaluate their fields top to bottom as written; a field named twice in one update still keeps the first value. Closes [#1240](https://github.com/jasisz/aver/issues/1240).

- **A law-claim the checker cannot credit no longer costs a package the exports it did certify.** `aver cert check` and `aver cert verify` report law-claims per claim: the verdict line now ends with `law-claims: 10 of 11 credited`, and every uncredited claim gets its own line naming it and the axiom its proof leans on (`law-claim not credited: Infra.Codec.unescapeField.escapedRoundtrip (proof depends on sorryAx)`). The producer cannot know in advance which claims the checker's kernel will close, so a package that declares a law its proof cannot finish keeps its export verdict and its exit code — `projects/payment_ops`, previously `CHECK FAILED` over one such law, now checks its five exports. A claim whose statement is not what the package proves is a different matter and still declines the whole package, as does a claim the checker cannot audit at all. Packages with no law-claims print exactly the line they printed before.

- **Certificate packages emitted from real project trees now build.** Two model-side failures surfaced on `projects/`: a carried model that compares user enums with `==` failed to elaborate, because certificate data may not carry `deriving` (the checker's token wall rejects it) and no `BEq` instance was emitted by hand; and a model function that recurses on a list under an `if` inside a match arm failed its termination proof, because the emitted `decreasing_by` tactic was too weak for that shape. Enums now get an explicit `BEq` instance in certificate model files, the termination tactic falls back to a stronger chain, and `projects/payment_ops` — previously `CHECK FAILED certificate data did not build` — now reaches its `check` verdict, guarded by a new certify test. A third gap surfaced right behind those two: `String.fromBool` was called by models of programs that interpolate a `Bool` but had no definition anywhere, so it is now defined in the shared prelude. The termination and `fromBool` fixes apply to `aver proof` exports of the same shapes too.

- **The VM collector no longer re-reads carried `List.concat`, `Vector<String>`, and all-immediate `Map` values at every frame boundary.** `Concat` promotion now stops at the settled part of the spine and processes a fresh deep spine with an explicit work stack, so a growing concat list is no longer quadratic merely to keep alive and a 12,000-node spine does not overflow Rust's stack. Heap-backed vectors carry the same lane receipt as lists and maps, so an unchanged vector is scanned once rather than once per step; a heap write invalidates that proof. Debug builds also trust the map flag proved by its producers instead of walking the table again inside the skip path. Closes [#1190](https://github.com/jasisz/aver/issues/1190).
...
- **A loop that reads characters and keeps them in a list no longer slows down as the list grows.** Building a list with `List.prepend` while reading characters was quadratic on the VM for a second, separate reason: every time the runtime tidied up after a step it walked the whole list that had been built so far, even though none of it could move. It now stops at the part of the list the step did not touch. Reading 65,536 characters and keeping each one took 50 seconds and now takes 0.15; the same walk at 16,384 and 32,768 characters went from 3.2 and 13 seconds to 0.04 and 0.07. Any loop that carries a growing list past a `String.charAt` gets this. Closes [#1187](https://github.com/jasisz/aver/issues/1187).

- **A raised `[[verify.costly]]` budget applies however you spell the path, and an entry is no longer called stale over a file the run did not reach.** `aver verify "$PWD/main.av" --module-root .` compared the absolute file against globs anchored at the module root, so `files = ["main.av"]` matched nothing: the expensive case ran under the default budget, came back as `1 not answered`, the run exited 1, and `aver verify main.av --module-root .` in the same directory passed. Both sides of that comparison are now resolved on disk first, so the four ways to spell one file give one answer. Separately, `the fn may be stale` is now said only by a run that verified every file the entry covers, because the function it names may well live in a file this run did not reach. An entry with no `files` glob covers every module, so in a project of several modules that verdict now comes from a run over the project — `aver verify .` — rather than from whichever module you verified last. Closes [#1200](https://github.com/jasisz/aver/issues/1200).

- **A job whose task or answer is a capability-owned record records and replays on every backend.** The ledger now spells those values by the canonical `Module.Type` name the provider boundary uses — `Scorer.Task`, `Scorer.Report` — where the VM used to write the program-side short name for a `begin` argument and generated Rust wrote it everywhere, so a recording could fail its own replay on a tag mismatch, and a generated binary refused a VM recording's canonical tags outright. Recordings that still carry a short name replay on all three backends, because a type's own short name is the tag's one legitimate alias; a tag naming any other type that happens to share it is reported as the foreign record it is rather than decoded. The VM still reruns the bound function beside the recorded answers and stops the replay on a different result, now naming the record under its canonical type. Closes [#1354](https://github.com/jasisz/aver/issues/1354).
...
- **The Lean map prelude carries `AverMap.len_set_ge` and `AverMap.set_set_self`.** A set never shrinks a map, and a set over a set under one key is one set; both are emitted on demand like `get_set_self` and are not cited by any proof rung yet.
