...
### Fixed — `aver proof` exports a function that calls `Wait.poll`

- **A function that calls `Wait.poll` and has a `verify` block now gets a Lean definition and its cases.** `Wait.poll` is generic over the key type of its wait set, and the proof export gave the function's oracle parameter the unbound key type. An older compiler panicked on it; a newer one dropped the function and still emitted its cases, so `lake build` failed on an unknown name. The export now reads the key from the program, as the Rust backend does (one program keys every wait the same way), and the oracle is typed at that key: `Map<Watch, Wait.Item>` gives `List (Watch × Wait.Item)`. A program whose waits disagree about the key, which `aver check` already rejects, has its claims declined with a reason, and they count against `--declined-budget`.
- **A case with a quantified oracle whose branch condition is a closed test now builds.** `polled({}, 10)` over `match Map.len(items) == 0` left `AverMap.len [] = 0` as a hypothesis beside the free oracle, and `native_decide` refused the goal. The case is now proved with `simp +decide`, which decides the condition. The same case over `Tcp.poll` had the same failure.
- **A case that calls an effectful function under `?` or by its module path now passes it its oracles.** `get(prunedUndo(fixture([]), 300)?, key()?)` and `Infra.Store.get(...)` were exported without the path and oracle arguments of the lifted function, and Lean rejected them. A qualified function whose name Lean reserves, such as `Domain.ByteField.at`, is now spelled `at'` in the case's unfold list, as it is defined.
