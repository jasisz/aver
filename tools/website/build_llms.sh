...
build_cli_md() {
    local out="$REPO_ROOT/docs/cli.md"
    {
        ...
        echo
        echo "Full toolchain reference for the \`aver\` binary. README points here for the per-command surface; deeper topics: [bench](bench.md), [transpilation](transpilation.md), [Lean](lean.md), [effects](effects.md), [WASI 0.2](wasip2.md), and [Rust](rust.md)."
        echo
        strip_skill_prompt "$SKILLS/aver-tooling.md"
    } > "$out"
    echo "wrote $out ($(wc -l < "$out") lines, $(wc -c < "$out") bytes)"
