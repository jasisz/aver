...
- **`String.fromFloat(x)` on `--wasm-gc` no longer traps on finite floats around `1e4`–`1e18`.** The wasm-gc backend prints floats through a hand-rolled WAT routine whose shortest-roundtrip search loop kept multiplying `abs_val * 10^N` until `i64.trunc_f64_s` overflowed and the module trapped with `wasm trap: integer overflow`. A guard now bails out of the loop one iteration before the product would exceed `2^63`, accepting fewer fractional digits at very large magnitudes instead of a runtime crash. Repro that crashed pre-fix: `String.fromFloat(((-0.5772) * (-61.8024)) * (-877.8128))` (≈ `-31313.64`); the VM had always printed it correctly.
- **A malformed compile no longer crashes the process with `panicked at vm/symbol.rs`.** Sources that registered two different meanings under one symbol — duplicate variant constructor with conflicting payload, a constant rebound under a name already used as a function, an empty namespace path — used to unwind the VM compiler with `panic!`, taking the whole `aver` invocation down. The VM symbol table now rejects these as ordinary `Compile error: VM symbol 'X' already exists as ...` diagnostics that point at the offending source. No legal program could ever trip these panics, so this only changes behaviour for malformed inputs (fuzz finds, hand-edited intermediate files, future codegen bugs) — but a crash is a worse signal than a typed error.
- **One source error no longer fans out into a cascade of "expected X, got Invalid" diagnostics.** A single unknown function call like `let z = add(unknownFn(1), unknownFn(2))` used to report the two real `Call to unknown function` errors plus two follow-on "Argument N of 'add': expected Int, got Invalid" reports — once per downstream consumer that received the recovery sentinel. The matcher now treats `Type::Invalid` as a wildcard ("already reported here, don't compound"), so callers see only the originating diagnostic. Same behaviour applies to malformed annotations, unknown record/variant constructors, and any other source error that leaves a `Type::Invalid` recovery node in its wake.
- **Two modules can't accidentally share a type just because they spell its name the same.** Pre-Iron, a project that imported `A` (exposing `type Shape { Circle, Square }`) and `B` (exposing `type Shape { Triangle, Hexagon }`) would silently let a `B.Shape` value satisfy a parameter typed `A.Shape`: the matcher's bare-vs-qualified `ends_with` fallback collapsed both onto the same string. The typechecker now resolves every named-type reference through `sig_aliases` to its canonical `Module.Type` form before comparing, so the crossed call is rejected with `Argument N of '...': expected A.Shape, got B.Shape`. The legitimate single-module case (a function inside `Types` declaring `fn open() -> Status` and a caller in `App` calling `Types.open()` against an annotation of `Types.Status`) still type-checks; the canonical resolution treats both sides as the same type.
