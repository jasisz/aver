...
Explanation obligations are universal. A stage the backend cannot prove remains failed; it does not silently substitute sampled evidence. Dafny implements a [restricted nonrecursive Int/Bool pilot](dafny-guidance-spike.md) with explicit local citations and declines guided laws outside that fragment. The syntax is provisional; [#1288](https://github.com/jasisz/aver/issues/1288) tracks the mechanism and acceptance checks.
