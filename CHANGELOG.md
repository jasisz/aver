...
- **Proof arguments live in Aver source.** Removed automatic loading and injection of hand-written Lean/Dafny proof sidecars and their `hand` manifest credit. Existing sidecar files no longer affect exports. K5's integer trunc-sticky law now proves universally with `because`/`using`; generic nested-floor, common-factor cancellation, and remainder-absorption laws can be cited with their original premises. The floating-point composition remains open after its manual proof is removed. Lean core floor proofs also use the current `Int.mul_ediv_add_emod` theorem name.
