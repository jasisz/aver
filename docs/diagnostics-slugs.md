...
| `work-binding` | error | A job kind of the program has no `work` binding in `aver.toml`, or its binding names a function that the program does not have, that belongs to the entry module the command was pointed at, that declares effects, or whose parameter or result is not the `begin` task type and the `take` payload type. The same slug covers the job seam: the `task`, `started` and `landed` keys on that binding, which are the three ends between the turn and an answer state. Either all three are declared or none. All three name a pure function of a module bound with `answer`, with signatures `(S) -> Option<T>`, `(S, T) -> S` and `(S, Result<R, String>) -> S` for that module's state `S`, the `begin` task `T` and the `take` payload `R`. `started` consumes the task that `begin` just answered `Ok` for, so `task` and `started` name functions of the same module, the one whose state offers and records the task. `landed` takes a `Result` because a job that was cancelled, whose body stopped, or whose id the engine has forgotten reaches the answer state as an error instead of stopping the run. The manifest itself rejects `work` beside `crate`/`package`/`factory`, any of `task`/`started`/`landed` without `work`, a seam declared in part, and a `work`, `task`, `started` or `landed` value that is not `Module.function`. | Add or fix one `[[providers.bindings]]` entry with `capability = "<JobKind>"` and `work = "Module.function"`. It must name a pure function with signature `(T) -> R` in a module of the program other than the entry module. Give it all three seam keys or none. |
| `answer-binding` | error | A capability is marked `answer = "Module"` in `aver.toml` and the module cannot answer it. This happens when: the capability is one the compiler ships (`Console`, `Disk`, `Tcp`, `Time`, `Wait`, `Work`, …), which the runtime's own providers answer and the generated turn calls itself; the named module is itself a capability module, which declares operations and computes nothing; the program has no such module; an answer function's first parameter is not the state, a type the answering module declares; an operation of the capability has no answer function; an answer function's signature is not `(S, a1: T1, …) -> Tuple<S, Cap.<Op>Reply>`, with the reply sum the capability declares beside the operation; one module answering several capabilities threads more than one state; or two answered capabilities of one module share an operation name, so one function would have to answer both. The manifest itself rejects `answer` beside `crate`/`package`/`factory`/`version`/`path` or beside `work`, `task`/`landed` beside `answer`, and an `answer` value that is not one module name. | Declare a capability of your own instead of marking one the compiler ships. Write one answer function per operation in an ordinary module; it takes that module's own state first and returns it beside the operation's declared reply sum. Rename an operation when two answered capabilities collide. |
| `answer-shape` | error / warning | Covers the shape an answer is read through and what an answer function itself does. **Error**: a capability that an `answer` binding names declares no `<Op>Reply` beside one of its operations, or declares one of another shape. The sum has exactly three constructors, `Now(R)`, `Later(Wait.Wake)` and `Then(Wait.Wake, R)`, where `R` is exactly the operation's result type. A missing declaration, a record of that name, a wrong payload, and a missing or extra constructor are each named, and the message prints the declaration the seam wants in the module's own names. A capability that nothing answers is not checked. **Error**: the function declares `yield`. An answer is computed inside the turn, so it cannot itself be a process the turn has to drive. **Warning**: the function declares effects. This is allowed, since an answer runs inside the turn, but it can stall every other process. | Paste the printed declaration into the capability module beside the operation and expose it. Drop `yield` from the answer function and make the process that needs it the caller instead. For the warning, accept the stall or move the work into a job. |
| `run-binding` | error | A `[run]` table supplies only some of `order`, `admit`, `stop`, `view` (use none for defaults, or all four for custom policies). Or `aver.toml` declares `[run]`, so the program's loop is generated, and something it is generated from is missing or has the wrong shape: the module named by `view` writes no process; a process takes parameters or answers something other than `Unit`, so the loop has nothing to seat it with or nowhere to put its result; the module writes its own `main`, which the loop generates; an answer module has no pure `fresh() -> <Module>.State`; two answer modules would hold their state in one field of the run table, or one of them would take a field the loop writes itself (`slots`, `jobs`, `dropped`, `stopping`, `now`, `nextId`); a policy is missing, effectful, or does not read the view; a job kind's `task` and `started` name two modules; a job kind's `started` function states no `verify` law named `aStartedTaskIsNotAskedAgain`, which the generated loop cites (the message prints the block to write); or the four `[run]` keys do not all name one module. The manifest itself rejects a `[run]` table that declares only some of the four keys, and a value that is not `Module.function` / `Module.Type`. | Write the processes, the answer modules and any custom policies, and let the loop write the rest. Every process takes no parameters, returns `Unit`, and is written in the module the loop is generated into. Every answer module has a pure `fresh` and a name that no other answer module and no field of the run table shares. Every job kind's `started` function states `aStartedTaskIsNotAskedAgain`. Either omit all four `[run]` keys for defaults or name all four in that same module. |
| `view-shape` | error | With custom policies, the record the three policies read, or the sum it keys its pending table by, does not have the shape the generated loop fills: a missing or extra field, a field of the wrong type, a missing `Pending` sum, a constructor for no process of this program, a process with no constructor, or a constructor that does not carry the instance number and the wake. The loop fills `pending: Map<Int, Pending>`, `ready: List<Int>`, `askable: List<Int>`, `jobs: Int`, `room: Int` and `stopping: Bool`. The message prints the whole declaration the loop fills. | Declare the view record and the marker sum exactly as the message prints them. |
| `intercept-outside-yield` | error | A function that does not declare `yield` performs an operation of a capability that `aver.toml` marks `answer = "Module"`. Such an operation is a request: the lowering cuts a process at it and hands it to the coordinator. Nothing answers it anywhere else, because no provider is bound to the capability and no request kind exists for the call. | Add `yield` to the function's effect list so the call becomes a request. If you wanted the answer itself, call the answer module's own function directly; the message names it with its parameters. |
...
| `work-max-jobs-ignored` | warning | `aver.toml` sets `[work] max-jobs`, and a program that declares a job kind is compiled or run for `wasm-gc` or `wasip2`. A program with no job kind starts no job, so the key changes nothing there either and no warning is emitted. On those targets a job runs inline at `begin` and finishes before the next expression, so at most one is ever running and the key decides nothing. The warning names the key and the target. | Keep the key for the VM and the Rust backend, where it has an effect, or remove it if the program only ever runs on a wasm target. |
