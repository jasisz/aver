fn render_certificate(
    ...
    for c in &analysis.certs {
        match c.inner() {
            ...
            // Selection-faced and compute-faced fragments are discharged
            // inside the wall from their checked face; they contribute no
            // semantic bridge, so they must be routed away from the
            // audited-generic bridge renderer even though their source types
            // sit inside its gate.
            Cert::ExprFragment { .. }
                if c.int_cmp_face().is_some() || c.record_compute_face().is_some() => {}
            Cert::ExprFragment { .. }
