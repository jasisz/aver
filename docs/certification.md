...
> Status: v0, certification level **L1** (conditional on named runtime contracts). Six certified function classes (three integer, three ADT); everything else is **declined fail-closed** and listed with a reason. This document is the contract; the emitted `cert-manifest.json` is its machine-readable form.
...
- **Six certified classes.** Three integer: straight-line integer functions of the add-a-constant shape, single-argument structural self-recursion, and two-argument accumulator self-recursion (`f(n, acc)`: the first integer parameter descends, the second is an accumulator updated in the straight-line fragment). Three ADT: a non-recursive **constructor** (one- or two-field), a non-recursive **field projection**, and a non-recursive **variant match**. The variant match is not a general `match`: it is the frozen three-branch shape `Add x -> x; Neg x -> 0 - x; Zero -> 0` over a single-integer-payload user inductive (the v0 shape of this class), and nothing wider. Everything else — including mutual recursion, multi-argument recursion beyond the two-argument accumulator shape, and any match outside that fixed three-branch Int-payload shape — is declined and listed under `source_level_only` with a reason. The classifier is fail-closed: a function whose emitted body does not match a certified template is declined even if it looks like it should fit.
