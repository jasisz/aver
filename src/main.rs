...
fn collect_all_decisions(contexts: &[FileContext]) -> Vec<&DecisionBlock> {
    contexts
        .iter()
...
fn format_decisions_md(decisions: &[&DecisionBlock], entry_file: &str) -> String {
    let mut out = String::new();
...
fn format_decisions_json(decisions: &[&DecisionBlock], entry_file: &str) -> String {
    let mut out = String::from("{\n");
    out.push_str(&format!("  \"entry\": {},\n", json_str(entry_file)));
    out.push_str("  \"decisions\": [");
    if decisions.is_empty() {
