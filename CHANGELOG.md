...
- **`compute_memo_fns` reads from `PipelineResult.analysis`.** First real consumer plumbed onto the centralised facts: the fn no longer recomputes `find_recursive_fns` / `recursive_callsite_counts` when an `AnalysisResult` is passed. Demonstrates the contract pattern (analyze produces facts, consumers read them); plumbing the rest of the codegen consumers is mechanical follow-up for 0.15.2.
