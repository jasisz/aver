...
### Removed (breaking)
- **Paren-tuple types `(A, B)` are no longer valid in type position.** `Tuple<A, B>` is the only tuple-type spelling. Tuple **value** literals — `(1, 2)`, `Result.Ok((a, b))`, `[(1, 2), (3, 4)]` — and tuple **patterns** — `match p { (a, b) -> … }`, `[(a, b), ..rest]` — stay unchanged: paren is the runtime-tuple constructor, `Tuple<…>` is the type. The two were the same surface character in two different positions, which made grep-for-types unreliable and codegen had to keep two canonical forms in sync (the symptom that surfaced this: wasm-gc registry indexed `List<Tuple<Int,Int>>` while emit-side stamps stayed `List<(Int,Int)>` — `cons helper for List<(Int,Int)> not registered`). Now there's exactly one tuple-type spelling, mirroring `Result<T,E>` / `Option<T>` / `Map<K,V>` / `List<T>` / `Vector<T>`. Migration is purely syntactic: every type annotation `(A, B)` becomes `Tuple<A, B>`. The old form parses to a clear error: `paren-tuple types removed — use 'Tuple<A, B>' instead`. Migrated in-tree: ~30 files across `examples/`, `self_hosted/`, `tools/website/`, `docs/`. `Type::display` and the type-error formatter both render `Tuple<…>` so diagnostics agree with source.
