...
Programs with `[run]` compile with `--target wasip2` and run with `aver run --wasip2`, provided their own effects are supported. The generated turn does not observe signals, and its `View.stopping` field stays `false`. An empty `[run]` uses the default rule: stop when no process is seated and no job remains. Custom policies keep their normal rule: stop when `stop(view)` is true or no process is seated. A custom policy that waits only for `view.stopping` needs another completion condition while processes remain seated. This policy is specific to the generated loop on this target; explicit `Process.stopRequested` calls still fail the capability check. VM, Rust and wasm-gc keep their host signal observation. The guide fixture, saved `Then` replies (including `Unit`), cancellation and multiple job kinds run as command components. A program using unsupported operations such as `Tcp.listen` still needs another target.
