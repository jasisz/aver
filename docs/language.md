...
`aver verify` does not discover or install host packages. A source-local cases-form binding such as `given hash: Hash160.digest = [fixtureHash]` installs that Aver function only for each expanded verify case. The alias may be unused in the assertion: the binding still redirects reached dispatch. It never satisfies normal `aver run` provider preflight and does not test the provider implementation itself.
