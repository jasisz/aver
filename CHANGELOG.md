...
- **A long-running program's job table no longer grows for ever.** A collected or cancelled job kept its slot so a second `take` could say "already taken", and every job kind kept the id of every job it ever started. Both are now bounded at 4096 entries, oldest forgotten first; a handle whose slot has been forgotten answers `Err("work: unknown job")`.
