...
| `yield-direct-call` | error | A function, an unsupported verify form, or another yielding function's argument calls a `yield` function directly; the function never runs as written. | Call its generated `__<fn>Start(...)` and answer the requests its `__<Fn>Outcome` carries, with `__<fn>Answer<Kind>` per request kind. |
| `yield-non-tail-call` | error | A yielding function calls itself outside tail position; a process nests another yielding function, not itself. | Pass what comes next as data, or make it a tail call. |
| `yield-unsupported` | error | A yielding function holds a construct the lowering does not cut: mutual nesting between two yielding functions, a cycle of tail calls between them, a request or a call to a yielding helper inside an independent product `(a, b)!`, a function value live across a request, or no stop at all. | Restructure as the message says: break the cycle, perform them one after another, pass data instead of a callback, or drop `yield`. |
...
| `work-binding` | error | A job kind of the program has no `work` binding in `aver.toml`, or its binding names a function this program does not have, one of the entry module the command was pointed at, one that declares effects, or one whose parameter or result is not the `begin` task type and the `take` payload type. The same slug covers the job seam — the `task`, `started` and `landed` keys on that binding, the three ends between the turn and an answer state: all three are declared or none, all three name a pure function of a module bound with `answer`, and their signatures are `(S) -> Option<T>`, `(S, T) -> S` and `(S, Result<R, String>) -> S` for that module's state `S`, the `begin` task `T` and the `take` payload `R`. `started` is the consumption of the task `begin` just answered `Ok` for — `task` and `started` name functions of the same module, the one whose state offers and records the task — and `landed` takes a `Result` because a job that was cancelled, whose body stopped, or whose id the engine has forgotten reaches the answer state as the error it is rather than stopping the run. `work` beside `crate`/`package`/`factory`, any of `task`/`started`/`landed` without `work`, a seam declared in part, and a `work`, `task`, `started` or `landed` value that is not `Module.function`, are rejected by the manifest itself. | Add or fix one `[[providers.bindings]]` entry with `capability = "<JobKind>"` and `work = "Module.function"` naming a pure function of a module of the program — not the entry module — whose signature is `(T) -> R`; give it all three seam keys or none. |
| `answer-binding` | error | A capability is marked `answer = "Module"` in `aver.toml` and the module cannot answer it: the capability is one the compiler ships (`Console`, `Disk`, `Tcp`, `Time`, `Wait`, `Work`, …), which the runtime's own providers answer and the generated turn calls itself; or the module named is itself a capability module, which declares operations and computes nothing; or the program has no such module; or an answer function's first parameter is not the state, a type the answering module declares; or an operation of the capability has no answer function; or an answer function's signature is not `(S, a1: T1, …) -> Tuple<S, Cap.<Op>Reply>`, the reply sum the capability declares beside the operation; or one module answering several capabilities threads more than one state; or two answered capabilities of one module share an operation name, so one function would have to answer both. `answer` beside `crate`/`package`/`factory`/`version`/`path` or beside `work`, `task`/`landed` beside `answer`, and an `answer` value that is not one module name, are rejected by the manifest itself. | Declare a capability of your own rather than marking one the compiler ships; write one answer function per operation of an ordinary module, taking that module's own state first and returning it beside the operation's declared reply sum; rename an operation when two answered capabilities collide. |
| `answer-shape` | error / warning | The shape an answer is read through, and what an answer function itself does. **Error**: a capability an `answer` binding names declares no `<Op>Reply` beside one of its operations, or declares one of another shape — the sum has exactly three constructors: `Now(R)`, `Later(Wait.Wake)` and `Then(Wait.Wake, R)`, where `R` is exactly the operation's result type; a missing declaration, a record of that name, a wrong payload, a missing or extra constructor are each named, and the message prints the declaration the seam wants in the module's own names. A capability nothing answers is not checked. **Error**: the function declares `yield` — an answer is computed inside the turn, so it cannot itself be a process the turn has to drive. **Warning**: the function declares effects; an answer runs inside the turn, so this is allowed, but it can stall every other process. | Paste the printed declaration into the capability module beside the operation and expose it; drop `yield` from the answer function and make the process that needs it the caller instead; for the warning, accept the stall, or move the work into a job. |
| `run-binding` | error | A `[run]` table supplies only some of `order`, `admit`, `stop`, `view` (use none for defaults or all four for custom policies), or `aver.toml` declares `[run]`, so the loop of this program is generated, and something it is generated from is missing or is the wrong shape: the module named by `view` writes no process; a process takes parameters, or answers something other than `Unit`, so the loop has nothing to seat it with or nowhere to put its result; the module writes its own `main`, which the loop generates; an answer module has no pure `fresh() -> <Module>.State`; two answer modules would hold their state in one field of the run table, or one of them would take a field the loop writes itself (`slots`, `jobs`, `dropped`, `stopping`, `now`, `nextId`); a policy is missing, effectful, or does not read the view; a job kind's `task` and `started` name two modules; a job kind's `started` function states no `verify` law named `aStartedTaskIsNotAskedAgain`, which the generated loop cites (the message prints the block to write); or the four `[run]` keys do not all name one module. A `[run]` table that declares only some of the four keys, or a value that is not `Module.function` / `Module.Type`, is rejected by the manifest itself. | Write the processes, the answer modules and any custom policies, and let the loop write the rest: give every process no parameters and a `Unit` result, write every process in the module the loop is generated into, give every answer module a pure `fresh` and a name no other answer module and no field of the run table shares, state `aStartedTaskIsNotAskedAgain` on every job kind's `started` function, and either omit all four `[run]` keys for defaults or name all four in that same module. |
| `view-shape` | error | With custom policies, the record the three policies read, or the sum it keys its pending table by, is not the shape the generated loop fills: a missing or extra field, a field of the wrong type, a missing `Pending` sum, a constructor for no process of this program, a process with no constructor, or a constructor that does not carry the instance number and the wake. The record the loop fills is `pending: Map<Int, Pending>`, `ready: List<Int>`, `askable: List<Int>`, `jobs: Int`, `room: Int` and `stopping: Bool`. The message prints the whole declaration the loop fills. | Declare the view record and the marker sum exactly as the message prints them. |
...
| `work-target` | error | History. A program with a Work-shaped capability — a job kind — used to be refused on a wasm backend. Every target runs a job kind since jasisz/aver#1329: the VM and `--target rust` run it beside the turn on a thread, and `--target wasm-gc` / `--target wasip2` run it inline at `begin`, because a component and a wasm-gc module are single-threaded. No door emits this slug any more. | Nothing to repair. See [services.md](services.md#on-wasm-gc-and-wasip2) for what a job does on a wasm target. |
| `work-max-jobs-ignored` | warning | `aver.toml` sets `[work] max-jobs` and a program that declares a job kind is compiled or run for `wasm-gc` or `wasip2` — a program with no job kind starts no job, so the key changes nothing there either and the door stays quiet. A job on those targets runs inline at `begin` and is over before the next expression, so at most one is ever running and the key decides nothing; the warning names the key and the target. | Leave the key for the VM and the Rust backend, where it holds, or remove it if the program only ever runs on a wasm target. |
...
| `unused-expose` | warning | Manifest-bound functions and types, and names the generated loop uses, count as used.  Module `exposes` a name nothing in the checked program(s) imports; a directory input judges every program in it. | Drop from `exposes` or start using it. |
