...
### Added
- **Duplicate `fn` name in a module is now a type error.** Pre-Iron the second definition silently shadowed the first; a typo could swap your function's body without any signal.
- **Polymorphic recursion that would need `T := F<...T...>` is rejected** with a normal type-incompatibility error instead of silently typechecking with a circular binding. E.g. `fn nest(v: A) -> Unit; nest([v])`.

### Improved
- **Unary minus on `Int` / `Float` skips runtime tag-dispatch on the VM** via two new typed opcodes (`NEG_INT` / `NEG_FLOAT`) — same pattern as the 0.17.2 typed-arith sweep.

### Fixed
- **`aver replay` can now load `Vector` values and single-string-key `Map`s.** Two missing decode arms in the replay codec rejected legal recordings with `unknown replay marker '$vector'` / `'key'`.
- **`-0.0` round-trips its sign bit on every backend.** Pre-Iron `-x` desugared into `0 - x` and IEEE 754 collapsed `0.0 - 0.0` to `+0.0`; unary minus is now first-class in the AST and every backend negates at the float bit level.
- **`String.fromFloat` on `--wasm-gc` no longer traps on finite floats around `1e4`–`1e18`.** The shortest-roundtrip search loop overflowed `i64.trunc_f64_s`; the loop now bails one iteration before overflow and accepts fewer fractional digits at very large magnitudes instead of a runtime crash.
- **Malformed compile inputs produce a typed diagnostic instead of `panic!`.** The VM symbol table used to abort the whole `aver` invocation when two definitions clashed under one name; the conflict now surfaces as a normal `Compile error: …`.
- **One source error no longer fans out into a cascade of "expected X, got Invalid" diagnostics.** The matcher treats the `Type::Invalid` recovery sentinel as a wildcard, so downstream consumers don't re-fire the same problem.
- **Two modules can't accidentally share a type just because they spell its name the same.** Cross-module references now resolve to canonical `Module.Type` form before matching, so a `B.Shape` value is correctly rejected where `A.Shape` is expected.
