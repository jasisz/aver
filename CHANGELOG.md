...
- **A raised `[[verify.costly]]` budget applies however you spell the path, and an entry is no longer called stale over a file the run did not reach.** `aver verify "$PWD/main.av" --module-root .` compared the absolute file against globs anchored at the module root, so `files = ["main.av"]` matched nothing: the expensive case ran under the default budget, came back as `1 not answered`, the run exited 1, and `aver verify main.av --module-root .` in the same directory passed. Both sides of that comparison are now resolved on disk first, so the four ways to spell one file give one answer. Separately, an entry whose glob covers several files is no longer reported as `the fn may be stale` when this run verified only some of them — the function it names may well live in the file the run did not reach. Closes [#1200](https://github.com/jasisz/aver/issues/1200).
