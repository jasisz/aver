...
- **Static page head / foot as interned literals.** The previous `page()` built a 65-element list of small string fragments per request and ran `String.join` on it before sending bytes to the bridge. Almost all of that list was static HTML/CSS — content that never changes between views. Two new functions `pageHead()` and `pageFoot()` return that static portion as single string literals, lowered by WASM to data-section pointers, so every request returns the same interned bytes with zero allocation for ~3 KB of static prefix and ~250 B of static suffix. `page()` itself shrinks to an 8-element `String.join` over only the dynamic parts (rendered fractal, navBar, landmarks). Output bytes are identical; the win is per-request allocation count.
- **Run-length encoding of adjacent identical half-block cells.** 200×120 cells used to emit 24 000 separate `<i c=XY>▀</i>` tags per request, even when long horizontal runs of solid in-set black or smooth gradient repeated the same (top, bot) palette pair across many neighbours. `cellRow` now threads the current run's (curTop, curBot, runCount) through tail-recursion args; identical neighbours extend the run, colour transitions or row end flush a single `<i c=XY>▀▀▀…</i>` tag covering the whole run. CSS attribute selectors still match because the `c` attribute is unchanged. Live measurements: HTML response sizes drop **365 KB → 242 KB on fullView (-33%)**, 365 → 288 KB on seahorse (-21%), 365 → 310 KB on spiral (-15%); fullView wall time **~280 ms → ~107 ms**. Deep-zoom views with chaotic boundary see smaller wins because palette pairs change every cell.
