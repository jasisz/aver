...
A function whose effect list names `yield` is a process. You write it in direct style (ask, then the next step), but it never runs as written. Every call to an operation of a capability the program answers itself is a request, and the self tail call is a `Yield` request. The compiler cuts the function at each one into state types and pure answer functions under the reserved `__` namespace. A program that serves several processes writes processes, answer modules and job kinds, and puts a `[run]` table in its `aver.toml`. The loop that seats the processes, waits once per turn, answers the requests and starts the jobs is generated. The smallest program of that shape is one ticker, one clock and one job kind, at `tests/fixtures/run_guide_example/` in the repository. Every block below is cut from it as it stands in the file. The whole program checks, verifies and runs, and a test pins the blocks to it.
...
`Clock` is a capability module of the program. Beside the operation it declares the sum the answer is read through: `TickReply`, named after the operation, with `Now(R)`, `Later(Wait.Wake)` and `Then(Wait.Wake, R)`, where `R` is exactly the operation's result. The program writes that sum. A program never writes a `__` name, and the compiler generates none here. The door holds the sum to that shape: a missing or differently shaped `<Op>Reply` on a capability the program answers is `error[answer-shape]`, which prints the declaration to paste. Naming `Wait.Wake` needs no `depends [Wait]`, because `Wait` is a reserved module of the standard library:
...
The module depends on the capability it answers and on `Wait`, whose `Wait.Wake` its replies carry. It may omit `exposes`: the generated loop reaches its functions either way, and exports used by the manifest or the loop do not raise `warning[unused-expose]`. Write one function per operation, all threading one state `S`: `fn op(state: S, args) -> Tuple<S, Cap.<Op>Reply>`, plus a pure `fresh() -> S` the loop starts the module from. `Cap.<Op>Reply` is the sum the capability module declared beside the operation. The reply is `Now(v)`, an immediate answer; `Later(wake)`, permission to retry later; or `Then(wake, v)`, a saved answer to reveal when the wake fires. All three keep the state the function returned, so a `Later` is where the module records its own progress (an offset, a retry count, a deadline of its own) while the request itself stays where it is. An answer function is pure and non-trivial, so it carries a `verify` block like any other, with the tuple it returns on the right-hand side:
...
- `After(ms)`: once the turn's clock reading has reached the deadline the park set, `now + ms`. A reading that has fallen more than `ms` behind the moment of the park also fires it, so a clock that steps backwards cannot strand the request
- `Either(item, ms)`: on whichever of the two comes first, which gives a read with a deadline
...
**A job kind** is a capability of Work shape: `depends [Work]` and exactly these two operations, with `T` and `R` ordinary data of the program. Only the generated loop calls `begin` and `take`, and it reaches them without `exposes`. A capability a process asks, like `Clock`, exposes its operations and types because the process names them:
...
`work` is `(T) -> R` with no effect list. `task` is `(S) -> Option<T>`, where the turn takes its next task from. `started` is `(S, T) -> S`, called the moment `begin` has answered `Ok` for that task. `landed` is `(S, Result<R, String>) -> S`, where a job that is over goes: `Result.Ok(r)` for one that finished, `Result.Err(reason)` for one that never will. Either all three are declared or none is, and `task` and `started` name functions of one module. One start runs in this order: ask, `begin`, then `started`. Every task `task` offers in a turn is begun in that turn, because a job begun at the limit is queued, so `started` must remove the task it is handed or the turn never ends. A `begin` that answers `Err` records nothing, and the task is offered again in a later turn. The program states one law on its `started` function, named exactly `aStartedTaskIsNotAskedAgain`, and the generated loop cites it. The law says only that the task just started is not the next one offered, so the same task is never started twice in a row; it does not rule out a task coming back later. Here the seam lives in `Clocked`, beside the answers, because the tasks and the scores are one state:
...
**The generated coordinator** is requested by `[run]`. An empty table uses seated ids in slot order, admits every askable id, and stops on the stop flag or when no process is seated and no job runs. The compiler generates `__View` and `__Pending`. To customize a policy, name all four keys (`order`, `admit`, `stop`, `view`); naming only some of them is `error[run-binding]`. The entry lists only the modules its own source names. The loop loads its answer modules, job kinds, `Wait` and `Work` from the manifest:
...
In the view, `ready` is what the turn's one wait reported. `askable` is the ids whose wake has fired, in slot order, and the turn asks `admit` only about those. `jobs` is how many jobs the run holds a handle for. `stopping` is the stop flag as data. Everything else is generated into the entry module under `__`: the slot table, the one `Wait.poll` per turn, the clock reading, the dispatch, the job seam, the shutdown that cancels every job still held and every job a parked request waits on, `main`, and the laws that pin the table's invariants. The program names none of it, and reply sums keep their declared source names. The program writes no `main`. `aver verify` runs the generated laws beside the program's own, and `AVER_YIELD_DUMP=1 aver check main.av --module-root .` prints the generated Aver.
...
- a process takes no parameters and answers `Unit`. A yielding helper of the same module may take parameters; a tail call enters its protocol, while a non-tail call nests its state under `In<G>At<N>`
...
- stops may sit in bindings, as match subjects, inside arguments and inside match arms, and `?` after a request works. Mutual nesting, a request or a helper call inside `(a, b)!`, a helper in another module, and a function value live across a request are rejected by name
...
- wasm-gc: the runner and Wasmtime packs execute jobs on host threads, bound them by `max-jobs` with the same queue, and support cancellation. Raw modules expose `aver:work/v1`; the JavaScript adapter uses workers and drives a generated coordinator between waits. See `docs/wasm-work.md`.
- wasip2: jobs currently run inline at `begin`, and `max-jobs` is ignored with `warning[work-max-jobs-ignored]`. There `take` never answers `Ok(None)`, a job that never ends blocks the turn, and a job whose body fails stops the component. Generated loops keep `View.stopping = false` and end through their policy or normal exhaustion; explicit `Process.stopRequested` calls are rejected.
...
**Driving the protocol by hand.** The generated names are compiler-defined and callable, so a program without a `[run]` table can drive a process itself. For `loop` the compiler generates, in the same module: `__LoopClaimState` (one state sum per request kind, one variant per stop, holding the live variables), `__LoopYieldState`, `__LoopRequest` (one constructor per kind: the operation's arguments plus the state), `__LoopOutcome = Done(<result>) | Waiting(__LoopRequest)`, `__loopStart(<params>)`, `__loopAnswerClaim(__state, __answer)` per kind (state only when the operation returns `Unit`) and `__loopAnswerYield(__state)`. The original function is removed:
