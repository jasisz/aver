...
- **Oracle invariant axioms in proof export** — `aver proof --backend lean` / `--backend dafny` now ship a small block of trust-axioms covering the per-effect bounds Aver's runtime guarantees: `Random.int` stays in `[min, max]`, `Random.float` in `[0.0, 1.0]`, `Time.unixMs ≥ 0`. Your own theorems can `apply oracle_random_int_in_bounds <rng> <path> <n> <min> <max> (by decide)` instead of carrying those obligations through every law. Same slug names in both backends.
...
### Changed
- **Verify summary line breaks down by hostile bucket.** When a block has hostile-injected cases, the per-block bracket reads `(1/1 declared, 11/35 hostile, 3 skipped by `when`)` instead of a flat `mismatch` counter. Skipped cases show explicitly so a strict `when` doesn't silently hide coverage.
- **Per-block summary in JSON gains four counters.** `verify_summary.blocks[]` now carries `declared_passed`, `declared_failed`, `hostile_passed`, `hostile_failed` alongside the existing `passed` / `failed` / `skipped` / `total`. Tooling can split "law regression" (`declared_failed > 0`) from "hostile coverage gap" (`hostile_failed > 0 && declared_failed == 0`).
- **Cartesian cap.** Hostile expansion is capped at 10,000 cases per block — same ceiling the parser already enforces for declared `given` ranges. Over-budget blocks get a clear error pointing at the law and listing the projected size; tighten the `given`, add a `when`, or run that block without `--hostile`.
