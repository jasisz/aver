...
impl aver_rt::AverDisplay for FnStore {
    fn aver_display(&self) -> String {
        format!("FnStore({})", vec![format!("nameToId: {}", self.nameToId.aver_display_inner()), format!("byId: {}", self.byId.aver_display_inner())].join(", "))
    }
...
impl aver_replay::ReplayValue for FnStore {
    fn to_replay_json(&self) -> serde_json::Value {
        let mut fields = serde_json::Map::new();
            fields.insert("nameToId".to_string(), ReplayValue::to_replay_json(&self.nameToId));
            fields.insert("byId".to_string(), ReplayValue::to_replay_json(&self.byId));
        let mut payload = serde_json::Map::new();
        payload.insert("type".to_string(), serde_json::Value::String("FnStore".to_string()));
    ...
    fn from_replay_json(value: &serde_json::Value) -> Result<Self, String> {
        ...
        if type_name != "FnStore" {
            return Err(format!("$record type mismatch: expected FnStore, got {}", type_name));
        }
        ...
        Ok(Self {
                nameToId: <aver_rt::AverMap<AverStr, i64> as ReplayValue>::from_replay_json(fields.get("nameToId").ok_or_else(|| "$record FnStore missing field 'nameToId'".to_string())?)?,
                byId: <aver_rt::AverVector<FnDef> as ReplayValue>::from_replay_json(fields.get("byId").ok_or_else(|| "$record FnStore missing field 'byId'".to_string())?)?,
        })
    }
}
...
pub fn lookupVar(env: &aver_rt::AverMap<AverStr, Val>, name: AverStr) -> Result<Val, AverStr> {
    ...
    match env.get(&name).cloned() { Some(v) => { Ok(v) }, None => { Err((AverStr::from("undefined variable: ") + &name)) } }
}

/// Create an empty function store.
pub fn emptyFnStore() -> FnStore {
    crate::cancel_checkpoint();
    FnStore { nameToId: HashMap::new(), byId: aver_rt::AverVector::from_vec(aver_rt::AverList::empty().to_vec()) }
}
...
pub fn lookupFnId(fns: &FnStore, name: AverStr) -> Result<i64, AverStr> {
    crate::cancel_checkpoint();
    match fns.nameToId.get(&name).cloned() { Some(id) => { Ok(id) }, None => { Err((AverStr::from("undefined function: ") + &name)) } }
}
...
pub fn lookupFnById(fns: &FnStore, id: i64) -> Result<FnDef, AverStr> {
    crate::cancel_checkpoint();
    match fns.byId.get(id as usize).cloned() { Some(fd) => { Ok(fd) }, None => { Err(aver_rt::AverStr::from({ let mut __b = { let mut __b = aver_rt::Buffer::with_capacity((39i64) as usize); __b.push_str(&AverStr::from("undefined function id: ")); __b }; __b.push_str(&aver_rt::AverStr::from(aver_rt::aver_display(&((id.to_string()).into_aver())))); __b })) } }
}
...
pub fn lookupFnOption(fns: &FnStore, name: AverStr) -> Option<FnDef> {
    crate::cancel_checkpoint();
    match fns.nameToId.get(&name).cloned() { Some(id) => { fns.byId.get(id as usize).cloned() }, None => { None } }
}
...
pub fn lookupFn(fns: &FnStore, name: AverStr) -> Result<FnDef, AverStr> {
    crate::cancel_checkpoint();
    match lookupFnOption(fns, name.clone()) { Some(fd) => { Ok(fd) }, None => { Err((AverStr::from("undefined function: ") + &name)) } }
}

/// Build env map from parameter names and argument values.
#[inline(always)]
pub fn zipArgs(mut params: aver_rt::AverList<AverStr>, mut args: aver_rt::AverList<Val>, mut acc: aver_rt::AverMap<AverStr, Val>) -> aver_rt::AverMap<AverStr, Val> {
    loop {
        crate::cancel_checkpoint();
...
#[inline(always)]
pub fn mergeBindings(mut bindings: aver_rt::AverList<(AverStr, Val)>, mut env: aver_rt::AverMap<AverStr, Val>) -> aver_rt::AverMap<AverStr, Val> {
    loop {
        crate::cancel_checkpoint();
...
pub fn fnsToStore(fns: &aver_rt::AverList<FnDef>) -> FnStore {
    ...
    FnStore { nameToId: nameToId, byId: aver_rt::AverVector::from_vec(fns.to_vec()) }
}

/// Convert a list of FnDefs to a name->id map. Later defs shadow earlier ones.
#[inline(always)]
pub fn fnsToIdMap(mut fns: aver_rt::AverList<FnDef>, mut acc: aver_rt::AverMap<AverStr, i64>, mut idx: i64) -> aver_rt::AverMap<AverStr, i64> {
    loop {
        crate::cancel_checkpoint();
