...
| Map       | empty/set/get/len/has/keys/values/remove/entries/fromList + fused `Option.withDefault(Map.get(...))` / `match Map.get(...)` shapes ✅. K ∈ `{String, user-defined record, Int, Float, Bool}` ✅. Record-K field types ∈ `{Int, Float, Bool, String, nested record, List<T>, Vector<T>}` ✅ (List/Vector hash+eq emitted as per-T helpers; nested records force-registered as pseudo-K). **missing**: record-key fields of sum types |
