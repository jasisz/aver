...
- **The coordinator is generated: write the processes, the answer modules and three policies, and nothing else.** Put a `[run]` table in `aver.toml` — `order`, `admit`, `stop` and `view`, all four naming things in the entry module — and the compiler writes the loop for you, into that module, in the `__` namespace: the slot table, one instance number per request, the seating of one of every process, the wait set, the poll timeout, the dispatch through the answer modules, the job seam, the stop observation, the turn, `__runAll` and `main`. A `Later` parks its request where it stands and its state is discarded, so "a `Later` changes nothing" is true by construction; a `Wait.Wake.After(ms)` becomes the turn's poll timeout and a `NextTurn` makes that turn poll with zero. The view the three policies read is a record your program declares and the loop fills — `pending: Map<Int, Pending>`, `ready`, `jobs`, `room`, `stopping`, with one `Pending` constructor per process carrying the instance it waits on and its wake — so a policy's law can sample it; a view that is not that shape is `error[view-shape]`, which prints the declaration it wants. The loop also emits its own invariants as `verify` laws over the generated functions, so you do not write those either. Requirements the loop puts on your program are `error[run-binding]`: a process takes no parameters and answers `Unit`, each answer module declares a pure `fresh()`, the policies are pure, and the module does not write its own `main`. `AVER_YIELD_DUMP=1` prints the whole generated loop after the protocol. The worked example is `tests/fixtures/run_all_slice/` — five processes, three answer modules, one job kind — with `examples/concurrency/README.md` pointing at it; see "The coordinator" in `docs/language.md` and the `answer` / `task` / `landed` / `[run]` keys in `docs/services.md`.
