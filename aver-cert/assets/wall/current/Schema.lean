...
-- which the checker renders from the bytes it reads (never a package file).
import Module
import SchemaCore
...
/-- The single audited certificate proposition: the manifest's pinned hash is
    the delivered artifact hash recorded in the checker-rendered `Module.lean`, its
...
def Holds (m : Manifest) : Prop :=
  m.subject.artifactHash = _root_.CertModule.wasmSha256 ∧
