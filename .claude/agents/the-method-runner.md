...
5. Read ONLY the `--check-json` summary line. `universal` and `sorries` come straight from it. `universal:true` with `sorries:0` ⟺ closed (self-contained — no discovery). Set `checkPassed:true`.

Reporting — this is the important constraint:
- Return ONLY Aver-level facts: `checkPassed` (did `aver check` parse+typecheck), the `universal` boolean, the `sorries` integer, whether every helper passed its own bounded check (from the `--check` output), and a one-line note in Aver terms — the verbatim first `aver check` error if the pre-flight failed, else e.g. "helper `revDist` failed its bounded check" or "all helpers verified, target law still open". 
