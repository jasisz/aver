...
A process or an answer module ends the run with a reason by calling `Run.fail(message)`, an operation of the shipped capability `Run` (`stdlib/run.av`) whose effect is `Run.fail`. The turn is finished, the loop stops the way a stop does, cancelling the jobs parked requests wait on, and `Run.all()` answers `Err(message)`. The first call of a run is the one that counts. The loop reads the reason back with `Run.failure() -> Option<String>` after it seats the processes and after every turn. Both operations are recorded, `Run.fail` as output and `Run.failure` like `Process.stopRequested`, so a failed run replays to the same `Err`. The VM and generated Rust keep the reason in the `aver-rt` Run provider, one per program. Both wasm targets keep it in a global of the module. On wasm-gc the two operations also call the recorder imports `aver.run_fail` and `aver.run_failure`, which a live host answers with nothing and with the reason it was handed. `tools/wasm-work/host.mjs` does that. The certificate format does not admit these two imports yet. See [The coordinator](language.md#the-coordinator) for the rest.

`Run.lastTurn() -> Run.Turn` answers which turn this is (`turn`: 0 while the loop seats the processes, then 1, 2, … one per wait that returned), how long the loop's wait before the current turn took (`waitedMs`), and how long the turn before it worked, from the return of its wait to the start of this one (`workedMs`), in whole milliseconds of a monotonic clock. All three are 0 while the loop seats the processes, and `workedMs` is 0 in the first turn. Every answer in one turn reads the same `Run.Turn`; `turn` is how a program that answers several requests a turn counts each turn once. A process or an answer module calls it and declares the effect `Run.lastTurn`. A `main` that declares it in a program that runs no generated loop is refused at check time. When some function of the program declares `Run.lastTurn`, the loop calls `Run.waitStarts()` right before its wait and `Run.waitEnds()` right after it returns, one clock reading each. `Run` does not expose those two: only the generated loop calls them, and a program that calls or names one is refused at check time. A program that does not declare it gets the loop it had, reads no clock for it, and carries no `Run.Turn`. `Run.lastTurn` is recorded like `Time.unixMs` and the two marks like `Run.fail`, so a replay answers the recorded numbers. The VM and generated Rust keep the readings in the `aver-rt` Run provider. Both wasm targets keep them in five globals of the module. On wasm-gc the marks read the host's clock through `aver.run_wait_starts` and `aver.run_wait_ends`, and `Run.lastTurn` hands the numbers through `aver.run_last_turn`, which a live host answers unchanged. On wasip2 the marks read `wasi:clocks/monotonic-clock.now`. The certificate format does not admit these imports yet.

The entry module's own `effects [...]` is widened by what is generated into it: `Process.stopRequested`, `Time.unixMs`, `Wait.poll`, what the answer modules perform, `Work.cancel` when the program binds a job kind, `Run.failure` when a process or an answer module calls `Run.fail`, and the loop's internal `Run.waitStarts` and `Run.waitEnds` when some function of the program declares `Run.lastTurn`. The module boundary has to hold the generated loop as well as the processes the program wrote. `Time.unixMs` is there because the turn reads the clock once, after its one wait has returned and before it serves. A deadline is measured against that one reading, so a deadline that fell due while the turn was asleep is askable in that turn and not the next one. The next turn's wait is measured against it too. A slot keeps the deadline it asked for next to the reading it falls due at, so the turn never waits longer than the shortest request in the program, even when the clock steps backwards. A recording replays the reading.
