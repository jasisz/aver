...
- **An error propagated with `?` from inside an argument now returns from the function that wrote it, instead of ending the program.** `Disk.readText(Disk.readText(path)?)` — read a file name out of one file, then read that file — handed `Result.Err` back to the caller in compiled code, but under `aver run` the missing file ended the whole program with `main returned error` and exit 1. The caller's `match` on the result never ran, and neither did anything after it: an error the program was written to handle was reported as a failure of `main`. Binding the inner call to a name first (`name = Disk.readText(path)?`) always behaved correctly, which is what made the difference read as a quirk of nesting.

  `aver run` now agrees with compiled code and with the self-hosted interpreter on every spelling: a `?` anywhere in an argument list — first argument or a later one, one call deep or several — returns from the function whose body contains it. A failing `?!` product written the same way is fixed alongside it. `verify` runs on the same engine, so a case exercising either shape is now judged on the corrected behaviour.
...
- **A map that fills up on the wasm-gc backend now stops and says so, instead of hanging.** A `Map` compiled with `--target wasm-gc` has 16384 buckets and does not grow. Storing the 16385th distinct key left the insert walking those buckets forever looking for a free one: no output, no error, no end. Looking an absent key up in a map that full did the same. The insert now stops and reports the limit, and the lookup answers what it has always answered for a key that is not there — `Option.None`, the supplied default, `false` from `Map.has`.

  A WebAssembly trap carries no message of its own, so the limit rides in the name of the function that hit it: the backtrace reads `Map.set Map<Int,Int> in place (fixed capacity 16384, no resize)`. **`--optimize` costs you that name** — the optimizer drops the name section and folds the helper into its caller, so an optimized build reports the same stop as `<wasm function 12>`. Compiling a program that uses a `Map` with `--optimize` now prints a note saying exactly that. The capacity itself is unchanged, and raising it or growing the table on demand is still open work.

  This is reachable from outside the program on `--target wasip2`: a request's headers become a `Map` one header at a time, so a peer sending more than 16384 distinct header names stops the guest. The header maps are built by the same code and get the same limit.
...
- **Folding over a map of plain numbers no longer re-reads the whole map on every step.** Under `aver run`, a map that stays live while the program keeps allocating was read entry by entry every time the garbage collector looked at it, to establish that nothing in it had moved — once per entry per step, so the cost grew with the square of the input and the reading was nearly all of the time. A map whose keys and values are all plain values — `Int`, `Float`, `Bool`, and strings of five UTF-8 bytes or fewer — holds nothing that can move, so it is now left alone entirely. A fold building a `Map<Int, Int>` while it also builds strings it throws away took 0.77 s at 16,000 keys, 3.1 s at 32,000 and 12.2 s at 64,000; it now takes about 10 ms, 20 ms and 30 ms.
  ...
  This is the map counterpart of what lists already got, and it has the same limit. A map holding anything that lives on the heap — longer strings, lists, records, variants, tuples, nested maps, big integers — must still be read one entry at a time on every collection just to establish that nothing moved, and a fold over one is still quadratic. It is also about 12% slower than before: the same `Map<Int, String>` fold of 16,000 heap-backed values goes from 0.98 s to 1.10 s. So a program that fills a map of numbers gets much faster here, and one that fills a map of long strings pays a little for it.
...
- **Walking a list while another one grows no longer exhausts memory.** Recursing over a list with `[head, ..tail]` while an accumulator list grows — the shape most parsers and decoders in Aver are written in — made `aver run` allocate as though it were copying the whole remaining input on every single step. Copying a 64,000-element list needed 7.2 GB; it now needs 17 MB for a list of integers and 22 MB for a list of strings. A program that used to take the machine down on a few hundred thousand elements now holds flat, whatever the list holds.
  ...
  **Speed is the narrower half of this, and how much you get depends on what the list holds.** Storage built entirely out of immediate values — `Int`, `Float`, `Bool`, and strings of five UTF-8 bytes or fewer — is now skipped without being read at all, and that case really is linear: copying a `List<Int>` takes 34 ms at 16,000 elements, 49 ms at 64,000 and 71 ms at 128,000. `Bytes.fromHex` is a list of integers and lands on that side too. Every other element type — longer strings, records, variants, tuples, nested lists, big integers — must still be read one element at a time on every collection just to establish that nothing moved, and stays quadratic: the same program over `List<String>` takes 464 ms, 7.1 s and 28.3 s at those same three sizes. So if your program is slow on a large list of anything other than numbers, this release makes it fit in memory but does not make it fast.
