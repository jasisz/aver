...
- **Legacy NaN-boxed wasm32 emitter dropped from the playground build.** The browser host has been wasm-gc-only since 0.16.0; the standalone `aver_runtime.wasm` sidecar and its `aver_runtime_wasm` wasm-bindgen export are gone from `aver_bg.wasm`. New `wasm-legacy` Cargo feature gates `codegen::wasm` for the CLI's `--target wasm` / `--bridge {wasip1,fetch}` paths (still works), the `playground` feature explicitly excludes it. `aver_bg.wasm` shrinks 4811 KiB → 4646 KiB after `wasm-opt -Oz` (-165 KiB on first-load).
