...
- **`Int` is now arbitrary-precision (ℤ) on every backend — VM, Rust, and wasm-gc.** Integer arithmetic no longer overflows or wraps: a result that exceeds 64 bits keeps going instead of silently truncating, so `Int` is the mathematical integer everywhere, matching what proofs and `verify` already assumed. This closes the last place a verified Aver program could disagree with its proof at runtime. Small values keep a native-integer fast path; only genuinely large magnitudes pay for big-integer storage. One limit remains: an integer *literal* in source is still lexed as a 64-bit value, so a constant whose digits exceed that range is reached by computation or written as `Int.n("<digits>")` rather than typed directly; the lexer now says so (pointing at `Int.n`) instead of reporting a bare "invalid literal".
