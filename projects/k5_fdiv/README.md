...
| **Rounding — trunc** | the paper's `trunc(x,n)` truncation (§5.2, p.13) on the normalized model + its elementary lemmas — `domain/round.av` | 🟡 **seeded** — `trunc` modelled faithfully: the truncated significand integer is `floor(sigBits·2^(n-1) / 2^(width-1))`, a single Euclidean floor division, with the sign and the **stored** exponent carried through (which sidesteps the recursive binary-exponent function and is exact precisely because the model is already normalized, `s_x ∈ [1,2)`). Two elementary lemmas landed `universal` on the Lean kernel (`#print axioms ⊆ {propext, Classical.choice, Quot.sound}`, 0 sorries): **Lemma 7.2.1 (p.19)** `trunc(−x,i) = −trunc(x,i)` — truncation toward zero is sign-symmetric (the unconditional value-equality closes by the cone-`simp` rung); and **Lemma 7.2.6 (p.19)** `e(trunc(x,i)) = e_x` for `x ≠ 0` — trunc keeps the exponent (a definitional projection through the float record, closed by the generic conditional-keystone grind once the cross-module power-of-two recursion was wired well-founded into the dependency build). **Deferred** (the next slice — each needs the `Int.ediv` floor-bridge in a recursive-expo-free window figure): 7.2.2 (approximation `trunc(x,i) = x − ε`, `\|ε\| < 2^(e_x−i+1)`), 7.2.5 (monotonicity), 7.2.10 (composition `trunc(trunc(x,i),j) = trunc(x,i)`), 7.2.14 (integer-fits), 7.2.12/7.2.13 (the floor workhorses), and away/sticky |
