...
- **wasm-gc: Option-returning functions now verify on the wasm-gc backend.** A `verify ... => Option.None` case (and a `f(x) == Option.None` comparison written in a fn body) failed wasm-gc compilation with "Option constructor: instantiation `Option<T>` was not registered" while `aver check` and VM verify passed — a bare `Option.None` has no payload to fix its `T`, and the type checker stamped it generically instead of adopting the other side of the comparison. Equality against `Option.None` now infers the concrete side first and propagates its type.
