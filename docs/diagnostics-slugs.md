...
| `work-target` | error | A program with a Work-shaped capability — a job kind — was compiled or run on a wasm backend (`--target wasm-gc`, `--target wasip2`, `aver run --wasm-gc`, `aver run --wasip2`, `aver verify --wasm-gc`). A job kind is answered by a function of the program started off the turn, and neither wasm target lowers that yet. The bytecode VM and `--target rust` both run such a program. A capability answered with `answer = "Module"` is no longer refused anywhere. A program that performs `Wait.poll` or `Work.cancel` without declaring a job kind never reaches this gate: it is refused first by `error[capability-target-unsupported]` with `reason[standard-binding-unavailable]`, because neither wasm target binds the `Wait` and `Work` contracts. | Run the program on the VM or compile it with `--target rust`; the wasm-gc and wasip2 backends follow in a later change. |
