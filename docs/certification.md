...
> contracts). The current goal matrix certifies 21 exports across 14 manifest
...
> compiler now emits source-projectable scalar and Int literal-predicate
...
All of it is bundled behind **one final theorem** per certificate, `AverCert.Final.cert : Schema.Holds manifest`. The fixed, audited `SchemaCore.lean` contains the dependency-closed statement schema, while the thin `Schema.lean` shim adds the artifact-hash equality; `manifest` is a literal describing this artifact (hash, exports, contracts, profile, ABI, host-role tables, and artifact-level certificate root). The schema states each obligation over a typed source **domain** and **codomain** with explicit representation relations (`Dom`, `Cod`, `domRepr`, `codRepr`, `model`), so the same schema covers integer, projection and ADT classes. Certificates use public schema version `1` and package format version `1`. The manifest carries one `wall_id`: a domain-separated SHA-256 over the exact Lean toolchain and every audited wall filename and byte sequence. The verifier rejects unknown schema, format, or wall identities before building. A consumer never approves arbitrary proof scripts or a manifest-provided wall; they check the fixed theorem name and type, the artifact certificate root, the manifest literal, and the exact wall already embedded in the verifier. If statement approval required interpreting arbitrary Lean syntax, the compiler bug would just move into a certificate-auditor bug — the schema exists so that it cannot.
...
5. pins the cert-supplied `AverCert.Artifact.data` to the checker-reconstructed artifact data, type-ascribes the final theorem to `Schema.Holds manifest`, aliases the artifact-carried `Artifact.certificate` root, and runs the kernel's axiom collector against that artifact-level root. The fast witness projects already-checked host-role and per-fragment acceptance facts from this root after the exact data pin; if any fast check declines, the diagnostic witness recomputes the redundant lowering, byte-slice and acceptance mirrors for localization, but can never upgrade the verdict.
...
The header's *21 exports across 14 manifest classes* and this section's *nine
...
- **Float claims fail closed at the general-Wasm NaN boundary.** The current exact-bit Float face may certify payload-independent Bool results such as `f64.le`, but it does not certify a Float result containing `f64.add` or `f64.mul`. General WebAssembly permits more than one valid NaN sign/payload result for those arithmetic operations, while `floatBitsRepr` names one exact `UInt64`; treating that equality as an all-inputs theorem would overclaim. Both the Rust verifier and audited Lean plan checker enforce this boundary, so `floatAddGoal` and `floatMulAddGoal` remain explicit `source_level_only` exports. Re-admitting them requires either a relational result face (exact equality when the result is non-NaN, membership in the Wasm-permitted result set when it is NaN) or a separately declared and verified deterministic NaN profile.
...
As of schema 1, the `cert_goals` matrix has **21 certified exports and 0
