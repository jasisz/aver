...
fn proof_export_cross_module_refined_types_keep_distinct_predicates() {
    ...
    // own predicate into the Lean export. Pre-fix
...
#[test]
fn embedded_bytes_and_crypto_digest_preserve_refinements_in_the_proof_export() {
    let aver_bin = env!("CARGO_BIN_EXE_aver");
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
...
fn proof_export_lake_builds_red_black_tree_after_singleton_and_fuel_gates() {
    ...
    //
    // Sorry budget 1 (was 2): the `detect.rs` resolved-subject fix also admits
...
fn proof_export_gates_trace_projection_law_lhs_as_runtime_only() {
    ...
    // buffer, not the lifted fn's return. The lifted Lean fn
