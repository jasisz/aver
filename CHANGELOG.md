...
- **The call index a `given` stub receives now counts the calls of its own operation.** The second argument of a generative stub used to count every effect the case reached, so a `Time.unixMs()` or a `Tcp.writeNow` between two `Random.int` reads pushed the second read from index 1 to index 2 and a stub written as `match call` answered the wrong line. Each operation now numbers its own calls from 0 on each branch path, in `aver verify` and in the Lean and Dafny that `aver proof` exports, so a scripted stub is a reply script for one operation and adding a clock read or a log line to the code under test does not rewrite it. Request operations of a generated coordinator follow the same rule: each request kind counts its own requests, in-place effects do not move them, and a self yield still charges nothing. A stub that ignores its index is unaffected, and so are recording, replay and `.trace` assertions. A stub that branches on the index and shares a case with another operation has to be rebased on the new numbering; the shipped hostile profiles needed no change, and their descriptions ("the wall clock advances one second per call", "three false observations") are now true as written. Laws over a function reaching two or more operations export different index literals, with the same truth value. See "Stub signatures" in `docs/oracle.md`, which also gains a worked pattern for driving a socket state machine with a scripted peer.
