...
| `Tcp.{connect, close, writeLine, readLine, send, ping}` | `wasi:sockets/{instance-network, ip-name-lookup, tcp-create-socket, tcp}` (Phase 4 / 0.20 "Pulse" shipped). `__rt_tcp_connect` walks lazy-network init → resolve-addresses → async pollable loop → first-IPv4 → create-tcp-socket → start/finish-connect → pool slot allocation → `Tcp.Connection` materialise. `Tcp.close` drops streams + shutdown + drops socket + marks the slot free (idempotent). `writeLine`/`readLine` thread bytes through chunked blocking-write / 1-byte blocking-read against the pooled streams. `Tcp.send` orchestrates the full one-shot (connect + writeLine + readLine + close); `Tcp.ping` is the light connect + close wrapper. Connections live in a 256-slot wasm-gc array; `Tcp.Connection.id` is `"tcp-N"` for cross-backend portability. IPv6 yields from the resolver are skipped (first-IPv4-wins); no in-line `subscribe-duration` connect timeout in v1. |
