...
- **`aver verify` no longer prints a green summary while a whole file went unchecked.** A file whose program could not be run — a type error anywhere in it, a backend or provider failure, or a refusal by verify itself — was dropped from the counts, so a directory where one file failed and one passed reported `Summary: 1 module | 1 block | 1/1 cases passed | 0 failed` in green, and the `--json` summary was byte-identical to a run over a file that simply has no verify blocks. Only the exit code said otherwise. The summary line now carries a `| N file(s) not checked` member and is never green while N is above zero; `--json` carries `files_skipped` and `blocks_unchecked`; each unchecked file is listed with the number of verify blocks that went unchecked with it. When the fault is in a dependency the dependency is named rather than the entry, and the false `No verify blocks found in <entry>` is gone. Modules that did verify before the walk stopped now keep their results instead of being discarded with their failing sibling. A refusal by verify itself gets its own reason line instead of being reported as a type error and sending the user to `aver check`, which passes. A clean run prints exactly what it printed before. Found on the first external project. Reported by Robin Owens ([@n1bor](https://github.com/n1bor)).
