...
- `--pack wasmtime`: native self-contained wasm-gc directory pack. It emits the module, `manifest.json`, and a stripped `aver-wasmtime-host` carrying Wasmtime plus the exact custom providers selected by `aver.toml`; the destination needs no Aver, Cargo, Wasmtime installation, provider source, or project tree. The host is built for the current OS/architecture and runs `main`; see `docs/wasmtime-pack.md`.
