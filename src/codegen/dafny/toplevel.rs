...
fn emit_floor_window_support_stack(
    ...
    match figure {
        ...
        FloorWindowFigure::FloorPow2Cancel { pow_fn, .. } => {
            ...
            let pow = d(pow_fn);
            let (gs, a, b) = (&givens[0], &givens[1], &givens[2]);
            format!(
                "// Law: {fn_name}.{law_name} — exact-division cancel over a power-of-two divisor\nlemma {u}pow_pos(n: int)\n  ensures {pow}(n) >= 1\n{{ }}\nlemma {{:vcs_split_on_every_assert}} {u}pow_add(m: int, n: int)\n  requires m >= 0 && n >= 0\n  ensures {pow}(m + n) == {pow}(m) * {pow}(n)\n{{\n  if m > 0 {{\n    {u}pow_add(m - 1, n);\n  }}\n}}\nlemma {u}mul_mono(p: int, q: int, d: int)\n  requires p <= q && d >= 0\n  ensures p * d <= q * d\n{{ }}\nlemma {{:vcs_split_on_every_assert}} {u}div_lower(x: int, d: int, k: int)\n  requires d >= 1 && k * d <= x\n  ensures k <= x / d\n{{\n  assert x == d * (x / d) + x % d;\n  if k > x / d {{\n    {u}mul_mono(k, x / d + 1, d);\n    assert false;\n  }}\n}}\nlemma {{:vcs_split_on_every_assert}} {u}div_upper(x: int, d: int, k: int)\n  requires d >= 1 && x < k * d\n  ensures x / d < k\n{{\n  assert x == d * (x / d) + x % d;\n  if x / d >= k {{\n    {u}mul_mono(k, x / d, d);\n    assert false;\n  }}\n}}\nlemma {u}exact_div(x: int, d: int, q: int, r: int)\n  requires d >= 1 && x == d * q + r && 0 <= r && r < d\n  ensures x / d == q\n{{\n  {u}div_lower(x, d, q);\n  assert x < (q + 1) * d;\n  {u}div_upper(x, d, q + 1);\n}}\nlemma {u}mul_reassoc(x: int, y: int, z: int)\n  ensures x * (y * z) == y * (x * z)\n{{ }}\nlemma {{:vcs_split_on_every_assert}} {fuel_attrs} {main_thm}({params})\n  requires {when}\n  ensures {lhs} == {rhs}\n{{\n  {u}pow_pos({a});\n  {u}pow_add({a}, {b} - {a});\n  assert {pow}({b}) == {pow}({a}) * {pow}({b} - {a});\n  {u}mul_reassoc({gs}, {pow}({a}), {pow}({b} - {a}));\n  assert {gs} * {pow}({b}) == {pow}({a}) * ({gs} * {pow}({b} - {a}));\n  {u}exact_div({gs} * {pow}({b}), {pow}({a}), {gs} * {pow}({b} - {a}), 0);\n}}\n",
            )
        }
    }
}
