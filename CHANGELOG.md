...
- **Filling a map no longer depends on how the empty one was written.** Building a map by repeated `Map.set` through a fold made `aver run` allocate as though it were duplicating the whole map on every insert — 8.4 GB for 20,000 short string pairs, where the same program now needs 15 MB. The size of the map was never the trigger. The seed was: `mapInto(n, {})` was already free, and `mapInto(n, Map.fromList([]))` — how you spell an empty accumulator when the type has to come from somewhere other than a literal — was quadratic.

  A `Map.set` may only update its target in place when nothing else can still see that map, which the compiler decides before the program runs. It knew a map literal is a fresh map that nobody else holds; it did not know that `Map.fromList` is one too. So an accumulator seeded from `Map.fromList` was treated as possibly shared for the whole fold, and every insert preserved the map it was handed by copying it. It is now recognised as fresh, and the fold consumes its accumulator like the literal-seeded one always did.

  **Memory is the half this fixes.** Filling a map is still slower than it should be — 20,000 inserts take about 1.6 s under `aver run` against 9 ms compiled — for the same reason a list of strings is: every collection that sees a live map reads it entry by entry to establish that nothing moved. Passing the accumulator through a helper function that returns a map is also still quadratic, in memory too: a result handed back by a function is not yet known to be unshared, whatever the function does.
