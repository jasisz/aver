...
- **Slot types lifted into IR** as `FnResolution.local_slot_types` — every backend that needs typed locals consumes the same table instead of re-walking patterns. Surfaced in `aver compile --explain-passes` (resolve stage). Multi-arm tuple-of-Constructors matches and chained-Attr value access (`Domain.Types.TaskStatus.Blocked`) now flatten through one shared rewrite path used by both VM and wasm-gc.
