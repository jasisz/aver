...
**How long a turn waited and worked.** The loop's one `Wait.poll` of a turn happens inside the generated code, so a process cannot time it with `Time.unixMs` around its own calls. `Run.lastTurn()` answers a `Run.Turn` with three fields. `turn` says which turn this is: 0 while the loop seats the processes, 1 for the first turn after a wait, and one more for every turn after that. `waitedMs` is how long the wait before the current turn took, and `workedMs` is how long the turn before it worked, from the return of its wait to the start of this one, both in whole milliseconds of a monotonic clock. Every answer served in one turn reads the same `Run.Turn`, so a program that reports the numbers once per turn keeps the last `turn` it reported and skips a reading with the same one. A process or an answer module calls it and declares the effect `Run.lastTurn`, from anywhere, like `Run.fail`. While the loop seats the processes, before any wait has returned, all three are 0. In the first turn after a wait, `workedMs` is 0, because no turn came before it. A turn whose wait did not wait, because some request could be asked at once and the timeout was 0, reads a `waitedMs` near 0: only what the host took to return. Only a program run by `Run.all()` can call it: a `main` that declares `Run.lastTurn` in a program that generates no loop is refused at check time, because there is no turn to report. The loop measures only when some function of the program declares `Run.lastTurn`. It then marks its wait with two operations `Run` keeps to itself, one right before the wait and one right after, and each reads the clock once. They are internal to the generated loop: a program cannot call them or name them in an effect list. Every other program gets the loop it had before, with no clock reading, no new recorded effect, no new wasm import and no `Run.Turn` in its artifact. The readings are recorded like `Time.unixMs`, so a replay answers the recorded numbers. `tests/fixtures/run_last_turn/` has a ticker that parks on a 250 ms deadline three times and reads `Run.lastTurn()` after each tick.
...
- `Run.waitStarts()` in `__turn` and `__workHostObserve`, and `Run.waitEnds()` at the start of `__workHostStep`: generated only when some function of the program declares `Run.lastTurn`. They mark the turn's wait so `Run.lastTurn()` can say how long it took and which turn it is. `Run` does not expose them, and only a generated function may call them: a program that calls one is told it is not exposed, and one that names one in an effect list is refused.
