...
- **The documentation sizes `[work] max-jobs` and shows the bounded outbox a non-blocking write needs.** A job's slot is freed when its body ends, so a `Work.cancel` does not hand it back: the VM stops the body at its next cancellation check, wasm-gc traps it at the next epoch check, and generated Rust checks no flag and runs it to completion. Because `begin` refuses at the limit rather than waiting, a limit set to exactly the steady-state need refuses the retry that follows any owner-side error, and the docs now say how much headroom to leave on each backend and that `wasip2` has nothing to size. Beside `Tcp.writeNow` they describe the outbox a program carries for itself: a bounded queue per connection with an exact partial-write cursor, the `Sending` key registered only while bytes remain, overflow charged to the slow peer, and a bulk producer that renders its next payload once the queue is under a watermark instead of enqueueing a whole answer at once, so a healthy peer is not dropped for reading slower than the program produces. See "Jobs" and `Tcp.writeNow` in `docs/services.md`, and "Common patterns" in the language guide, whose sketch `llms.txt` and `aver agent-connect` carry as well.
