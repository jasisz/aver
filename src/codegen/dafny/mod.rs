...
pub fn transpile(ctx: &CodegenContext) -> ProjectOutput {
    ...
    // Built-in record types are pulled from the shared
    // `codegen::builtin_records` table. Same source of truth Lean uses;
    // until Dafny restructures to compute the body before the prelude
    // we always include the full set (`force_all = true`).
    ...
    sections.push(DAFNY_PRELUDE_AFTER_RECORDS.to_string());
