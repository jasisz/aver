...
`Wait.Wake` is the companion sum that a module answering a capability uses to say why it has no answer yet. It also decides when that request is asked again. `Item(Wait.Item)` parks the request on a socket or a job, and the request is asked again only in a turn whose wait reported that key. `After(Int)` parks it on a deadline that many milliseconds out. It is asked again once the turn's clock reading has reached the deadline, or once that reading has fallen further back than the deadline asked for, because a wall clock that steps backwards would otherwise leave the request waiting for a reading that never comes. `Either(Wait.Item, Int)` parks it on both at once, and it is asked again on whichever comes first. The item goes into the turn's wait set exactly as `Item` puts it there, and the deadline counts in the poll timeout exactly as `After` counts it, backwards clock included. `NextTurn` asks to be asked again on the next turn. A `Later` also keeps whatever state the answering module returned with it, so the offset a partial write reached, a retry count, or a deadline of the module's own survives the park.
...
False-positive readiness is why a `take` can answer `Ok(None)` after a wait already reported the job. A reported key means "ask", not "it is done". Whoever asked keeps the handle and asks again in a later turn. The generated coordinator does exactly that, and a handle it drops is a job that keeps running with nobody left to collect it. A `take` that answers `Err` is the other end of that. A job that was cancelled, whose body stopped, or whose id the engine has forgotten will never produce a payload, so the handle is finished with. The generated coordinator drops it and passes the error to the answer state through `landed` as `Result.Err(reason)`. The run continues, and what a rejection means is up to the program. A wait over jobs only is not woken by a job outside its own set either. The engine's signal is per engine, so the wait checks its own set again and goes back to sleep until one of its own jobs is ready or the timeout runs out.

The table answers "already taken" and "job cancelled" from a bounded number of dead slots: at most 4096, oldest forgotten first. A slot is dead once its answer was collected or its job was cancelled. A program that collects or cancels more jobs than that over its life gets `Err("work: unknown job")` for a handle whose slot has been forgotten. The alternative would be a store of tombstones that grows for as long as the program runs. A job that finished and was never collected or cancelled keeps its slot, because its answer is still there for whoever holds the handle. A job kind remembers the ids it minted exactly as long as the engine keeps their slots, so a handle the engine still knows is never mistaken for one that another job kind started.

The bytecode VM runs jobs. `begin` starts the bound function on its own thread and returns the handle at once. `take` answers `Ok(None)` while the job runs and `Ok(Some(result))` once it has finished. `Work.cancel` stops it, and `Wait.poll` returns as soon as a socket is ready, a job settles or the timeout elapses. `begin` never blocks the turn: at the job limit it answers `Err("work: job limit N reached")`. A second `take` of the same job answers `Err("work: job already taken")`, and taking a cancelled job answers `Err("work: job cancelled")`. A handle belongs to the job kind that started it. Because `Work.Job` is one type, a handle from one job kind type-checks as an argument to another kind's `take`. The runtime answers that with `Err("work: this job was not started by job kind 'K'")` instead of handing over an answer the program never asked for. When the program ends, jobs still running are cancelled and the runtime waits for them only briefly.

How many jobs may run at once is a deployment choice, so it also lives in the manifest. Without it, a program gets the host's own available parallelism:
...
Set `max-jobs` above the number of jobs the program keeps running, because a slot is freed when a job's body ends, not by `Work.cancel`. `begin` refuses at the limit instead of waiting for a slot, so a program at its limit reads `Err("work: job limit N reached")` in the turn that asked. `Work.cancel` sets the job's cancellation flag and drops its answer, and the running count falls only once the body settles. How long that takes depends on the backend. The VM's interpreter stops the body at its next cancellation check, wasm-gc traps it at the next epoch check, and generated Rust checks no flag, so there the body runs to completion. A retry after an owner-side error usually cancels a job and begins its replacement in the same turn. That asks for a slot the cancelled job still holds, so a limit set to exactly the steady-state need refuses the retry. Size the limit with that in mind. On the VM and on wasm-gc, one spare slot per job that may be retried at once covers the gap between the cancel and the next check. On the Rust backend the cancelled body holds its slot for its whole remaining run, so a program that may replace every job it is running needs twice its steady-state need. On `wasip2` there is nothing to size, because a job runs inline at `begin` and the key is ignored.
...
There are two differences from the VM. First, `Work.cancel` drops the job's answer and sets its cancellation flag, but generated Rust has no cancellation check, so the thread runs to completion in the background. On the Rust backend, cancelling detaches the work instead of stopping it, and the process exits without waiting for it. A program can observe one consequence: a cancelled job keeps counting against `[work] max-jobs` until its body ends. At a limit of one, a `begin` right after a `Work.cancel` answers `Err("work: job limit 1 reached")` on the Rust backend. The VM, whose interpreter stops the job at its next cancellation check, has the slot back. Second, replaying a recording serves the recorded answers without recomputing the job beside them, so for now the divergence check is done only on the VM.
...
`aver run --wasm-gc` and `--target wasm-gc --pack wasmtime` run jobs on host threads with separate Wasm instances and GC heaps. The compiled module is shared, and completed instances go back to a bounded pool. `begin` returns immediately, `take` may answer `Ok(None)`, and `[work] max-jobs` is enforced. `Work.cancel` interrupts a running body at an epoch check. `Wait.poll` watches sockets and job completions together, and an empty set waits for the requested timeout. Taken, cancelled and foreign-kind handles get the same answers as on the VM.

Raw wasm-gc exposes the versioned `aver:work/v1` ABI. The supplied JavaScript adapter uses Web Workers (or Node worker threads) and drives a generated coordinator through its post-wait step, returning to the event loop between turns. Task and result data are copied between instances with the existing typed capability helpers. GC references stay local, and live transport preserves full `Int`. See [Parallel Work on wasm-gc](wasm-work.md) for embedding, imports, cancellation and deployment.
...
`wasip2` keeps the current inline implementation. `begin` computes the result immediately, `take` can collect it at once, cancellation drops an uncollected answer, and every job key is ready immediately. `[work] max-jobs` still produces `warning[work-max-jobs-ignored]` on this target. The component has no `aver:work/v1` imports, and `--record` is still refused. A generated WASI loop keeps its signal stop flag false and ends through its policy or normal exhaustion. WASI 0.2 does not bind `Process.stopRequested`, and explicit calls are still rejected.

### Capabilities the program answers — `answer`, `task`, `started`, `landed` and `[run]`

A job kind is answered by the runtime running a pure function off the turn. A program can also answer a capability of its own *inside* the turn, from one of its own modules. That is one more key on the same `[[providers.bindings]]` entry:
...
`answer = "<Module>"` names one module of the program that answers **every** operation of that capability. The module has one function per operation, `op(state: S, a1: T1, …) -> Tuple<S, Cap.<Op>Reply>`, with the same state `S` across every operation of every capability it answers. `Cap.<Op>Reply` is a sum that the capability module declares beside the operation and exposes like any other type of the module. It is named after the operation with the first letter upper-cased (`tick` -> `TickReply`) and has exactly three constructors: `Now` carrying the operation's result type, `Later` carrying `Wait.Wake`, and `Then` carrying the wake followed by that result type. If an `answer` binding names a capability whose `<Op>Reply` is missing or has another shape, that is `error[answer-shape]`, and the message prints the declaration to paste. A capability that nothing answers is not held to this.

`answer` cannot be combined with `crate`/`package`/`factory`/`version`/`path` or with `work`. A capability is answered by exactly one of three things: a host package, a pure function of the program through the job engine, or a module of the program. Only a capability this program declares may carry `answer`. `Console`, `Disk`, `Tcp`, `Time`, `Wait`, `Work` and the rest are answered by the runtime's own providers, and the turn calls them itself. Marking an operation makes every call to it a request, which is legal only inside a function whose effect list names `yield`. See [Yielding functions](language.md#yielding-functions) for what the compiler generates from that, and [The coordinator](language.md#the-coordinator) for the loop that answers it.

`task`, `started` and `landed` are the three ends of the seam between a job kind and the answer state the turn already holds. `T` is the `begin` task type and `R` the `take` payload type. `task = "Module.function"` is `(S) -> Option<T>`, where the turn takes its next task from. `started = "Module.function"` is `(S, T) -> S`. The turn calls it with the offered task as soon as `begin` has answered `Ok(job)` for it. The start is recorded, so the next `task` ask is offered a different task and no task is started twice. `landed = "Module.function"` is `(S, Result<R, String>) -> S`, where a job that is over goes. A job that finished arrives as `Result.Ok(r)`. A job that never will finish (cancelled, stopped, or forgotten by the engine) arrives as `Result.Err(reason)`, because losing a job does not stop the run. One start happens in this order: ask, `begin`, `started`. A `begin` that answers `Err` records nothing, so the task stays in the answer state and is offered again once there is room to take it.

`task` and `started` must name functions of the same module, because the state one offers from is the state the other writes back to. `landed` names the module that owns the landed state. Either all three are declared or none is, and each names a pure function. They live on the `work` binding and not beside `answer`, because they belong to one job kind. Getting any of that wrong is `error[work-binding]`.

A program whose loop is generated states one law about the seam itself. The law goes on the `started` function and is named exactly `aStartedTaskIsNotAskedAgain`: `when task(state) == Option.Some(t)`, `task(started(state, t)) != Option.Some(t) holds`. The generated loop cites it by that name on its own `__consumed<K>` law. That makes "n starts in one turn take n distinct tasks" a fact of the program, not of the generator. A `[run]` program whose `started` function states no law by that name is `error[run-binding]`, and the message prints the block to write.

Every answered operation declares the exact reply shape `Now(R) | Later(Wait.Wake) | Then(Wait.Wake, R)`. `Later` waits for permission to ask again. `Then` waits for permission to reveal an answer that is already known. All three keep the state the answer module returned. `Then` uses the same wake gate as `Later` and resumes the process directly with its saved result.

`[run]` is the table that asks for the loop itself to be generated. Its answer modules, job kinds, `Wait` and `Work` are loaded from the manifest, so the entry lists only its own source dependencies. When none of the four keys is set, the defaults take seated ids in slot order, admit every askable id, and stop on the flag or when no process is seated and no job runs. `__View` and `__Pending` are generated.

To customize a policy, name all four keys. All four name things in one module: the entry module, which is where the loop is generated and what it can see. `order`, `admit` and `stop` are three pure policies: `order(view: View) -> List<Int>`, `admit(view: View, id: Int) -> Bool` and `stop(view: View) -> Bool`. `view` names the record they read, which the program declares and the loop fills: `pending`, `ready`, `askable`, `jobs`, `room` and `stopping`. `ready` is what the turn's one wait reported. `askable` is the ids this turn may ask, in slot order, which means every seated slot whose wake has fired. `order` and `admit` see the whole view, but the turn asks `admit` only about askable ids and never serves a slot that is not askable, whatever `order` returned.

Some mistakes are refused when `aver.toml` is read: a `[run]` table with only some of the four keys, a key that is not `Module.function` / `Module.Type`, or four names that do not share a module. Problems in what the loop is generated from (a process with parameters, a process written in another module of the program, an answer module with no `fresh`, a policy with effects) are `error[run-binding]`. A view record or marker sum that does not have the shape the loop fills is `error[view-shape]`, which prints the declaration it wants.

A program whose loop is generated may declare any number of job kinds, each with its own `task`/`started`/`landed` seam. The generated run keeps one job table, a `Map<Int, __Job>` whose `__Job` sum has one variant per kind, and one `[work] max-jobs` limit that the kinds share. So `max-jobs = 3` bounds all running jobs together, not three per kind. The turn first takes every kind's ready jobs and then starts jobs of every kind while room remains, so a job that landed this turn frees a slot the same turn can use.

The entry module's own `effects [...]` is widened by what is generated into it: `Process.stopRequested`, `Time.unixMs`, `Wait.poll`, both ends of every job kind, and `Work.cancel`. The module boundary has to hold the generated loop as well as the processes the program wrote. `Time.unixMs` is there because the turn reads the clock once, after its one wait has returned and before it serves. An `After` deadline is measured against that one reading, so a deadline that fell due while the turn was asleep is askable in that turn and not the next one. The next turn's wait is measured against it too. A slot keeps the `ms` its `After` asked for next to the reading it falls due at, so the turn never waits longer than the shortest request in the program, even when the clock steps backwards. A recording replays the reading.
