...
fn vm_compile_error(src: &str) -> String {
    ...
    let mut arena = Arena::new();
    let (resolved, symbols) = resolve_for_vm(&items);
    vm::compile_program(&resolved, &symbols, &mut arena, None)
...
#[test]
fn vm_list_get_and_result_from_option_keep_strings_alive() {
    let src = "fn extract(parts: List<String>) -> Result<String, String>\n    v = Vector.fromList(parts)\n    section = Result.fromOption(Vector.get(v, 1), \"Missing path segment\")?\n    match section == \"weather\"\n        false -> Result.Err(\"Not a weather endpoint\")\n        true  -> Result.fromOption(Vector.get(v, 2), \"Missing city\")\n\nfn main() -> Result<String, String>\n    extract([\"\", \"other\", \"Warsaw\"])\n";
    let (result, arena) = vm_run_with_arena(src);
    assert_eq!(
        result.to_value(&arena),
