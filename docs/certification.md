...
The header's *23 exports across 14 manifest classes* and this section's *nine certified classes* count the same certified functions at two different granularities, not two conflicting totals. The nine-class taxonomy below is the coarse byte-recognizer grouping — the source shapes the classifier recognizes. The emitted `cert-manifest.json` enumerates that same surface more finely as 23 exports across 14 manifest classes, and tags each export with the acceptance path it rides. Ten of the 23 exports ride the plan-first `accepted-artifact-v1` path — the seven scalar `expr-fragment-v1` fragments plus the `adt-constructor`, `verbatim-string-eq` and `verbatim-string-concat` bridge families; the remaining thirteen ride the legacy `legacy-witness-v1` byte-recognizer path. The nine byte-recognizer families and the fourteen manifest classes therefore describe the same work, and `artifact_bridge_counts` in the manifest reports the 10/13 split directly.
