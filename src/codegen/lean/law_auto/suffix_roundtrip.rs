...
pub(super) fn emit_string_escape_roundtrip_law(
    ...
    push(floored(
        &format!("private theorem {p}_str_ext {{a b : String}} (h : a.data = b.data) : a = b"),
        "cases a\ncases b\nexact congrArg String.mk h",
    ));
    push(format!(
        "private theorem {p}_data_append (a b : String) : (a ++ b).data = a.data ++ b.data := rfl"
    ));
    push(format!(
        "private theorem {p}_data_add (a b : String) : (a + b).data = a.data ++ b.data := rfl"
    ));
    push(format!(
        "private theorem {p}_empty_data : (\"\" : String).data = ([] : List Char) := rfl"
    ));
    push(format!(
        "private theorem {p}_ts_data (c : Char) : (Char.toString c).data = [c] := rfl"
    ));
    push(format!(
        "private theorem {p}_mk_cons (c : Char) (cs : List Char) :\n    Char.toString c ++ String.mk cs = String.mk (c :: cs) := rfl"
    ));
    push(format!(
        "private theorem {p}_mk_data_self (s : String) : String.mk s.data = s := rfl"
    ));
    push(floored(
    ...
    push(floored(
        ...
        &format!(
            "have h2 : (Char.toString a).data = (Char.toString b).data := by rw [h]\nsimpa [{p}_ts_data] using h2"
        ),
    ));
    push(format!(
    ...
    push(format!(
        "private theorem {p}_toCode_toString (c : Char) : Char.toCode (Char.toString c) = (c.toNat : Int) := rfl"
    ));
    push(floored(
        &format!(
            "private theorem {p}_inter_go_snoc (acc a : String) (xs : List String) :\n    String.intercalate.go acc \"\" (xs ++ [a]) = String.intercalate.go acc \"\" xs ++ a"
        ),
        &format!(
            "induction xs generalizing acc with\n| nil => simp [String.intercalate.go, {p}_str_append_empty]\n| cons x xs ih => simp [String.intercalate.go, ih]"
        ),
    ));
    push(floored(
        &format!(
        ...
        &format!(
            "cases xs with\n| nil => simp [String.intercalate, String.intercalate.go, {p}_str_empty_append]\n| cons x xs => simp [String.intercalate, {p}_inter_go_snoc]"
        ),
    ));
    push(format!(
        "private theorem {p}_inter_single (a : String) : String.intercalate \"\" [a] = a := rfl"
    ));
    push(format!(
        "private theorem {p}_inter_nil : String.intercalate \"\" ([] : List String) = \"\" := rfl"
    ));
    push(floored(
        &format!(
    ...
    push(floored(
        &format!(
            "private theorem {p}_charAt_of_drop {{s : String}} {{pos : Int}} {{c : Char}} {{rest : List Char}}\n    (h0 : 0 ≤ pos) (h : s.data.drop pos.toNat = c :: rest) :\n    String.charAt s pos = some (Char.toString c)"
        ),
        &format!(
            "have hneg : ¬ pos < 0 := by omega\nsimp [String.charAt, hneg, String.toList]\nexact ⟨c, {p}_getElem_of_drop h, rfl⟩"
        ),
    ));
    push(floored(
        &format!(
            "private theorem {p}_slice_self (s : String) (a : Int) : String.slice s a a = \"\""
        ),
        &format!("apply {p}_str_ext\nsimp [String.slice, {p}_empty_data]"),
    ));
    push(floored(
        &format!(
            "private theorem {p}_slice_snoc {{s : String}} {{a b : Int}} {{c : Char}} {{rest : List Char}}\n    (h0 : 0 ≤ a) (hab : a ≤ b) (h : s.data.drop b.toNat = c :: rest) :\n    String.slice s a (b + 1) = String.slice s a b ++ Char.toString c"
        ),
        &format!(
            "apply {p}_str_ext\nhave ha : ¬ a < 0 := by omega\nhave hb : ¬ b < 0 := by omega\nhave hb1 : ¬ b + 1 < 0 := by omega\nsimp [String.slice, ha, hb, hb1, {p}_data_append, {p}_ts_data, String.toList]\nhave h1 : (b + 1).toNat - a.toNat = (b.toNat - a.toNat) + 1 := by omega\nrw [h1]\nhave h2 : (s.data.drop a.toNat).drop (b.toNat - a.toNat) = c :: rest := by\n  rw [List.drop_drop]\n  have heq : a.toNat + (b.toNat - a.toNat) = b.toNat := by omega\n  rw [heq]\n  exact h\nexact {p}_take_succ_of_drop h2"
        ),
    ));
    ...
    push(floored(
        ...
        &format!("simp [{val_f}, {p}_toCode_toString, hge]"),
    ));
    ...
    push(floored(
        &format!(
            "private theorem {p}_hexDigit_data :\n    ∀ d : Nat, d < 16 → (hexDigit (d : Int)).data = [(hexDigit (d : Int)).data.head!]"
        ),
        "decide",
    ));
    push(floored(
        &format!(
            "private theorem {p}_hexVal_round :\n    ∀ d : Nat, d < 16 → {hexval} (Char.toString ((hexDigit (d : Int)).data.head!)) = some (d : Int)"
        ),
        "decide",
    ));
    ...
    push(floored(
        ...
        &format!(
            "simp [{ctl}, {p}_toCode_toString,{ladder_ne_shows}\n  show ¬ ((c.toNat : Int) < {t}) from by omega]"
        ),
    ));
    {
    ...
    // printable branch — generated hypothesis bundle per classifier arm.
    {
        ...
        body.push_str(&format!(
            "have hn1 : s.data.drop (pos.toNat + 1) = ({p}_escL cs).data ++ [{term_c}] := {p}_drop_succ_of_drop hdrop\n"
        ));
        body.push_str(&format!(
            "have hi1 : s.data.drop (pos + 1).toNat = ({p}_escL cs).data ++ [{term_c}] := by\n  rw [show (pos + 1).toNat = pos.toNat + 1 from by omega]; exact hn1\n"
        ));
        body.push_str(&format!(
            "have hAt0 : String.charAt s pos = some (Char.toString c) := {p}_charAt_of_drop hpos hdrop\n"
        ));
    ...
    // ticks + the separately-fueled readHex micro-run).
    {
        ...
        body.push_str(&format!(
            "have hdataE : ({cls} (Char.toString c)).data\n    = {esc_c} :: {uni_c} :: '0' :: '0' ::\n      (hexDigit ((c.toNat / 16 : Nat) : Int)).data.head! ::\n      (hexDigit ((c.toNat % 16 : Nat) : Int)).data.head! :: [] := by\n  rw [hesc, {p}_data_add, {p}_data_append, hA, hB]\n  rfl\n"
        ));
        body.push_str(&format!(
            "have hlen : ({p}_escL (c :: cs)).data.length = 6 + ({p}_escL cs).data.length := by\n  rw [{p}_escL_cons, {p}_data_append, List.length_append, hdataE]\n  rfl\n"
        ));
        body.push_str(&format!(
        ...
        let hex_a = "(hexDigit ((c.toNat / 16 : Nat) : Int)).data.head!";
        let hex_b = "(hexDigit ((c.toNat % 16 : Nat) : Int)).data.head!";
        let tail = format!("(({p}_escL cs).data ++ [{term_c}])");
        ...
        body.push_str(&format!(
            "have hi6 : s.data.drop (((pos + 1) + 1) + 4).toNat = ({p}_escL cs).data ++ [{term_c}] := by\n  rw [show (((pos + 1) + 1) + 4).toNat = pos.toNat + 1 + 1 + 1 + 1 + 1 + 1 from by omega]; exact hn6\n"
        ));
        body.push_str(&format!(
            "have hAt0 : String.charAt s pos = some (Char.toString {esc_c}) := {p}_charAt_of_drop hpos hdrop\n"
        ));
        body.push_str(&format!(
            "have hAt1 : String.charAt s (pos + 1) = some (Char.toString {uni_c}) := {p}_charAt_of_drop (by omega) hi1\n"
        ));
        for k in 1..=5 {
            let prev = if k == 1 {
                "fuel".to_string()
    ...
    let glue = format!(
        "have hdata : {subj}.data = {open_c} :: (({p}_escL {n}.data).data ++ [{term_c}]) := by\n  rw [{p}_data_add, {p}_data_add, {p}_producer_eq]\n  rfl\nshow {scan_f} (averStringPosFuel {subj} 1 {rank}) {subj} 1 1 [] = _\nrw [{p}_chunk_inv {n}.data {subj} 1 1 [] (averStringPosFuel {subj} 1 {rank})\n    (by omega) (by omega)\n    (by rw [hdata]; rfl)\n    (by simp [averStringPosFuel, hdata]; omega)]\nrw [{p}_slice_self]\nhave hval : String.intercalate \"\" ([] ++ [(\"\" : String)]) ++ String.mk {n}.data = {n} := by\n  simp [{p}_inter_single, {p}_str_empty_append, {p}_mk_data_self]\nrw [hval]\nhave hpos : ((1 : Int) + (({p}_escL {n}.data).data.length : Int) + 1) =\n    ((({prod_text}).length : Int) + 2) := by\n  rw [{p}_producer_eq]\n  show (1 : Int) + (({p}_escL {n}.data).data.length : Int) + 1 = ((({p}_escL {n}.data).data.length : Int) + 2)\n  omega\nrw [hpos]",
