...
- **`aver bench --save-baseline PATH` / `--compare PATH` / `--fail-on-regression`.** Capture a baseline once on a stable machine; subsequent runs compare current p50_ms / p95_ms vs. the stored values and exit non-zero when any gated metric exceeds its tolerance budget. Per-scenario tolerances configurable via `[tolerance] wall_time_p50_pct = 25.0` (default 20%), `wall_time_p95_pct = 35.0` (default 30%). The CI gate shape: `aver bench scenario.toml --compare baseline.json --fail-on-regression`.
