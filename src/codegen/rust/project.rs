...
pub fn generate_cargo_toml(
    ...
    // Iteration keeps release-like runtime semantics while trading peak
    // optimisation for a much shorter edit/build/run loop. This is a named
    // profile rather than a weakening of `release`: deployable artifacts keep
    // whole-program LTO and one codegen unit above.
    lines.push(String::new());
    lines.push("[profile.iteration]".to_string());
    lines.push("inherits = \"release\"".to_string());
    lines.push("opt-level = 1".to_string());
    lines.push("lto = false".to_string());
    lines.push("codegen-units = 256".to_string());
    lines.push("incremental = true".to_string());
...
pub fn generate_readme(name: &str) -> String {
    format!(
        "# {name}\n\n\
         Generated by `aver compile --target rust`. Re-running Aver preserves\n\
         byte-identical files, so Cargo can reuse its incremental state.\n\n\
         ## Fast edit/run loop\n\n\
         ```bash\n\
         cargo check\n\
         cargo run --profile iteration\n\
         ```\n\n\
         The `iteration` profile uses incremental compilation, no whole-program\n\
         LTO, 256 codegen units, and optimisation level 1. It is intended for\n\
         running realistic workloads while editing.\n\n\
         ## Final release\n\n\
         ```bash\n\
         cargo build --release\n\
         ```\n\n\
         The release profile retains whole-program LTO and one codegen unit for\n\
...
mod tests {
    ...
    use super::{generate_cargo_toml, generate_readme, runtime_dependency_line, toml_string};
    use crate::codegen::rust::composition::ProviderComposition;
    ...
    #[test]
    fn generated_project_keeps_iteration_and_final_release_profiles_distinct() {
    ...
    #[test]
    fn generated_readme_names_iteration_and_final_release_commands() {
        let readme = generate_readme("demo");
        assert!(readme.contains("cargo check\ncargo run --profile iteration"));
        assert!(readme.contains("cargo build --release"));
        assert!(readme.contains("whole-program LTO"));
    }
}
