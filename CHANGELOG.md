...
- **A wasm-gc program can now carry its own native Wasmtime host.** `aver compile app.av --target wasm-gc --pack wasmtime -o out/` emits the module, an exact manifest, and a stripped `aver-wasmtime-host` executable with Wasmtime and every custom Rust provider selected by `aver.toml` linked in. The destination runs that executable without Aver, Cargo, a Wasmtime installation, provider source, or the project tree. Before instantiation the host recomputes the module hash, import names and signatures, capability contract/model hashes, and linked provider provenance; it also carries and enforces the build project's runtime effect policy. Provider-specific release hosts are content-addressed and cached, so changing only Aver source repacks without rebuilding Rust. The first slice builds for the current OS/architecture and runs `main`; cross-platform standard hosts, provider cross-compilation, wasip2 packs, and AOT images remain separate follow-ups to [#1254](https://github.com/jasisz/aver/issues/1254).
