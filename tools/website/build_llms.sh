...
build_cli_md() {
    local out="$REPO_ROOT/docs/cli.md"
    {
        ...
        echo
        echo "Full toolchain reference for the \`aver\` binary. README points here for the per-command surface; deeper topics live in dedicated docs (\`docs/bench.md\`, \`docs/transpilation.md\`, \`docs/lean.md\`, \`docs/dafny.md\`, \`docs/wasm.md\`, \`docs/rust.md\`)."
        echo
        strip_skill_prompt "$SKILLS/aver-tooling.md"
    } > "$out"
    echo "wrote $out ($(wc -l < "$out") lines, $(wc -c < "$out") bytes)"
