...
- **`aver bench scenario.toml [--target=vm] [--json]` scenario harness.** Loads a TOML manifest pinning entry file, iterations, warmup, optional `[expected]` response-byte constraints, and optional `[tolerance]` regression budgets; runs the canonical pipeline + VM for `warmup + iterations` runs; emits a structured report. Human-readable text by default; `--json` produces the structured shape downstream tools consume. Directory mode (`aver bench bench/scenarios/`) globs every `*.toml`, runs them alphabetically, and emits one report per scenario — NDJSON when combined with `--json` for streaming consumers. Thirteen scenarios ship in `bench/scenarios/`: the eleven cargo-bench programs (fib, countdown, record, map_build, map_lookup, match_dispatch, string_interp, vector_ops, three newtype variants) plus `factorial` (smoke) and `fractal_seahorse` (deforestation regression). The cargo bench (`benches/comparison_bench.rs`) and `aver bench` read from the same `.av` files via `include_str!`, so VM/WASM/Rust/self-host comparison and the in-process scenario harness measure the exact same programs. Release script (`tools/release.py`) runs the full scenario suite in directory mode as a smoke gate before publishing.
