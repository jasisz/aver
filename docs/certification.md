...
4. authors a **kernel witness** binding the computed artifact hash, obligation count and export names. For every non-expression family, audited `CertDecode.lean` computes the `WCode` entries and carrier index directly from checker-regenerated `ArtifactBytes`; construct and projection claims also compute their struct field counts there. Once per module it additionally computes the plan-first **box/add/sub host-role table** from function signatures, instruction boundaries and uniqueness, and equates it to the manifest. Expression fragments retain their existing checked-plan/canonical-code-entry equality. Each obligation's **self index** (which module function the fuel semantics reasons about) is pinned into the byte-decoded export section by the whole-module `exportsAccounted` conjunct. The obligation `host` function is not removed yet: its String.eq/String.concat components remain the last Rust-supplied byte fact, with their in-kernel follow-up in flight. The witness also pins the obligation's **typed face** to the standard form of its byte-derived class: a `Nonempty Dom` proof for every obligation (so a `Dom := Empty` vacuity is rejected), and per class the codomain representation (`intRepr`/`verbatimRepr`), the integer domain representation with the byte-bound arity, and the projection domain representation — each by `HEq.rfl`. A manifest that weakens `Dom`/`Cod`/`domRepr`/`codRepr` fails one of these checks;
...
What a consumer trusts, exhaustively: the Lean kernel and pinned toolchain; `SchemaCore.lean` + `Schema.lean` + `CertDecode.lean` + `PlanCheck.lean` + `PlanLower.lean` + `PlanBytes.lean` + `WasmSlice.lean` + `ExprFragmentAccepted.lean` + `AcceptedArtifactCore.lean` + `AcceptedArtifact.lean` + `CertPrelude.lean` (hash-pinned in the manifest); and the checker orchestration above. `CertDecode.lean` now computes the non-expression code/carrier/struct facts and the module-wide add/sub/box host-role table from raw module bytes in-kernel. Rust rederivation, host-role classification and `validate_all` still run first as fail-fast belt-and-braces checks, but Rust no longer supplies the trusted add/sub/box table equality; the checker also no longer splices a Rust `self` list, because each obligation's self index is pinned into the byte-decoded export section by the whole-module export-accounting conjunct. The String.eq/String.concat helper components of `o.host` remain the last Rust-supplied byte fact, with a separate in-kernel follow-up in flight; this change deliberately does not remove `o.host`. `PlanBytes.lean` mirrors the expr-fragment canonical byte encoder in Lean, `WasmSlice.lean` checks that the checker-read module bytes expose each expr-fragment export's exact `FuncBinding` (function index, code index, type index and code-entry bytes), `ExprFragmentAccepted.lean` packages those checks into one accepted-export predicate, and `AcceptedArtifactCore.lean` exposes the dependency-closed artifact-level bridge machinery; the thin `AcceptedArtifact.lean` shim adds its `Schema.Holds` and decoded-facts conjunctions. Source-level `SymFragmentClaim`s must encode to byte-bound representation plans inside Lean; representation-only expr fragments are not admitted by that artifact-level bridge yet. The generated `Artifact.lean` is not trusted as a definition source; it is an artifact-carried witness whose `data` is pinned by the checker before its `certificate` root is audited. `certificate` is the artifact-carried wrapper around `acceptedWithFinal AverCert.Final.cert`; the checker separately type-ascribes `Final.cert` before using that root. Before any byte-derived fact is read — on both the producer and the verifier side — the module is passed through a full `wasmparser` validation (GC / tail-call / function-reference proposals enabled), so every subsequent rederivation is over a well-typed module. The in-kernel decoder does not yet replace the complete disassembler or validator. Each obligation's self index and the certified-export closure are already byte-derived in-kernel (the whole-module export-accounting and closure-isolation conjuncts). The Lean **model** of each function is data in the certificate: reading the model, or the named laws proven about it, is how a consumer knows *what* was certified; the schema keeps that reading surface small and fixed. The model itself is not byte-derivable — it is forced by the kernel proof instead (a tampered model makes the certificate fail to check), and its meaning is read, not re-derived.
...
As of schema 51, the `cert_goals` matrix has **23 certified exports and 0
