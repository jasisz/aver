...
- **A map iterates in one order now, and the exported proof reads that same order.** `Map.keys`, `Map.values` and `Map.entries` return entries sorted by key — that part was already true when you ran a program — but `aver proof` exported a model that kept insertion order instead. A map built by writing `z`, `a`, `m` has values `[2, 3, 1]`, which is what `aver verify` reports; the exported Lean said the values were `[1, 2, 3]` and the kernel accepted it. Either tool could confirm a claim the other refuted, on the same source, on the same commit.
  ...
  Two consequences to know about if you have integer or float keys. Integer keys sort numerically everywhere now — a map holding `2` and `10` iterates `[2, 10]`; running a program used to give `[10, 2]`, because that path sorted keys by their printed form rather than by the documented comparator. Float keys and non-scalar keys (tuples, variants, lists) are ordered at runtime by bit pattern and by printed form respectively, and neither is reproducible in a proof, so a law that *reads* iteration order over such a map is now refused by `aver proof` with a message saying why, on both the Lean and the Dafny backend. Order-blind laws over the same maps — `Map.len`, `Map.get`, `Map.has` — export exactly as before.
