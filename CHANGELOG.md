...
### Proof export — Lean
- **Verify `when` clauses are no longer silently dropped when they're stronger than (or orthogonal to) a refinement-lifted given's type invariant.** Pre-fix the proof emitter dropped the `when` premise whenever any law given was refinement-lifted (e.g. `given a: Int` becoming `∀ (a : Natural)` because the law body wraps `a` in `Natural(value = a)`), on the unchecked assumption that the user's `when` predicate was just restating the type's invariant. A user-written `when a >= 10` over `Natural` whose invariant is `a.val >= 0` would lose the `>= 10` part — the proof artifact would universally quantify over all Naturals rather than the user's tighter domain. Now both Lean and Dafny do a commutator-relaxed syntactic identity check (`when a >= 0` ≡ `0 <= a`) against the conjunction of lifted invariants and only drop the `when` when the match is bijective. Stronger / orthogonal predicates stay as premises. Equivalent ones still drop cleanly so the canonical `examples/refinement/natural.av`-style cases keep the one-line `∀ (a : Natural) (b : Natural), add a b = add b a` shape.
- **Private `match n { L -> base; _ -> rec(n-1, ...) }` countdown recursions emit as native Lean defs instead of fuel-encoded helpers, with the precondition inferred from the single external caller's surrounding guards.** Fibonacci's `fibTR` shape — the canonical Aver TCO worker — used to land on the `def fibTR__fuel` opaque-helper path because Lean's wf elaboration couldn't prove `(n - 1).natAbs < n.natAbs` without knowing the domain. The classifier now splits closed-world IntCountdown fns (declared in a module with no explicit `exposes`, or absent from its `exposes` list) into a native aux that carries the caller-derived precondition + a thin wrapper preserving the original signature; recursive callsites discharge their preservation obligation via `(by omega)` synthesized at codegen, termination closes on `Int.natAbs n`. For `fibTR`: `fib`'s `match (n < 0) { false -> fibTR(n, ...) }` arm yields `(h_dom : n ≥ 0)` (the `Lt` comparator is flipped to `Gte` so the predicate stays positive — same `Spanned<Expr>`-as-predicate representation opaque-types' smart-constructor invariant uses). For a caller wrapping `worker(n)` in nested `match (n > 2) { true -> match (n < 500) { true -> worker(n) }}`, the aux gets `(h_dom : n > 2 ∧ n < 500)`. Single-caller assumption is the issue-84 simplification — analog to opaque types having one smart constructor. The wrapper's fall-through for inputs outside the inferred domain returns the body's literal-arm value, unreachable in well-formed Aver. Exposed fns, multi-caller fns, fns whose caller passes a non-identity arg, and shapes with leading let-bindings keep the fuel encoding. (#84)
...
- **Mutual-recursion SCCs with a `List` / `Vector` sizeOf measure emit as a native `mutual ... termination_by ... end` block** instead of routing through a fuel-bounded helper-and-wrapper pair. The user's measure (`sizeOf x` over each sizeOf-relevant param, lex-paired with the classifier's SCC rank) goes directly into the `termination_by` clause; `decreasing_tactic` + a `simp_wf; (try simp_all); omega` chain closes the obligations on the shapes the existing classifier already accepts. BigInt, grok_s_language, and mission_control emit `mutual ... end` blocks with no `__fuel` suffix functions cluttering the proof file. SCCs with a recursive ADT measure (`Term` etc.) or a tail-recursive accumulator pattern (`[x] + acc`) — plus the specific `roomList` / `roomListItem` shape that calls an intra-SCC fn from inside a `s!"..."` interpolation — fall back to the fuel encoding, so no example regresses against pre-PR Lean output.

### Proof export — Dafny
- **Mutual-recursion SCCs emit as native `decreases` tuples.** An Aver SCC like BigInt's `addDigits → addLeft → addStep → addDigits` no longer routes through a fuel-bounded encoding — each fn emits as a plain Dafny `function ... decreases <size_measure>, <rank>` where the size measure is the sum of `|seq_param|` for every `List`/`Vector`/`String` param and the rank comes from the recursion classifier's existing topo sort over "same-measure" callees. Dafny verifies termination via the lexicographic tuple and can symbolically unfold the SCC to ground terms during proof obligations, removing the fuel ceiling that used to cliff out for large literals (BigInt's `10⁹`). Falls back to the fuel encoding when an SCC member has no `List`/`String` parameter to measure on.
- **Bounded-∀ universal laws over mutual recursion close as real proofs.** A `verify <fn> law` with `given a: Int = [...]` + `given b: Int = [...]` over a mutual-rec SCC now emits the universal lemma as a bounded `∀ (a, b: Int)` with `requires a == k1 || ... ` + a case-split body that dispatches each `(a, b)` pair to its per-sample lemma. Universal `add_commutative` for BigInt is a verified theorem over the declared domain (was `assume {:axiom}` on trust pre-PR). Lean parity: bounded ∀ closed by `rcases` + `native_decide` per case. BigInt: 18 verified + 5 errors → 36 verified + 0 errors, with all 25 per-pair sample lemmas as real proofs (no `assume` body anywhere).
