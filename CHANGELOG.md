...
- **A certificate can now say, in the kernel, that the plan it certifies IS the function you wrote.** Exports certified through the compute face used to be certified against a plan the compiler derived from your source; that the plan and your function were the same thing rested on the compiler, not on a proof. Each such export now carries a source-bridge theorem — the plan's model applied to your arguments equals your function applied to them — which the verifier re-checks at its declared statement and audits for stray axioms. `aver cert check` and `aver cert verify` report `source-bridges: N of M credited` beside the law-claims count, `aver cert explain` prints `model: plan ≡ <Module>.<fn>` on each bridged export, and a law about fully bridged functions now conjoins those bridges, so one checked name covers the law, the bytes, and the identification. Shapes the encoders do not reach (nested records, non-Int record fields, Float, String, ADT arguments) stay unbridged and say so.
