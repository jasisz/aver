//! Native WASI 0.2 lowering for `Tcp.readNow`.
//!
//! The non-blocking twin of `Tcp.readSome`: one `input-stream.read(maxBytes)`
//! call, which never waits. An `Ok` with no bytes means nothing is available
//! right now and becomes `Ok(None)`; the `closed` stream error is clean EOF and
//! becomes `Ok(Some(empty))`, the convention `readSome` uses; any other stream
//! error poisons the pool slot because the stream position is no longer
//! trustworthy.
...
pub(in crate::codegen::wasm_gc) struct TcpReadNowIndices {
    pub fn_type: u32,
    pub fn_idx: u32,
    ...
    /// `Result<Bytes, String>`: the helper builds the `Bytes` through that
    /// shape's Ok factory and lifts the payload into the option result.
    pub result_bytes_type_idx: u32,
...
pub(in crate::codegen::wasm_gc) struct TcpReadNowHelperFns {
    pub parse_id_fn: u32,
    pub cabi_realloc_fn: u32,
    /// `wasi:io/streams.[method]input-stream.read`.
    pub read_fn: u32,
    /// `__rt_result_bytes_string_ok(list) -> Result<Bytes, String>`.
    pub result_bytes_ok_fn: u32,
    /// `__rt_result_option_bytes_string_some(bytes)`.
    pub result_some_fn: u32,
    /// `__rt_result_option_bytes_string_none()`.
    pub result_none_fn: u32,
    /// `__rt_result_option_bytes_string_err(message)`.
...
pub(in crate::codegen::wasm_gc) fn emit_tcp_read_now(
    indices: &TcpReadNowIndices,
    helpers: &TcpReadNowHelperFns,
) -> Function {
    ...
    function.instruction(&Instruction::Call(helpers.read_fn));
    function.instruction(&Instruction::LocalGet(retptr));
    function.instruction(&Instruction::I32Load8U(mem1));
    function.instruction(&Instruction::If(BlockType::Empty));
    // Err(closed) is clean EOF and yields Some(empty). Err(last-operation-
    // failed) poisons the slot.
    function.instruction(&Instruction::LocalGet(retptr));
    ...
    function.instruction(&Instruction::LocalSet(data_len));
    // Ok with no bytes: nothing is available right now.
    function.instruction(&Instruction::LocalGet(data_len));
    function.instruction(&Instruction::I32Eqz);
    function.instruction(&Instruction::If(BlockType::Empty));
    function.instruction(&Instruction::Call(helpers.result_none_fn));
    ...
    // Materialise Bytes through the Result<Bytes, String> Ok factory (which
    // owns the packing policy) and lift its payload into Some.
    function.instruction(&Instruction::LocalGet(list));
    function.instruction(&Instruction::Call(helpers.result_bytes_ok_fn));
    function.instruction(&Instruction::StructGet {
        struct_type_index: indices.result_bytes_type_idx,
        field_index: 1,
    });
    function.instruction(&Instruction::Call(helpers.result_some_fn));
    restore_bump(&mut function, saved_alloc, helpers.bump_alloc_ptr_global);
    function.instruction(&Instruction::End);
    function
}
